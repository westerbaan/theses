...
That is: the relations~$x \mathrel{E_{f,\varepsilon}} y
     \Leftrightarrow 
     \| x - y \|_f \leq \varepsilon$
form a subbase for the uniformity.
\begin{point}%
...
\item the ultranorm uniformity on~$W$
        and the uniformity on~$W$ induced by~$\overline{V}$
        are related by
    $E^W_{f,\varepsilon} = \widetilde{E_{f,\varepsilon}}$
        for~$\varepsilon \geq 0$ and np-map~$f\colon \scrB \to \C$,
        (where~$E^W_{f,\varepsilon}$ is the subbase of the ultranorm
    uniformity for~$W$ from~\sref{dils-ultranorm};
         $E_{f,\varepsilon} \equiv E^V_{f,\varepsilon}$
         and~$\widetilde{E_{f,\varepsilon}}$ is
         the subbase for the uniformity on~$\overline{V}$
         defined in~\sref{dils-uniformity-completion}.)
\end{enumerate}
The last requirement implies
    that on a compatible extension~$W$
    the ultranorm uniformity and
    $\overline{V}$ induced uniformity coincide.
...
Later, we will apply Zorn's lemma to the compatible extensions
    ordered in this way.  Before we continue
    with the extensions we need a lemma on the entourages of~$\overline{V}$.
\end{point}
\begin{point}{Lemma. $\bigcap_{n \in \N} 
        \widetilde{E_{f,\varepsilon+\frac{1}{n}}}=
        \widetilde{E_{f,\varepsilon}} $}%
For the moment assume~$(x_\alpha)_\alpha
    \mathrel{\widehat{E_{f,\varepsilon+ \frac{1}{n}}}} 0$
    for all~$n \in \N$.
Then~$f([x_\alpha,x_\alpha])\leq \varepsilon + \frac{1}{n}$
    for sufficiently large~$\alpha$
    and so
    for each~$n \in \N$
    we have~$\lim_\alpha f([x_\alpha,x_\alpha]) \leq \varepsilon + \frac{1}{n}$.
    Thus~$\lim_\alpha f([x_\alpha,x_\alpha]) \leq \varepsilon$.
...
\begin{point}{Induction base case}%
    ...
    To show~$E^{V_0}_{f,\varepsilon} = \widetilde{E_{f,\varepsilon}}$
        we need a lemma.
\end{point}
...
By \TODO{} it suffices to show that a net
    $(x^\gamma)_\gamma$ over~$\sigma(W)$
    converges in the ultranorm uniformity to~$0$
    if and only if it converges to~$0$ in~$\overline{V}$.
By definition, for each~$\gamma$, there exist
    norm-bounded nets~$(x^\gamma_\alpha)_\alpha$ in~$W$
    such that~$x^\gamma_\alpha \to x^\gamma$ in~$\overline{V}$.

Suppose~$x^\gamma \to 0$ in~$\overline{V}$.
To show it also vanishes in the~$\sigma(W)$-ultranorm,
    pick any~$\varepsilon >0$ and np-map~$f\colon \scrB \to \C$.
As the $W$-ultranorm uniformity
    coincides with induced uniformity of~$\overline{V}$,
    there is an entourage~$\delta \in \tilde\Phi$
    such that $x \mathrel\delta 0$ implies~$f([x,x])\leq \varepsilon$
        for any~$x \in W$.
Find~$\gamma_0$ such that for all~$\gamma \geq \gamma_0$
we have~$x^\gamma \mathrel{\nicefrac{\delta}{2}} 0$.
Pick any~$\gamma \geq \gamma_0$.
For this~$\gamma$, find~$\alpha_\gamma$
    such that~$x^\gamma_\alpha \mathrel{\nicefrac{\delta}{2}} x^\gamma$
    for~$\alpha \geq \alpha_\gamma$.
So~$x^\gamma_\alpha \mathrel\delta 0$
    and~$f([x^\gamma_\alpha, x^\gamma_\alpha]) \leq \varepsilon$
    for~$\alpha \geq \alpha_\gamma$.
Consequently
\begin{equation*}
    f([x^\gamma,x^\gamma]) = f(\uwlim_\alpha [x^\gamma_\alpha,x^\gamma_\alpha])
            = \lim_\alpha f([x^\gamma_\alpha,x^\gamma_\alpha]) \leq \varepsilon
\end{equation*}
and so~$x^\gamma$ vanishes $\sigma(W)$-ultranorm.
To prove the converse, assume~$x^\gamma \to 0$ in~$\sigma(W)$-ultranorm
    uniformity.
Let~$\delta \in \tilde\Phi$ be any entourage on~$\overline{V}$.
By equivalence of the uniformities on~$W$,
    there are np-maps~$f_1,\ldots,f_n\colon \scrB\to\C$
    and~$\varepsilon > 0$
    such that~$f_i ([x,x]) \leq \varepsilon$
    for all~$1 \leq i \leq n$
    implies~$x \mathrel{\nicefrac{\delta}{2}} 0$.
Find~$\gamma_0$ such that
$f_i([x^\gamma, x^\gamma]) \leq \frac{1}{2} \varepsilon$
        for~$\gamma \geq \gamma_0$ and~$1 \leq i \leq n$.
Pick any~$\gamma \geq \gamma_0$.
As~$f_i([x^\gamma,x^\gamma])
    = \lim_\alpha f_i([x^\gamma_\alpha,x^\gamma_\alpha])$,
there is an~$\alpha_0$ such that
    $f_i([x_\alpha^\gamma,x_\alpha^\gamma]) \leq \varepsilon$
    and so~$x^\gamma_\alpha \mathrel{\nicefrac{\delta}{2}} 0$
    for all~$\alpha \geq \alpha_0$.
As~$x_\alpha^\gamma \to x^\gamma$ in~$\overline{V}$
there is some~$\alpha_1$ with~$
x^\gamma \mathrel{\nicefrac{\delta}{2}} x^\gamma_\alpha$
for~$\alpha \geq \alpha_1$
and so as there exists some~$\alpha \geq \alpha_0,\alpha_1$
we conclude~$x^\gamma \mathrel\delta 0$.
Thus~$x^\gamma$ vanishes in~$\overline{V}$.
We have shown~$\sigma(W)$ is a compatible extension.
\end{point}
\begin{point}{Limit step}%
    Assume~$\mathscr{V}$
    ...
    This turns~$W$ into a pre-Hilbert~$\mathscr{B}$-module
        ...
        which are contained in some single compatbile extension far enough up
        in the chain.
\end{point}
