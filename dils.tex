...
\begin{point}{Definition}%
    ...
    Pick any orthonormal basis~$(e_i)_{i \in I}$ of~$X$
        and~$(d_j)_{j\in J}$ of~$Y$.
    We have~$X \cong \ell^2((p_i)_{i \in I})$
        and~$Y \cong \ell^2((q_j)_{j \in J})$
            for~$p_i = \left<e_i,e_i\right>$
            and~$q_j = \left<d_j,d_j\right>$,
            see \sref{hilbmod-el2}.
The \Define{(self-dual exterior) tensor product} of~$X$ and~$Y$
    is the self-dual Hilbert~$\scrA \otimes \scrB$-module
    $\Define{X\otimes Y}
    = \ell^2((p_i\otimes q_j)_{i,j \in I\times J})$.
\end{point}
\begin{point}{Examples}%
...
\end{point}
\begin{point}{Remark}%
Usually a non-self-dual exterior tensor product
    is defined for arbitrary Hilbert C$^*$-modules~$X$ and~$Y$,
    say over~$\scrA$ and~$\scrB$ respectively,
    by completing~$X \odot Y$ with respect to the
    following~$\scrA\otimes\scrB$-valued inner product.
\begin{equation*}
    [x_1 \otimes y_1, x_2 \otimes y_2] = \left<x_1,x_2\right>\otimes
                \left<y_1, y_2\right>.
\end{equation*}
In this general case  it is hard to show the  inner product is definite:
...
In our situation, where~$X$ and~$Y$ are self-dual
    ...
    we can identify~$X \odot Y$
    as a submodule of~$X \otimes Y$
    \todo{finish note}
\end{point}
\end{parsec}
