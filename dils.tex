...
If~$V$ is normed, $B$ is said to be \Define{bounded}
...
Write~$\Define{\ell^2((p_i)_{i \in I})}$
    for the set of~$I$-tuples~$(b_i)_{i \in I}$
        from~$\scrB$
    such that~$b_i \in p_i \scrB$
    and the partial sums of~$\sum_{i \in I} b_i^*b_i$ are bounded.
...
\begin{point}{Proof}%
By~\sref{hilbmod-el2}
    we can find an ordinal~$\beta$
    and projections~$(p_\alpha)_{\alpha \in \beta}$ from~$\scrB$
    such that~$X \cong \ell^2((p_\alpha)_{\alpha \in \beta})$.
\end{point}
\begin{point}%
As a warming up, assume~$\beta = n$ for some~$n \in \omega$.
\TODO{...}
\end{point}
\end{point}
\end{parsec}
...
\begin{parsec}%
\begin{point}%
Suppose~$X$ is a self-dual Hilbert~$\scrA$-module
    and~$Y$ is a self-dual Hilbert~$\scrB$-module
    for some von Neumann algebras~$\scrA$ and~$\scrB$.
The algebraic tensor product~$X \odot Y$
    is a right-$\scrA \otimes \scrB$-module
    with~$(x\otimes y)\cdot (a\otimes b) = (xa) \otimes (yb)$.
It has a~$\scrA \otimes \scrB$-valued inner
    product fixed by~$[x\otimes y, x'\otimes y']
        = \left<x,x'\right> \otimes \left<y,y'\right>$.

This inner product on~$X \odot Y$ is definite
    and can be extended to its norm completion.
This is a Hilbert C$^*$ which is called the exterior tensor product
    of~$X$ and~$Y$.
For arbitrary  Hilbert C$^*$-modules it is actually quite difficult
    to perform this construction:
    Lance discusses the difficulties in \cite[Ch.~4]{lance}.
In our situation, where~$X$ and~$Y$ are self-dual
    \emph{and}~$\scrA$ and~$\scrB$ are von Neumann algebras
    it is much easier.
\end{point}
\begin{point}{Theorem}%
Suppose~$\scrA$ and~$\scrB$ are von Neumanna algebras.
For any self-dual Hilbert $\scrA$-module $X$
    ...
    together with a~$\scrA \otimes \scrB$-linear
    inner product preservering map~$\otimes \colon X \odot Y \to X\otimes Y$
    with the following universal property:
    for every self-dual~Hilbert~$\scrA\otimes\scrB$-module
    $Z$ with bounded~$\scrA \otimes \scrB$-linear
        $T\colon X \odot Y \to Z$,
    there is a unique bounded~$\scrA \otimes \scrB$-linear
        map~$\hat{T}\colon X \otimes Y \to Z$
    with~$\hat{T}(x \otimes y) = x \otimes y$
    for all~$x \in X$ and~$y \in Y$.

For such~$X \otimes Y$ we have the following.
\begin{enumerate}
    ...
    \item If~$(e_i)_{i \in I}$ is an orthonormal basis of~$X$
                ...
                then~$(e_i \otimes d_j)_{i,j\in I\times J}$
                is an orthonormal basis of~$X \otimes Y$.
\end{enumerate}
\begin{point}{Proof}%
If~$X = \{ 0\}$ or~$Y = \{ 0 \}$
    then~$X \otimes Y = \{0\}$
    does the job trivially.
For the remainder assume both~$X$ and~$Y$ contain non-zero vectors.
\begin{point}[exttensor-ultranorm-dense]{Ultranorm dense}%
First we show the image of~$X \odot Y$ is ultranorm dense in~$X \otimes Y$.
Write~$Z$ for the ultranorm closure of the image of~$X \odot Y$.
By \sref{hilbmod-projthm}~$Z$ is a self-dual Hilbert submodule of~$X \otimes Y$
    and~$Z \oplus Z^\perp \cong X \otimes Y$
    via~$\vartheta\colon(x,y) \mapsto x+y$.
Thus it is sufficient to show~$Z^\perp = \{0\}$.
Reasoning towards contradiction, suppose~$z' \in Z^\perp$ and~$z' \neq 0$.
Also pick~$z \in Z$ with~$z \neq 0$.
Define~$T_1,T_2\colon X\otimes Y \to Z$
by~$T_1(\vartheta(y,y')) = y$
and~$T_2(\vartheta(y,y')) = y + z\left<z',y'\right>$.
Then~$T_1 \neq T_2$, but for all~$x\in X$ and~$y \in Y$
    we have~$T_1(x\otimes y) = T_2(x\otimes y)$,
    contradicting the univeral property of~$X \otimes Y$.
Thus~$Z = X \otimes Y$ and so the image of~$X \odot Y$
is ultranorm dense.
\end{point}
\begin{point}{Uniqueness}%
Assume there are self-dual Hilbert~$\scrA\otimes \scrB$-modules
$X \otimes_1 Y$ and~$X \otimes_2 Y$
with inner product preserving~$\otimes_1 \colon X \odot Y \to X \otimes_1 Y$
    and~$\otimes_2 \colon X \odot Y \to X \otimes_2 Y$
    that both satisfy the universal property.
By the universal property of~$\otimes_1$ applied to~$\otimes_2$,
    there must exist a unique~$U \colon X \otimes_1 Y \to X \otimes_2 Y$
    with~$U x\otimes_1 y = x \otimes_2 y$.
Similarly, there is a~$V \colon X \otimes_2 Y \to X \otimes_1 Y$
    with~$V x\otimes_2 y = x \otimes_1 y$.
Clearly~$\id_{X \otimes_1 Y} x \otimes_1 y = VU x \otimes_1 y$
    and so by the uniqueness in the universal property for~$\otimes_1$
    we get~$VU = \id$. Similarly~$UV = \id$.
It remains to be shown~$U$ preserves the inner product.
As
\begin{equation*}
    \left<x \otimes_1 y, x' \otimes_1 y'\right>=
    [x \otimes y, x' \otimes y'] =
    \left<x \otimes_2 y, x' \otimes_2 y'\right> 
    = \left<U x \otimes_1 y, U x'\otimes_1 y' \right>
\end{equation*}
for all~$x,x' \in X$ and~$y,y' \in Y$.
Thus since the images of~$\otimes_1$ is ultranorm dense
    and \TODO{uwlim inner product}
    we see~$U$ preserves the inner product.
\end{point}
\begin{point}{Existence}%
By self-duality
    there is an orthonormal basis~$(e_i)_{i \in I}$ of~$X$,
    see \sref{dils-selfdual}.
Similarly there is an orthonormal basis~$(d_j)_{j \in J}$ of~$Y$.
Define~$p_{ij} = \left<e_i,e_i\right>\otimes\left<d_j,d_j\right>$
    and~$X \otimes Y = \ell^2((p_{ij})_{i,j\in I\times J})$,
    see \sref{hilbmod-el2}.
Write~$w_{ij} \in X \otimes Y$
    for the basisvector corresponding to~$p_{ij}$.

Pick any~$x \in X$ and~$y \in Y$.
We are going to define~$x \otimes y$.
Write~$x_i = \left<e_i,x\right>$
and~$y_j = \left<d_j,y\right>$.
Recall~$\sup_{I_0} \sum_{i \in I_0}x_i^*x_i = \left<x,x\right>$
    where~$I_0$ ranges over the finite subsets of~$I$.
Similarly~$\sup_{J_0} \sum_{j \in J_0}y_j^*y_j = \left<y,y\right>$.
By \TODO{otimes normal}
    we see~
\begin{equation*}
    \left<x,x\right> \otimes \left<y,y\right>
    = 
    ...
    \bigl(\sum_{i \in I_0}
    x_i^*x_i \bigr) \otimes
    \bigl(\sum_{j \in J_0} y_j^*y_j
    \bigr)
    =
    \sup_{(I_0,J_0)}
    \sum_{\substack{i \in I_0\\ j \in J_0}}
    x_i^*x_i \otimes y_j^*y_j.
\end{equation*}
Thus~$(x_i\otimes y_j)_{(i,j) \in I\times J}$
    is a square summable
    and so we may define
\begin{equation*}
    \otimes\colon X\odot Y\to X\otimes Y \quad\text{by}\quad
    x \otimes y \ =\  \sum_{i,j} w_{ij} (x_i \otimes y_j).
\end{equation*}
It is easy to see~$\otimes$ is a module map.
Finally~$\left<x,x'\right>\otimes\left<y,y'\right>
        = \left<x\otimes x', y\otimes y'\right>$
    follows from~\TODO{normal otimes} % CHECK!
    and the definition of the inner product on~$\ell^2$.
\end{point}
\begin{point}{Universal property}%
By construction, the~$\scrA\otimes \scrB$-linear span
    of~$\{w_{ij}\}_{i,j}$ is ultranorm dense in~$\ell^2$.
As~$e_i \otimes d_j = w_{ij}$,
    the image of~$X\odot Y$ is ultranorm dense in~$X \otimes Y$.
Now
\sref{selfdual-completion-univ}
gives us the desired universal property.
(In fact, we see~$X\otimes Y$ is isomorphic to the ultranorm completion
of~$X \odot Y$.) \qed
\end{point}
\end{point}
\end{point}
...
Then there is a nmiu-isomorphism
$\scrB^a(X) \otimes \scrB^a(Y) \cong \scrB^a(X \otimes Y)$.
\begin{point}{Proof}%
