...
By~$\sref{central-carrier-formula}$ we have
\begin{equation*}
\cceil{p} \cceil{q} \ =\  \cceil{p} \sup_{b \in \scrA} \ceil{b^* q b}
            \ =\  \sup_{b \in \scrA} \cceil{p}\ceil{b^* q b}
\end{equation*}
and so~$\cceil{p} \ceil{b^*qb} \neq 0$
    for some~$b \in \scrA$.
In the same way, there is some~$a \in \scrA$
    with~$ 0 \neq \ceil{a^*pa}\ceil{b^*qb}$.
    Thus \TODO{$cd = 0 \Leftrightarrow \ceil{c^*c}\ceil{dd^*} = 0 $}
    $p(ab^*)q \neq 0$.
\end{point}
...
\begin{parsec}%
\begin{point}%
To compute the Paschke dilation of a corner,
...
of~$p$ is given by~$(\cceil{p}\scrA\cceil{p}, h_{\cceil{p}}, h_p)$.
\begin{point}{Proof}%
Let~$(\scrA \otimes_{h_p} \scrA_p, \varrho, h)$
    denote the Paschke dilation constructed in~\sref{existence-paschke}.
We proceed roughly in two steps:
    first we will show~$\scrA \otimes_{h_p} p\scrA p$
    can be identified with~$\scrA p$
    and then we prove~$\scrB^a(\scrA p) \cong \cceil{p} \scrA$.
\begin{point}%
