...
Thus for any finite subset~$S \subseteq E$ we have
\begin{equation*}
...
$ \sum_{e \in S} \left<x,e\right>\left<e,x\right>
    \leq \left<x,x\right>$.
As a corollary we get~$\|\sum_{e \in S} e\left<e,x\right> \| \leq \|x\|$
and~$\sum_{e \in S}\| e\left<e,x\right>\|^2_f \leq \|x\|^2_f$
for any np-map~$f\colon \scrB\to \C$.
For finite~$S,T \subseteq E$ we have
\begin{align*}
    \bigl\| \sum_{e\in S} e \left<e,x\right> - \sum_{e\in T} e \left<e,x\right> \bigr\|_f^2
    = \sum_{e\in S\Delta T} \| e \left<e,x\right> \|_f^2,
\end{align*}
which must get small as~$S\cap T$ get large.
Thus the partial sums of~$\sum_{e \in E} e \left<e,x\right>$
are ultranorm-Cauchy and norm bounded by~$\|x\|$.
Thus by assumption this sum converges in the ultranorm uniformity.
