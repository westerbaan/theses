...
It is independant of representatives
...
Extending addition is more involved, but straight-forward:
...
Assume~$\scrA$ is a von Neumann algebra.
    ...
    Here we used that~$\left<x, (\,\cdot\,)x\right>$,
        $M_n\varphi$ and 
    $d \mapsto (a_i^*da_j)_{ij}$ are all normal.
    The first is normal by~\sref{hilb-suprema}
        and the second by~\sref{mn-vna}.
...
Show that in this case~$\dim \scrK' = n\cdot m$ Kraus operators suffice.
\end{point}
\end{parsec}
...
\end{equation}
We can apply this on~$\varrho'(a)^*\varrho'(a)$
...
Thus $\lambda S_1= S_2$ for some~$\lambda \in \C$,
...
\begin{point}[ess-uniq-pur]{Exercise (Essential uniqueness of purification)}
    ...
    Show  that the general case follows from the special case that the dilations are minimal.
\end{point}
\end{parsec}
\begin{parsec}%
\begin{point}%
    We are ready to state the universal
        ...
        which we will use to generalize it to arbitrary ncp-maps.
\end{point}
\parpic[r]{
...
\end{point}
\begin{point}{Example}%
Later, in \sref{paschke-tensor}, we will see
...
\end{equation*}
Will we be able to show that~$\bigcup_n V_n$ is finally complete?
Not yet.
...
Using \sref{hilbmod-ordersep} again 
...
\begin{align*}
    ...
    A_2' & \ \equiv \  \frac{1}{1+ \langle y_\alpha,y_\alpha \rangle}
     ...
     \frac{1}{1+ \langle y,y \rangle}.
\end{align*}
A straight-forward computations shows
...
\begin{equation*}
    \omega(\ketbra{y}{y})
     ...
     \ = \ \| \omega \| \| y\|^2
\end{equation*}
and so from \sref{order-separating-norm} it follows~$\| \ketbra{y}{y} \| \leq \|y\|^2$
    and so
\begin{align*}
    \| \ketbra{z}{y} \|^2
...
it is sufficient to show the right equality in
    ...
    \end{equation*}
    which indeed holds by~\TODO{prod states sep}.
\end{point}
\begin{point}%
...
From this it follows by~\sref{tensor-uniqueness},
that there is a
    nmiu-isomorphism~$\vartheta\colon \scrB^a(X) \otimes \scrB^a(Y) \cong \scrB^a(X \otimes Y)$
...
Some straight-forward computations
