...
So there are~$n \in \N$, np-maps~$f_1, \ldots, f_n \colon \scrB\to \C$,
    $x_1, \ldots, x_n \in X$
    such that~$\| g - g' \| \leq \varepsilon$,
    where~$g'(T) \equiv \sum_{i=1}^n f_i(\langle x_i,T x_i\rangle)$.
Clearly~$g'$ is ultranorm continuous,
    so there is an~$\alpha_0$ such that for all~$\alpha \geq \alpha_0$
    we have~$|g'(\ketbra{x_\alpha}{y})| \leq \varepsilon$
    and so~$g(\ketbra{x_\alpha}{y}) $. \TODO{finish}
\end{point}
\end{point}
\end{point}
\end{parsec}
