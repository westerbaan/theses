...
We want to complete~$V$ into a self-dual Hilbert~$\scrB$-module.
...
Using the norm boundedness we can extend
    the module structure and inner product of~$V\cong V_0$ to~$V_1$.
...
\begin{equation*}
   V \cong V_0 \ \subseteq \ V_1 \ \subseteq\  V_2\  \subseteq\  V_3 \ \subseteq \ \cdots\ \subseteq \ \bigcup_{n \in \N} V_n.
\end{equation*}
...
\begin{equation*}
   V \cong V_0 \subseteq V_1 \subseteq \cdots \subseteq V_\omega \subseteq
...
Define~$V_\omega = \sigma(\bigcup_{n \in \N} V_n)$,
 $V_{\omega+1} = \sigma(V_\omega)$, \ldots.
...
\end{point}
\begin{point}{$X$ is a pre-Hilbert~$\scrB$-module}%
...
Assume~$(x_\alpha)_\alpha$,
    ...
    Let~$(x_\alpha)_\alpha$ be a representative for~$\tau$.
    Then for any np-map~$f\colon \scrB \to \C$
    we have~$0 = f(\left<(x_\alpha)_\alpha,(x_\alpha)_\alpha\right>)
    = \lim_\alpha \|x_\alpha\|^2_f$ and so~$\tau = 0$.
    We have shown~$\left<\,\cdot\,,\,\cdot\,\right>$ is definite.
\end{point}
