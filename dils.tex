...
A straight-forward computation
    ...
    map~$\varphi\colon \scrA \otimes_{h_p} p \scrA p \to \scrA p$
   fixed by~$\varphi(\alpha \otimes pap) = B(\alpha,pap) \equiv \alpha pap $.
...
From this and
    $\left<\alpha \otimes p, a\otimes p\right> = 
        p \alpha^* a p
        =\left< \varphi(\alpha \otimes p), \varphi(a \otimes p)\right> $
        we see~$\varphi$ preserves the inner product.
...
where the sums converge ultrastrongly in~$\scrA$
    and so the last converges ultranorm in~$\scrA p$ as well.
Thus~$E$ is an orthonormal basis.
\end{point}
\end{point}
\end{point}
\end{parsec}
