...
\TODO{reference? (ook andere plekken)}
...
Let~$X$ be a Hilbert~$\scrB$-module for a C$^*$-algebra~$\scrB$.
Assume~$D \subseteq X$ is an ultranorm dense submodule.
Then: for every~$x \in X$,
    there is a net~$x_\alpha$ in~$D$
    with~$x_\alpha \to x$ ultranorm
    and~$\|x_\alpha\| \leq \|x\|$.
\begin{point}{Proof}%
This proof is based on a
    proof of the regular Kaplansky density theorem
...
\begin{align*}
   ...
   \bigl(\langle y_\alpha,y_\alpha \rangle - \langle y,y \rangle\bigr) 
    \frac{1}{1+\langle y_\alpha,y_\alpha\rangle }
            \frac{\langle y,y \rangle}{1+\langle y,y \rangle},
\end{align*}
which vanishes ultraweakly by \sref{innerprod-ultraweak}
    and \TODO{ultraweak prod bounded}.
In a similar way one sees~$A_1' \to 0$ ultraweakly.
Using \sref{innerprod-ultraweak} and \TODO{} directly,
    one sees~$A_2$ and~$A_2'$ converge ultraweakly to~$0$. \qed
\end{point}
\end{point}
\end{point}
\end{parsec}
...
\end{point}
\end{point}
\begin{point}[dilationspace-dense-subset]{Exercise}%
Let~$\varphi \colon \scrA \to \scrB$ be an ncp-map between von Neumann algebras.
Assume~$U \subseteq \scrA$ and~$V \subseteq \scrB$
    are ultrastrongly dense subalgebras.
Show that the linear span of
$ U \otimes V  =  \{ u \otimes v;\ u \in U, \ v\in V\}$
is ultranorm dense in~$\scrA \otimes_\varphi \scrB$.
    (Hint: reason in a similar fashion to \sref{exttensor-dense-subsets}
        and \sref{ultranorm-continuity-ext-tensor}
        using \sref{kaplansky}.) \TODO{}
\end{point}
\end{parsec}
...
(\scrA_1 \otimes \scrA_2) \otimes_{\varphi_1\otimes \varphi_2} (\scrB_1 \otimes \scrB_2)$.
Finally, we show that the natural bijection
