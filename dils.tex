...
\ar@{..>}[u]_-{\sigma}
    ...
    such that for every (other\footnote{Other choice of possibly the same such triplet. See \sref{other-remark}.})
...
I feel, however, that this would be harder to follow for a reader
    unfamiliar with universal properties:
        the new triplet would be introduced without being directly related
            to the previous triplet and the reader has to
            notice that the assumptions on both triplets are the same.
...
Hence, by~\sref{inner-product-basic},
    we know that~$\|\,\cdot\,\|_f$ is a seminorm
        (ie.~a non-positive-definite norm).
\end{point}
...
We are ready to show that every ncp-map has a Paschke dilation.
    That is: for every ncp-map~$\varphi$
        there is a triplet~$(\scrP, \varrho, h)$
        with~$\varphi = h \after \varrho$ via~$\scrP$
            ...
            that for any (other) such triplet~$(\scrP', \varrho', h')$
            there is a unique ncp-map~$\sigma\colon \scrP' \to \scrP$
...
\end{point}
\begin{point}{30}%
For our purposes (the dilation of arbitrary ncp-maps~$\scrA \to \scrB$),
...
\end{point}
\begin{point}{20}[kaplansky-hilbmod]{Theorem}%
