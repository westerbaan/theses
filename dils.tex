...
Use \sref{nmiu-between-type-I} and~\sref{stinespring-theorem} to show
    ...
    Conclude that any CPTP superoperator \TODO{ref}~$
...
We already encountered Hilbert C$^*$-modules
    ...
    in my twin's thesis. (They appear in \sref{chilb-basic}.)
...
\begin{point}{Definition}%
    Let~$\scrB$ be a C$^*$-algebra and~$X$
        ...
        A \Define{$\scrB$-valued inner product}\index{$\scrB$-valued!inner product} on~$X$
        ...
        \end{enumerate}
        A $\scrB$-valued inner product is called \Define{definite}\index{$\scrB$-valued!inner product!definite}
...
A~\Define{$\scrB$-sesquilinear form}\index{$\scrB$-valued!sesquilinear form} on~$V$
...
We will use a different construction than Paschke used.
...
There are several uniformities on~$\scrB$ --- two of which
    ...
    The \Define{ultrastrong uniformity}\index{uniformity!ultrastrong} on~$\scrB$
is fixed by the seminorms~$\|b\|_f = f(b^*b)^{\frac{1}{2}}$
        where~$f\colon \scrB \to \C$ is an np-map.
        The \Define{ultraweak uniformity}\index{uniformity!ultraweak} on~$\scrB$
...
We call the uniformity on~$X$
    ...
    for np-maps~$f\colon \scrB \to \C$
    the \Define{ultranorm uniformity}\index{uniformity!ultranorm}.
\begin{point}%
...
\begin{point}[dfn-selfdual-basis]{Definition}
    ...
    \begin{enumerate}
        ...
        \item A sequence $(b_e)_{e \in E}$ in~$\scrB$
                ...
                are bounded.
        \item $E$ is an \Define{(orthonormal) basis}\index{(orthonormal) basis} if
...
\begin{point}[dils-selfdual]{Theorem}%
    ...
    For any pre-Hilbert~$\scrB$-module~$X$,
        ...
        \begin{enumerate}
            \item $X$ is \Define{self-dual}\index{self-dual} ---
...
is said to be \Define{bounded}\index{$\scrB$-valued!sesquilinear form!bounded}
...
\end{equation*}
is an order seperating set of ncp-maps, see \sref{separating}.
...
This is happends if and only if~$a \ceil{\varphi} a^* \leq p^\perp$
...
by~$\Define{\ketbra{x}{y}}\index{$\ketbra{x}{y}$} z = x \left<y,z\right>$.
\begin{point}%
...
From \sref{innerprod-ultraweak} it follows easily that
...
Let~$X$ be a Hilbert~$\scrB$-module
    ...
    Write~$\Define{V^\perp}$ for the \Define{orthocomplement}\index{orthocomplement} of~$V$, defined by
\begin{equation*}
...
Let~$\scrB$ be a von Neumann algebra
    ...
    and furthermore~$\ceil{b_i^*b_i} \leq p_i$ for every~$i \in I$.
...
\item
Perhaps any two orthonormal bases have a common refinement?
Take~$\scrB = M_3$.
...
For any two projections~$p,q \in \scrB$,
    either~$p \lesssim q$ or~$q \lesssim p$.
\begin{point}{Proof}%
...
For any self-dual Hilbert $\scrA$-module $X$
    ...
    $X \otimes Y$, called the \Define{(self-dual exterior) tensor product}\index{tensor product!Hilbert $\scrB$-modules},
...
This definition of~$X \otimes Y$ seems to depend
    ...
    We show it does not, in \sref{ext-tensor-uniqueness}.
...
it is sufficient to show the right equality in
    ...
    \end{equation*}
    which holds as product np-functionals are seperating
    by definition, see~\sref{tensor}.
\end{point}
\begin{point}%
    Finally we prove that the liner span of
                \begin{equation*}
...
there is a unique operator~$\Define{S \otimes T} \in \scrB^a(X\otimes Y)$\index{$S \otimes T$}
...
\end{point}
\begin{point}%
Finally, to show that~$\Omega$ is separating,
        assume~$A \in \scrB^a(X \otimes Y)$ and $(\sigma \otimes \tau)(A) = 0$
        for all~$\sigma \otimes \tau \in \Omega$.
Then by~\TODO{}
    we know~$\langle x\otimes y, A (x\otimes y)\rangle = 0$
    for all~$x \in X$ and~$y \in Y$.
By polarization (see \sref{hilbmod-polarization}),
    we also have~$\langle x\otimes y, A (x'\otimes y')\rangle = 0$.
    So~$A =0 $ as there is a basis of elements of the form $e \otimes d$. \qed
\end{point}
\end{point}
\end{point}
\end{point}
...
By the Kaplansky density theorem, \TODO{ref}
...
$(\scrA_1 \odot \scrB_1) \odot (\scrA_2 \odot \scrB_2) \cong
(\scrA_1 \odot \scrA_2) \odot (\scrB_1 \odot \scrB_2) $
...
\begin{point}{Density}%
By construction~$\scrA_1 \odot \scrB_1$ is an ultranorm dense submodule
...
\otimes (\scrA_2 \otimes_{\varphi_2} \scrB_2)$.
As~$\scrA_1 \odot \scrA_2$
    is an ultrastrongly dense subalgebra
    of~$\scrA_1 \otimes \scrA_2$ by \TODO{}
    and similarly~$\scrB_1 \odot \scrB_2$
    is ultrastrongly dense in~$\scrB_1 \otimes \scrB_2$,
    we can apply~\sref{dilationspace-dense-subset}
    to find~$
    (\scrA_1 \odot \scrA_2) \odot 
    (\scrB_1 \odot \scrB_2)$ is ultranorm dense
    in
    $(\scrA_1 \otimes \scrA_2) \otimes 
...
\begin{alignat*}{2}
    ...
    & \qquad\ = \ 
    (b_1^* \varphi_1( a_1^* \alpha_1) \beta_2)
    \,\otimes \,
...
As~$U_0$ preserves the inner product,
    it is also bounded.
Thus by \sref{selfdual-completion-univ}
    there is a unique bounded~$\scrB_1\otimes \scrB_2$-linear
    extension
\begin{equation*}
    U\colon (\scrA_1 \otimes_{\varphi_1} \scrB_1) \otimes (\scrA_2 \otimes_{\varphi_2} \scrB_2) \to
        (\scrA_1 \otimes \scrA_2) \otimes_{\varphi_1 \otimes \varphi_2} (\scrB_1 \otimes \scrB_2).
\end{equation*}
    By the ultranorm density and \sref{innerprod-ultraweak}
    we know~$U$ also preserves the inner product.
There is also a unique bounded linear extension of~$U_0^{-1}$,
    which is easily seen to be the inverse of~$U$.
    So~$U$ is an isomorphism and~$U^* = U^{-1}$.
...
It is unreasonable to define an arbitrary ncp-map to be pure
    ...
    as we will see in \sref{nmiu-ncp-extreme}.
...
To help alleviate some confusion caused by the change of direction,
    ...
    see \sref{dfn-comprehension}.  See also \sref{effintro}.
\end{point}
\end{point}
\begin{point}{Example}%
...
\end{point}
\begin{point}{Definition}%
    Filters, corners and their compositions are called \Define{pure}\index{pure}.
\end{point}
...
for~$p$ is given by~$(\cceil{p}\scrA, h_{\cceil{p}}, h_p)$.
\begin{point}{Proof}%
...
We want to show~$t = 0$.
As~$c$ is injective by \TODO{filters injective} $ptp = 0$.
