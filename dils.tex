...
\begin{point}%
Later, in \sref{selfdual-normalish-form},
    we will show that every self dual Hilbert~$\scrB$ over a factor
    is isomorphic to~$\ell^2((1)_{\alpha \in \kappa})\oplus p\scrB$
    for some projection~$p \in \scrB$ and cardinal~$\kappa$.
We will need a lemma first.
\end{point}
\end{point}
\begin{point}{Lemma}%
    Assume~$X$ is a self dual Hilbert~$\scrB$-module.
    Let~$(E_\alpha)_{\alpha \in \beta}$ 
        ...
        respectively.
    If for each~$\alpha \in \beta$ we know~$E_\alpha \cup D_\alpha$ is an orthonormal basis,
