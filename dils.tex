...
\begin{parsec}{1390}%
\begin{point}{10}[dfn-stinespring]{Definition}%
Let~$\varphi\colon \scrA \to \scrB(\scrH)$
    be any ncp-map
...
Define~$\scrW$ similary for~$W$.

Show that there is a unitary~$U \colon \scrK' \to \scrK'$
        such that~$V = (1 \otimes U) W$
    whenever
\begin{inparaenum}
\item both dilations are minimal,
    i.e.~$\scrV = \scrH \otimes \scrK' = \scrW$;
\item
    $\dim \scrV^\perp = \dim \scrW^\perp$;
\item
    and~$\scrH$ and~$\scrK$ are finite dimensional.
\end{inparaenum}
For the first point, show that there is a (unique) unitary~$U_0$ on $\scrH \otimes \scrK'$
    ...
    Show that the other cases follow from this.
\end{point}
\end{parsec}
\begin{parsec}{1400}%
\begin{point}{10}%
