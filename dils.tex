...
\begin{point}{$X$ is self-dual}%
    ...
    The restriction of~$[\,\cdot\,,\,\cdot\,]_f$
        to~$\scrH_{f,S}$ is by construction definite
        and finite dimensional. So~$\scrH_{f,S}$ is complete and thus
        a Hilbert space.
    By the Riesz representation theorem
    there exists a unique~$x_{f,S}+N_{f,S} \in \scrH_{f,S}$
    such that~$[x_{f,S}+ N_{f,S}, y]_f = f(T(\eta(y)))$.
\end{point}
