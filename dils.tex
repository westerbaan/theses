...
Assume~$X$ is a self-dual Hilbert~$\scrB$-module for a von Neumann
    algebra~$\scrB$.
Suppose~$E \subseteq X$ is any subset
    for which
    \begin{inparaenum}
    \item $\left<e,e\right>$ is a non-zero projection for~$e \in E$
    and
\item~$\left<e,d\right>=0$ whenever~$e\neq d$ for~$e,d\in E$.
    \end{inparaenum}

Show that~$E$ is an orthonormal basis of~$E^{\perp\perp}$.
\end{point}
\end{parsec}
...
For any self-dual Hilbert $\scrA$-module $X$
    ...
    with~$\hat{T} \after \eta= T$.
    (For brevity we write~$x \hotimes y = \eta(x\otimes y)$.)
...
\end{enumerate}
\begin{point}{Proof}%
Define~$\eta\colon X \odot Y \to X \otimes Y$
as the ultranorm completion from \sref{dils-completion}.
By \sref{selfdual-compl-defining}
    it is the up-to-isomorphism unique completion
    satisfying the desired universal property.
Also by \sref{selfdual-compl-defining}
    the image of~$X \odot Y$ is ultranorm dense in~$X \otimes Y$.
Assume~$(e_i)_{i \in I}$
 and~$(d_j)_{j \in J}$ are orthonormal
 bases of~$X$ and~$Y$ respectively.
 It remains to be shown~$E = \{ e_i\hotimes d_j;\ i,j \in I\times J\}$
    is an orthonormal basis of~$X \otimes Y$.
\begin{point}%
Clearly for any~$i,i' \in I$ and~$j,j' \in J$
    we have~$[e_i\otimes d_i, e_{i'}\otimes d_{j'}]=
            \langle e_i,e_{i'}\rangle \otimes
            \langle d_j,d_{j'}\rangle =0$
            if~$i \neq i'$ or~$j \neq j'$.
Also~$\langle e_i \hotimes d_j, e_i \hotimes d_j\rangle
= [e_i \otimes d_j, e_i \otimes d_j]
            = \langle e_i,e_i\rangle\otimes \langle d_j,d_j\rangle$
    is a non-zero projection.
Thus by \sref{selfdual-orthn-basis}
    $E$ is an orthonormal basis of~$E^{\perp\perp} \subseteq X \otimes Y$.
By \sref{hilbmod-projthm}
    it is sufficient to show~$E^\perp = \{0\}$.
\end{point}
\begin{point}%
Pick~$t \in E^\perp$. We want to show~$t=0$.
For the moment, assume~$x \hotimes y \in E^{\perp\perp}$
    for any~$x \in X$ and~$y \in Y$.
By construction, there is a net~$t_\alpha$ in the image of~$\eta$
    with~$t_\alpha \to t$ ultranorm.
For each~$\alpha$, there are~$n \in \N$, ~$x_1,\ldots,x_n \in X$
    and~$y_1,\ldots, y_n \in Y$
    with~$t_\alpha = \sum_{k = 0}^n x_k \hotimes y_k$.
As~$x_k \hotimes y_k \in E^{\perp\perp}$,
    we see~$t_\alpha \in E^{\perp\perp}$ and so~$t \in E^{\perp\perp}$
    as~$E^{\perp\perp}$ is ultranorm closed.
Thus~$t = 0$ as~$E^{\perp\perp} \cap E^\perp = \{0\}$
\begin{point}%
We still have to show~$x \hotimes y \in E^{\perp\perp}$.
Write~$x_i = \left<e_i, x\right>$
    and~$y_j = \left<d_j, y\right>$.
We have~$x = \sum_{i \in I} e_ix_i$
and~$y = \sum_{j \in J} d_jy_j$.
Note
\begin{equation}\label{eq-basis-innerprod-ext-tensor}
\langle e_i \hotimes d_j, x \hotimes y\rangle
\ =\  [e_i \otimes d_j, x \otimes y]
\ =\  \left<e_i, x\right>\otimes \left<d_j, y\right>
\ =\  x_i \otimes y_j.
\end{equation}
Thus~$z \equiv \sum_{i,j} (e_i \hotimes d_j) (x_i \otimes y_j)$
    converges ultranorm.
As~$E^{\perp\perp}$ is the ultranorm closure of the~$\scrB$-linear span of~$E$,
    we know~$z \in E^{\perp\perp}$ and we are done if~$z = x \hotimes y$.
We will show
\begin{equation*}
\langle z - x \hotimes y, z - x \hotimes y\rangle
\ \equiv\ 
\langle z,z \rangle
- \langle x \hotimes y, z \rangle
- \langle z, x \hotimes y \rangle +
\langle x \hotimes y, x\hotimes y \rangle
\ = \ 0.
\end{equation*}
For the first cross-term
\begin{equation*}
    \langle z, x \hotimes y \rangle
    \ \overset{\sref{innerprod-ultraweak}}{=}\  \sum_{i,j} (x_i \otimes y_j)^* \langle e_i \hotimes d_j, x \hotimes y\rangle
    \ \overset{\eqref{eq-basis-innerprod-ext-tensor}}{=}\  \sum_{i,j} x_i^*x_i \otimes y_j^*y_j \ = \ \langle z,z\rangle.
\end{equation*}
Similarly~$\langle x \hotimes y, z\rangle = \langle z,z\rangle$.
For the last term, \TODO{tensor prod ultraweak sums}
\begin{align*}
    \langle x \hotimes y, x \hotimes y \rangle
    &\ =\ \left<x,x\right> \otimes \left<y,y\right> \\
