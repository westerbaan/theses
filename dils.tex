...
\end{enumerate}
\begin{point}{Proof}%
The first part of the proof follows the lines of \cite[Thm.~5.2]{pashcke}.
...
The column vector
    $(b_1\ \cdots \ b_n)$
    corresponds to a~$\scrB$-linear map~$b\colon \scrB^{\oplus n} \to \scrB$.
    Similarly, write~$a$ for the~$\scrA$-linear
    map~$a\colon \scrA^{\oplus n} \to \scrA$. \TODO{}
\begin{align*}
    \bigl\| \sum_i B(a_i,b_i) \bigr\|^2
...
Write~$x = \sum_i a_i \otimes b_i$
for column vectors~$a \equiv(a_1\ \cdots\ a_n)$ from~$\scrA$
and~$b\equiv(b_1\ \cdots\ b_n)$ from~$\scrB$.
Then
\begin{align*}
