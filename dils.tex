...
For any~$\scrB$-linear bounded operator $T \colon X \to Y$
    between right~$\scrB$-modules
    with~$\scrB$-valued inner products
    $\left<Tx,Tx\right>\leq \|T\|^2 \left<x,x\right>$
    for all~$x \in X$.
\begin{point}{Proof}
...
\begin{point}{Proof}%
By \TODO{} bounded~$\scrB$-linear maps are ultranorm uniformly continuous.
...
\end{equation*}
By \TODO{} this is a~$\scrB$-valued inner product on~$\scrA \otimes \scrB$.
