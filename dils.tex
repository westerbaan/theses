...
\ar@{..>}[u]_-{\sigma}
    ...
    such that for every (other\footnote{Other choice of possibly the same such triple. See \sref{other-remark}.})
...
% Many authors use a different definition for orthonormal basis
%     in a Hilbert~$\scrB$-module.
% For instance in~\cite[dfn.~1.2]{cabrera1995hilbert}
%     an orthonormal basis is defined as an orthonormal subset
%     whose~$\scrB$-linear span is norm-dense.
% This definition is strictly weaker than ours:
%     there are Hilbert~$\scrB$-modules
%     with such a weaker orthonormal basis
%     that are not self dual, cf.~\sref{dils-selfdual}.
% \end{point}
\end{point}
...
\begin{point}{50}[dils-selfdual]{Theorem}%
    ...
    For any pre-Hilbert~$\scrB$-module~$X$,
        ...
        \begin{enumerate}
            \item $X$ is \Define{self-dual}\index{Hilbert~$\scrB$-module!self-dual} ---
