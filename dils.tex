...
\item
    ...
    span of~$V$ \emph{and}
\item
    $X \cong V^{\perp\perp} \oplus V^\perp$.
\end{enumerate}
\begin{point}{Proof}%
...
\end{point}
\begin{point}%
As~$(e_i)_i$ is a basis for~$W = V^{\perp\perp}$
    and~$(d_j)_j$ is a basis for~$W^\perp = V^\perp$,
    we see
\begin{equation*}
    x \mapsto \bigl(\sum_i e_i \left<e_i, x\right>, \sum_j d_j\left<d_j, x\right>\bigr)
\end{equation*}
is an isomorphism~$X \cong V^{\perp\perp} \oplus V^\perp$. \qed
\end{point}
\end{point}
\end{point}
\end{parsec}
