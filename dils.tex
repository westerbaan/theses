...
\begin{point}[stinespring-theorem]{Theorem (Stinespring)}
    ...
    there is a Hilbert space~$\scrK$,
        ...
        where~$\Define{\ad_V}\index{$\ad_V$} \colon \scrB(\scrK) \to \scrB(\scrH)$
...
$(\scrK,\varrho,V)$ is said to be \Define{minimal}
...
Prove the \Define{polarization identity}:
\begin{equation*}
...
Show \Define{Parseval's identity} holds, that is:
\begin{equation*}
    \langle x, x \rangle \ =\ 
...
To see~$x'=0$, we
first show that~$X$ has \Define{polar decomposition}:
...
\begin{point}{Definition}%
Assume~$\varphi\colon \scrA \to \scrB$ is an ncp-map.
We say~$\varphi$ is \Define{ncp-extreme}
