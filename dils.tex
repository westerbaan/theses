...
\begin{equation*}
    [a\otimes x, b \otimes y] = \left<\varphi(a^*b)x, y\right>_{\scrH}.
\end{equation*}
...
we need to show
\begin{equation*}
    0 \leq [\sum_i a_i\otimes x_i, \sum_j a_j\otimes x_j]
        = \sum_{i,j} [a_i\otimes x_i, a_j\otimes x_j]
        = \sum_{i,j} \left< \varphi(a_i^*a_j) x_i,x_j \right>,
\end{equation*}
which is equivalent to the complete positivity of~$\varphi$
by~\TODO{}.
Let~$\eta\colon A\odot \scrH \to \scrK$ denote the Hilbert space completion
    described in~\sref{prop-complete-into-hilbert-space}.
\begin{point}%
Let~$T\colon A \odot \scrH \to A \odot \scrH$
be a bouned linear map.
We show there is a unique lifting to~$\scrB{\scrK}$.
The operator $\hat{T}$
