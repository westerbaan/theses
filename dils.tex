...
The correspoding uniformity is called the \Define{ultraweak uniformity}.
    \end{enumerate}
\end{point}
\end{parsec}
...
such that for all~$\alpha < \alpha_0$ and~$\beta < \beta_0$
we have~$x_\alpha \mathrel\varepsilon x_\beta$.
...
\begin{point}{Proposition}%
    Assume~$X$ is a uniform space with entourages~$\Phi$.
    There exists a bounded complete uniform space~$C$
        with uniformly continuous~$\eta \colon X \to C$
        such that the image of~$\eta$ is dense in~$C$.
    Furthermore, for every uniformly continuous~$f\colon X \to Y$
        into a bounded complete Hausdorff uniform space~$Y$,
        there exists a unique continuous map~$g\colon C \to Y$
        such that~$g \after \eta = f$.
\begin{point}{Proof}%
    The set of entourages~$\Phi$ is a filter
...
However, we claim the net~$(x_{\alpha_{\varepsilon}})_{\varepsilon\in\Phi}$
    is a Cauchy, fast and equivalent to~$(x_{\alpha})_{\alpha\in I}$.
...
Note that if fast Cauchy nets~$(x_\alpha)_{\alpha \in \Phi}$
...
\end{point}
\begin{point}%
Write~$N$ for the set of bounded fast Cauchy nets
    and~$C$ for~$N$ modulo equivalence of Cauchy nets.
...
Unfortunately the entourages~$\hat\varepsilon$ do not necessarily preserve
    equivalence of Cauchy nets and do not lift directly to~$C$.
\end{point}
\begin{point}{$N$ is bounded complete}%
As every bounded Cauchy net is equivalent to a fast one,
it is sufficient to show convergence of bounded fast Cauchy nets.
Thus let~$((x^\gamma_\alpha)_\alpha)_\gamma$
    be a bounded fast Cauchy net in~$N$.
First we will show~$(x^{\hat\alpha}_\alpha)_\alpha$
    is a bounded Cauchy net.
