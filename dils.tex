...
Write~$Z$ for the ultranorm closure of the image of~$X \odot Y$
    in~$X \otimes Y$.
By \sref{ultranormcontstruct} $Z$ is a submodule.
Clearly the induced uniformity of~$X \otimes Y$
    on~$Z$ is the same as its ultranorm uniformity
    and so~$Z$ is ultranorm complete as well
    thus by \sref{dils-selfdual}
    there is an orthonormal basis~$(z_i)_{i \in I}$ of~$Z$.
Assume~$Z \neq X \otimes Y$, say~$v \in X \otimes Y$ but~$v \notin Z$.
Considering~$v- \sum_{i \in I} z_i \left<z_i, v\right> $
    instead of~$v$,
    we may assume without loss of generality that~$\left<z_i,v\right>=0$.


Write~$v' = v- \sum_{i \in I} z_i \left<z_i, v\right> \in Z$.
Note~
...
\end{equation*}
for all~$x,x' \in X$ and~$y,y' \in Y$ \TODO{inner product uwlim}
    it is sufficient to show the image of~$X \odot Y$ under~$\otimes_2$
    is ultraweakly dense in~$X \otimes_2 Y$.
