...
Assume~$(x_\alpha)_\alpha$,
    ...
    Only norm completeness of~$X$ remains.
    We first show norm convergence implies convergence in the uniformity.
        If~$\| \tau \|^2 \leq \varepsilon$
        for some~$\tau \in X$ with
            representative~$(x_\alpha)_\alpha$,
        then for any np-map~$f\colon X \to \scrB$
        \begin{equation*}
        \lim_\alpha \|x_\alpha \|_f
                = f(\uwlim_\alpha [x_\alpha,x_\alpha])
                \leq \|f\|\, \|\left<\tau,\tau\right>\|
                \leq \|f\|\, \varepsilon.
        \end{equation*}
        So for any sequence~$\tau_1, \tau_2, \ldots$ in~$X$
         we know~$\tau_n \to 0$ in the uniformity when~$\tau_n \to 0$
         in the norm.
    To show norm completeness,
    assume~$\tau_1, \tau_2, \ldots$ is a norm-Cauchy sequence in~$X$.
\end{point}
\end{point}
