...
Even then, the main result of the exercise as was written does not hold without
        an extra assumption: either the dilations have to be minimal;
the ancillar spaces of the same dimension;
or~$\dim \scrH, \dim \scrK < \infty$.
\end{erratum}
...
\end{erratum}
\begin{erratum}{ea-modularity-prop}%
For elements~$a \perp b$ in an effect algebra,
    contrary to the Proposition,
    it is \emph{not} the case
    that~$a \vee b$ exists if~$a \wedge b$ does,
    but the converse is true.
The proof is as given with order reversed.
\end{erratum}
\begin{erratum}{exc-emonzero}%
The exercise asks to prove~$a \odot 0 = a = 0 \odot a$.
This should have read~$a \odot 0 = 0 = 0 \odot a$.
\end{erratum}
\begin{erratum}{one-m-is-id}%
The stated equality~$1 = 1 \after 1$ isn't established yet,
    but also irrelevant for the proof.
\end{erratum}
\begin{erratum}{eff-aconv-exa}%
It's claimed abstract~$2$-convex sets are exactly semilattices.
This is false.
\end{erratum}
\begin{erratum}{aconvalmosteffectus}%
When arguing~$\AConvM$ has an initial object,
    it is claimed~$\mathcal{D}_M \emptyset = \emptyset$,
    but that is wrong in the case~$M=1$.
The category does have an initial object in that case,
    by noticing all abstract convex sets are singletons.
\end{erratum}
\begin{erratum}{eff-divisoid-add}%
The original proof is incorrect. Instead, the result can be obtained in
    two steps. First we consider the special case~$c = a\ovee b$.
    This is proven by noting
        that~$rfrac{a \ovee b}{a \ovee b} \ominus \rfrac{a}{a \ovee b}$
    satisfies the defining property of~$\rfrac{b}{a \ovee b}$.
The main result follows by multiplying on
    the left by~$\rfrac{a \ovee b}{c}$.
\end{erratum}
...
\end{erratum}
\begin{erratum}{eff-dagger-conc-ex}%
In the first point~$\ceil{1 \after f}$ appears,
    which should've been~$\ceil{1 \after g}$
\end{erratum}
