...
is measurable when~$A$ is open up to
a \emph{meagre} (defined below) subset.
...
The first technical step in this project
involves a special topological property of~$\spec(\scrA)$,
see~\sref{vn-spectrum-extremally-disconnected},
but let us first show that~$C(\spec(\scrA))$ is a von Neumann algebra
to begin with.
\end{point}
...
\item
The boundary, $\overline{U}\backslash U$,
of an open subset~$U$ of~$X$
is meagre.
...
with $A_n\approx B_n$.
\item
$A\approx B$ entails $X\backslash A\approx X\backslash B$
for all~$A,B\subseteq X$.
\item
The almost clopen subsets of~$X$ form a $\sigma$-algebra.
\end{enumerate}
\end{point}
...
be a faithful np-functional
on a commutative von Neumann algebra~$\scrA$.
Recall from~\sref{ngelfand} that
the Gelfand representation $\gamma_\scrA\colon 
\scrA\to C(\spec(\scrA))$
is a nmiu-isomorphism,
and~$C(\spec(\scrA))$ is a von Neumann algebra.

For every almost clopen (see~\sref{meagre})
subset~$A$ of~$X$
there is a unique clopen~$C_A\subseteq X$
with~$A\approx C_A$.
Setting~$\mu(A)=\omega(\gamma_\scrA^{-1} (\mathbf{1}_{C_A}))$
for every almost clopen~$A\subseteq C$
turns~$\spec(\scrA)$ into a finite complete measure space
(with the almost clopen subsets as measurable subsets.)

With respect to this measure a function~$f\colon \spec(\scrA)\to\C$
...
\end{equation*}
\begin{point}{Proof}%
Since a meagre subset has empty interior
(by Baire's category theorem, \sref{baire-category-theorem},)
an open subset~$V$ of~$\spec(\scrA)$ is meagre
iff~$V=\varnothing$.
In particular,
given clopens~$U,U'$ of~$\spec(\scrA)$
the set $U\cup U'\,\backslash\,U\cap U'$ is open,
and so~$U\approx U'$ iff~$U\cup U'\backslash U\cap U'=\varnothing$
iff~$U=U'$.
Therefore 
there is at most one clopen~$U$ of~$\spec(\scrA)$
with~$A\approx U$
for every subset~$A$ of~$\spec(\scrA)$:
 if~$U,U'$ are clopens of~$\spec(\scrA)$
with~$U\approx A\approx U'$, then~$U\approx U'$
(since~$\approx$ is easily seen to be transitive),
and so~$U=U'$.
