...
in~$\scrA$ converges ultraweakly to some~$b$ in~$\scrA$
    iff $\omega(b_\alpha)\to \omega(b)$ for all
...
iff it is a union of finite intersections
of sets of the form
$\{\,a\in\scrA\colon\, \|a-b\|_\omega < \varepsilon\,\}$,
...
with~$f(x)=1$ and~$f(y)=0$ for all~$y\in \spec(\scrA)\backslash U$.
...
Let~$y\in \spec(\scrA)\backslash \overline{U}$ be given.
...
Hence~$(\bigvee D)(y)=0$ for all~$y\in \spec(\scrA)\backslash \overline{U}$.
...
$a\in\scrA$,
\begin{equation*}
	\ceill{f(a)\,}\,\leq\,\ceil{f(\,\ceill{a}\,)}
	\qquad\text{and}\qquad
	\ceilr{\,f(a)}\,\leq\,\ceil{f(\,\ceilr{a}\,)}.
\end{equation*}
\spacingfix%
\begin{point}{30}{Proof}%
...
One obtains $\ceilr{\,f(a)}\leq\ceil{f(\,\ceilr{a}\,)}$
...
for any bounded directed subset of positive elements of~$\scrA$.
...
\,\leq\, \|a\|_\omega\, \|b\|\, \|c\|_\omega$
for all~$a,b,c\in\scrA$.

Deduce that $\| b*\omega - b'*\omega\|
\,\leq\, \|b-b'\|_\omega\, (\|b\|_\omega + \|b'\|_\omega)$
for all~$b,b'\in\scrA$.
...
\item
Show that~$\ceil{b}c^{\sim1}\ceil{b}\leq b^{\sim1}$
...
with~$a=bc$.
\end{point}
\begin{point}{20}[division-basic]{Exercise}%
Let~$a$ and~$b$
...
\end{equation*}
is both ultrastrongly and ultraweakly continuous,
and the map
\begin{equation*}
...
the series~$\sum_n at_n$ converges ultrastrongly
(and so also ultraweakly, by~\sref{uwweaker})
to~$a/b$,
where~$t_1,t_2,\dotsc$
is an approximate pseudoinverse of~$b$,
...
is ultrastrongly and ultraweakly continuous (by~\sref{mult-uws-cont})
--- and the uniform limit of continuous functions is continuous ---
we see that~$a\mapsto a/b,
\ (\scrA)_1b\to\scrA$ is ultrastrongly and ultraweakly continuous.

Now~$c\backslash x=(x^*/c^*)^*$
for~$x\in c(\scrA)_1$
by~\sref{division-basic}(\ref{division-basic-star}),
so~$c\backslash(\,\cdot\,)\colon\, c(\scrA)_1\to\scrA$
is ultraweakly continuous,
$a\mapsto a^*$ being so;
and as~$(\,\cdot\,)/b$ maps~$c(\scrA)_1b$ into~$c(\scrA)_1$,
their composition $c\backslash\,\cdot\,/b\colon\,
c(\scrA)_1b\to\scrA$
is ultraweakly continuous too.\qed
\end{point}
...
\begin{point}{120}%
Moreover, the maps~$a\mapsto c\backslash a/b$
and $a\mapsto c\backslash a$
need not be ultrastrongly continuous,
    even on $c(\scrA)_1b$ and $c(\scrA)_1$, respectively.
Indeed, take~$\scrA=\scrB(\ell^2)$, $b=1$,
and
$c=\sum_n \frac{1}{n}\ketbra{n}{n}$.
The elements
$c\ketbra{n}{0} \equiv  \frac{1}{n}\ketbra{n}{0}$
of~$c(\scrA)_1$ converge to~$0$ in norm (and thus also ultrastrongly),
    while $c\backslash (c\ketbra{n}{0}) =  \ketbra{n}{0}$
    does not converge ultrastrongly to~$0$,
    since $\left\|\smash{\ketbra{n}{0}}\right\|_\omega = 1 \not\longrightarrow 0$
for $\omega:=\left<0\right|\, \,\cdot\, \left|0\right>$.
\end{point}
\end{point}
\end{point}
\end{parsec}
