...
\begin{point}{Remark}%
The reason
that the space~$\scrA_*$ is called the \Define{predual}
of~$\scrA$
...
is complete.
\begin{point}{Proof}%
\TODO{We'd like to use the fact
that a functional $f\colon \scrA\to\C$
is ultraweakly continuous iff 
it is ultraweakly continuous on~$(\scrA)_1$,
...
of a von Neumann algebra~$\scrA$
is a partial isometry with $\cfloor{u^*u}\cup\cfloor{uu^*}=1$.
\begin{point}{Remark}%
...
To see that~$\cfloor{u^*u}\cup \cfloor{uu^*}=1$,
i.e.~$\cceil{(u^*u)^\perp} \cap \cceil{(uu^*)^\perp }=0$,
it suffices (by \TODO{})
to show that $(uu^*)^\perp \scrA (u^*u)^\perp = (0)$.
So let~$a\in (uu^*)^\perp \scrA (u^*u)^\perp$
be given, and assume (without loss of generality)
that~$\|a\|\leq 1$.
