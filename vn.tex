...
\end{point}
\begin{point}[cceil-sum]{Theorem}%
\index{$C^*$-algebra!finite-dimensional}%
...
and $(\scrA)_1$
is norm compact.
\begin{point}{$\scrA$ is a von Neumann algebra}%
...
By scaling,
we may also assume that~$\|a\|\leq 1$.
...
Hence~$\ceil{\|a\|e-a}=0$,
and so~$a=\|a\|e$.
In particular, $a$ is in the range of~$\varrho$.\qed
\end{point}
\end{point}
\end{point}
\end{parsec}
