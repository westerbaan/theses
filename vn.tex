...
for while $\left|a\right|\equiv \smash{\sqrt{a^2}}$,
neither $a\mapsto a^2$ nor $a\mapsto \sqrt{a}$
are ultrastrongly \TODO{check!} or ultraweakly continuous 
...
Prove that~$a\mapsto \left|a\right|$ is ultrastrongly continuous.
\end{point}
...
are effects
by~\sref{kaplansky}
because~$p$ is a projection (and thus an effect).
...
Caring only about~$\omega_0$ and~$\omega_1$,
pick a subsequence~$(b_n)_n$ of~$(b_\alpha)_\alpha$
such that $\omega_0( (b_n p^\perp)^* b_n p^\perp ) 
\leq (n2^n)^{-2}$
and $\omega_1( (p b_n^\perp)^* p b_n^\perp)\leq n^{-2}$
for all~$n$.
For~$n < m$, define
\begin{equation*}
\textstyle
...
\end{equation*}
where~$a_n:= \bigvee_m a_{nm}$
and~$a := \bigwedge_n a_n$.
We will prove that~$\omega_0(a)=0=\omega_1(a^\perp)$.
\begin{point}{$\omega_0(a^\perp)=0$}%
Note that $\omega_0(b_n)^2\leq
\omega_0(b_n^2) =
\omega_0(p^\perp b_n^2 p^\perp) \leq (n2^n)^{-2}$
by \TODO{Cauchy--Schwarz and Comprehension-stuff}
(because~$\omega_0(p)=0$),
and so~$\omega_0(b_n)\leq n2^n$ for all~$n$.
Then, since $a_{nm}\leq \sum_{k=n}^m k b_k$,
...
Note that~$\omega_1(pb_n^\perp p)^2
\leq \omega_1(pb_n^\perp pb_n^\perp p)
= \omega_1(b_n^\perp p b_n^\perp ) 
\leq n^{-2}$
(because~$\omega_1(p)=0$),
and so~$\omega_1(b_n^\perp)\leq \frac{1}{n}$
for every~$n$.
Let~$m> n$ be given.
...
\begin{point}{Proof}%
\TODO{Reduction to projections}

Let~$p$ be a projection of~$\scrA$ which is the ultrastrong limit
of a net from~$\scrB$. We must show that~$p\in\scrB$.
Given an npu-map $\omega\colon \scrA\to \C$
 (i.e.~$\omega\in\NStat(\scrA)$),
write~ $\ceil{\omega}_\scrB$ for the carrier
of the restriction of~$\omega$
to a npu-map~$\scrB\to \C$.
Note that~$\ceil{\omega}\leq \ceil{\omega}_\scrB$,
but equality need not hold in general.
We prove that~$p\in\scrB$ by approximating~$p$
by these~$\ceil{\omega}_\scrB\in\scrB$; define
\begin{alignat*}{3}
q\ &= \textstyle\ \bigvee\{ \ \ceil{\omega_1}_\scrB\colon\ 
\omega_1(p^\perp)=0
\text{ where }
\omega_1\in\NStat(\scrA)\ \} \\
r\ &=\ 
 \textstyle\bigvee\{ \ \ceil{\omega_0}_\scrB\colon\ 
\omega_0(p)=0\
\text{ where }
\omega_0\in\NStat(\scrA)\ \}. 
\end{alignat*}
Note that the set of which~$q$ is defined to be the supremum is directed,
because for~$\omega_1,\omega_1'\in\NStat(\scrA)$
with $\omega_1(p^\perp)=0=\omega_1'(p^\perp)$
writing $\omega_1'':= \frac{1}{2}\omega_1 + \frac{1}{2}\omega_1'$
we have $\ceil{\omega_1}_\scrB \vee \ceil{\omega_1'} 
 = \ceil{\omega_1''}$
and~$\omega_1''(p^\perp)=0$.
\TODO{finish...}
