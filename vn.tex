...
\end{point}
\begin{point}{Remark}%
A projection~$E$ in a von Neumann algebra~$\scrR$
of bounded operators on a Hilbert space~$\scrH$
is called \Define{cyclic} when it is the projection
onto $\overline{\scrR^\square x}$
for some vector~$x\in\scrH$,
which turns out to be the carrier
of the vector functional $\left<x,(\,\cdot\,)x\right>$
restricted to~$\scrR$.
\end{point}
