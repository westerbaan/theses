...
\item
Use this, and~\TODO{square-root-monotone}
to show that~$f(a^{\nicefrac{1}{2}}) \leq f(a)^{\nicefrac{1}{2}}$.
\item
...
Show that
for elements~$a$ and~$b\geq 0$ from a von Neumann algebra,
\begin{equation*}
...
\begin{enumerate}
\item
Show that $cb=0$ for $c\in\scrA$ iff $\ceill{c}\leq \ceilr{b}^\perp$.
...
\end{point}
\end{parsec}
\section{Projections~II}
\begin{parsec}%
\begin{point}{Definition}%
We'll say that a \emph{positive} element~$a$
of a von Neumann algebra~$\scrA$
is \Define{pseudoinvertible}
if it has a \Define{pseudoinverse},
that is, an element~$t\in\scrA_+$
with $t\leq \ceil{a}$
and $ta=\ceil{a}=at$.
If such~$t$ exists,
it is unique (see~\sref{pseudoinverse-basic}),
and so we can invent a notation for it: \Define{$t^{\sim 1}$}.
\end{point}
\begin{point}[pseudoinverse-basic]{Exercise}%
Let~$a$ be a positive element of a von Neumann algebra~$\scrA$.
\begin{enumerate}
\item
Show that~$a$ has at most one pseudoinverse.
\item
...
forming what one might call an ``approximate pseudoinverse'' for 
$(0,0,1,\frac{1}{2},\frac{1}{3},\dotsc)$.
That this can also be done for an arbitrary positive element
of a von Neumann algebra
is what we'll see next.
\end{point}
\begin{point}[approximate-pseudoinverse]{Proposition}%
Every positive element~$a$ of a von Neumann algebra~$\scrA$
has (what we'll call) an \Define{approximate pseudoinverse},
that is, a sequence
$t_0, t_1, t_2,\dotsc$ of positive elements of~$\scrA$
with~$\sum_n \ceil{t_n}=\ceil{a}$,
such that
$t_n a=\ceil{t_n}=at_n$,
$\ceil{t_n}n\leq t_n\leq \ceil{t_n}(n+1)$,
and $t_n\in \{a\}^{\square\square}$ for all~$n$.
\begin{point}{Proof}%
...
so that~$a=\bigvee_n (a-\frac{1}{n})_+$ by~\TODO{...},
...
we clearly have (using~\sref{pseudoinverse-basic})
$t_n\in\{a\}^{\square\square}$
(since~$ae_n\in\{a\}^{\square\square}$)
and~$\ceil{t_n}=e_n$
(since~$\ceil{ae_n}=e_n)$,
and $ne_n \leq t_n \leq (n+1)e_n$
(since~$\frac{1}{n+1}e_n\leq ae_n \leq \frac{1}{n}e_n$).

Finally, $t_na=t_n\ceil{t_n}a=t_ne_na=\ceil{a}$
and similarly, $at_n = \ceil{a}$.\qed
\end{point}
\end{point}
\begin{point}[douglas]{Proposition}%
...
\end{enumerate}
Moreover,
in that case there is a unique~$c$
with $a=cb$ and~$\ceill{c}\leq \ceilr{b}$
and for this~$c$ we have
$\ceilr{c}\leq \ceilr{a}$.
\begin{point}{Proof}%
Let's get rid of the easy bits:
there is at most one $c\in\scrA$
with $a=cb$ 
and $\ceill{c}\leq \ceilr{b}$
by~\sref{mult-cancellation};
and 
if $a=cb$ for some~$c\in\scrA$,
then~$a^*a=b^*c^*cb\leq \|c\|^2 \,b^*b$.

For the other direction,
suppose that~$a^*a \leq \lambda\,b^*b$
for some~$\lambda\geq 0$;
we must find~$c\in\scrA$ with~$a=cb$.
Let~$t_0,t_1,\dotsc$
be an approximate pseudoinverse of~$b^*b$
(see~\sref{approximate-pseudoinverse}),
and define~$c:=\sum_{n=0}^\infty at_nb^*$,
assuming for the moment that this series converges ultraweakly.
Then~$cb=\sum_n at_nb^*b=\sum_n a\ceil{t_n}
=a\sum_n \ceil{t_n}= a\ceil{b^*b} =a$,
because~$\ceil{b^*b}\leq\ceil{\lambda\,a^*a}\leq \ceil{a^*a}$,
and, since~$at_nb^*=\ceilr{a}at_nb^*\ceill{b^*}=
\ceilr{a}at_nb^*\ceilr{b}$,
we also see that~$\ceilr{a}c\ceilr{b}=c$,
and thus that~$\ceill{c}\leq \ceilr{b}$
and~$\ceilr{c}\leq \ceilr{a}$.

So the only thing that remains to be shown is that~$\sum_{n=0}^N at_nb^*$
converges ultraweakly as~$N\to \infty$,
and for this it suffices
to show that~$(\,\sum_{n=0}^N at_nb^*\,)_N$
is ultrastrongly Cauchy
(because~$\scrA$ is ultrastrongly complete, by~\sref{vn-complete}).
Note that
\begin{alignat*}{3}
\textstyle 
(\, \sum_{n=M}^N at_nb^*\,)^*\ \,\sum_{n=M}^N\,at_nb^*
\ &= \textstyle\ b \,(\sum_{n=M}^Nt_n) \,a^*a\, (\sum_{n=M}^N t_n)\, b^* 
\\
\ &\leq\ \textstyle 
\lambda \, b\, (\sum_{n=M}^N t_n) \,b^*b\, (\sum_{n=M}^Nt_n)\, b^*
\\
\ &\leq\ \textstyle 
\lambda \sum_{n,m=M}^N
bt_nb^*bt_m b^*
...
fact that $(bt_nb^*)^* bt_mb^*
\equiv bt_n b^* b t_m b^*
= bt_n\ceil{t_m} b^*$
equals~$bt_nb^*$ when~$n=m$,
and is~$0$ otherwise.
This actually implies
that the $bt_nb^*$ form an orthogonal set of 
projections,
and so the series~$\sum_{n=0}^\infty bt_nb^*$
converges ultraweakly by~\TODO{...}.
This, coupled with the
inequality above,
gives us that~$\sum_{n=0}^N at_nb^*$
is ultrastrongly Cauchy.\qed
\end{point}
\end{point}
...
where~$[a]$
is an element of~$\scrA$
with~$\ceill{\,[a]\,}\equiv \ceil{\,[a]^*[a]\,}\leq 
\ceil{a^*a}\equiv \ceill{a}$.
Moreover,
\begin{enumerate}
\item
...
Note that~$[a][a]^*$
is a projection (because $[a]$ is a partial isometry, see~\TODO{}).
