...
is \Define{meagre} when~$A\subseteq \bigcup_n B_n$
for some closed subsets $B_1\subseteq B_2\subseteq \dotsb$
of~$X$ with empty interior (that is,  $B_n^\circ=\varnothing$.)
\end{point}
...
Writing~$\Define{A\approx B}$
for~$A,B\subseteq \spec(\scrA)$
when~$A\backslash B\cup B\backslash A$
is meagre,
there is for every subset~$A$ of~$\spec(\scrA)$
at most one clopen subset~$U$ of~$\spec(\scrA)$
with~$A\approx U$.
Calling such~$A$ measurable,
and setting~$\mu(A)=\omega(\gamma_\scrA^{-1} (\mathbf{1}_U))$
