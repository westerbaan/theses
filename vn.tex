...
Let~$b$ be an element of a von Neumann algebra~$\scrA$,
and let~$t_1,t_2,\dotsc$ be an approximate pseudoinverse of~$b$,
and let~$\lambda\geq 0$.
\begin{enumerate}
\item
For every~$a\in\scrA$
with $a^*a\leq \lambda b^*b$
there is a unique 
element~$\Define{a/b}$
of~$\scrA\!\ceilr{b}$
with $a=(a/b)\,b$.
...
to~$a/b$ 
(and uniformly so)
for $a\in \scrA$ with $a^*a\leq \lambda b^*b$.
\item
For every~$a\in \scrA$
that can be written as $a\equiv \sum_{k=0}^3 i^k a_k$
where $a_0,\dotsc,a_k\in \scrA_+$
are such that $a_k\leq \frac{1}{4}\lambda b^*b$,
\TODO{...}
\end{enumerate}
\end{point}
...
is an approximate pseudoinverse of~$b$,
then~$\sum_n at_n$ converges ultrastrongly
to this~$c$,
and $\ceilr{c}\leq \ceill{b}$.
\begin{point}{Proof}%
...
Let~$t_0,t_1,\dotsc$
be an approximate pseudoinverse of~$b$
(see~\sref{approximate-pseudoinverse}),
and define~$c:=\sum_{n=0}^\infty at_n$,
assuming for the moment that this series converges ultrastrongly.
Since~$a^*a \leq \lambda b^*b$,
we have $\ceill{a}\leq \ceill{b}$,
and so~$a=a\ceill{b} = a\sum_n t_n b = \sum_n at_n b
= cb$.
Since~$at_n=\ceilr{a}at_n\ceilr{b}$
(as~$\ceill{t_n}\leq \ceilr{b}$)
we also see that~$\ceilr{a}c\ceilr{b}=c$,
giving~$\ceill{c}\leq \ceilr{b}$
and~$\ceilr{c}\leq \ceilr{a}$.

So the only thing that remains to be shown is that~$\sum_{n=0}^N at_n$
...
\\
\ &\leq\ \textstyle 
\lambda \, (\sum_{n=M}^N t_n) \,b^*b\, (\sum_{n=M}^Nt_n)
\\
\ &\leq\ \textstyle 
\lambda \sum_{n,m=M}^N
t_nb^*bt_m 
...
so that the series~$\sum_{n=0}^\infty bt_m$
