...
(and are therefore called \emph{normal}, see~\sref{p-uwcont}),
because 
\begin{enumerate}
\item
...
and while many arguments are borrowed from these
...
the~$C^*$-algebra $\scrB_a(X)$ (\sref{bax-cstar})
of bounded adjoinable module maps on
a self-dual (\sref{self-dual}) Hilbert $\scrA$-module~$X$
is a  von Neumann algebra,
and
$\left<x,(\,\cdot\,)x\right>\colon \scrB_a(X)\to\scrA$
is normal for every~$x\in X$.
\begin{point}{Proof}%
We'll first show that
a  bounded directed subset~$\scrD$ of~$\Real{\scrB_a(X)}$
has a supremum (in~$\Real{\scrB_a(X)}$).
...
Given another upper bound~$S'\in\Real{\scrB_a(X)}$
of~$\scrD$
...
we immediately see
that $\left<x,(\,\cdot\,)x\right>\colon \scrB_a(X)\to\scrA$
preserves bounded directed suprema
for every~$x\in X$.

It remains to be shown that
there are sufficiently many np-functionals on $\scrB_a(X)$
in the sense that~$T\in(\scrB_a(X))_+$ is zero
when~$\omega(T)=0$ for every np-functional $\omega\colon \scrB_a(X)\to\C$.
This is indeed
the case for such an operator~$T$,
because~$\xi(\left<x,(\,\cdot\,)x\right>)$
is an np-functional on~$\scrB_a(X)$
for every~$x\in X$
...
we have $\omega(a)\leq \omega(\ceil{a})$,
...
\end{equation*}
where~$a^*(\,\cdot\,)a$
is interpretted as an np-map~$\scrA\to\scrA$.
\item
...
is interpretted as a map~$\scrB(\scrH)\to\C$.
...
We'll deal with commutative case first
...
Then $u^*u$,
represented
as continuous function (on~$\spec(a)$ cf.~\sref{functional-calculus}),
...
of commutant, see~\sref{commutant}.
\end{point}
