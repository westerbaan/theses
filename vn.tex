...
The carrier~$\ceil{f}$
of a nmiu-map $f\colon \scrA\to\scrB$
between von Neumann algebras
is central,
and $\ker(f)=\ceil{f}\!\scrA$.
\end{point}
\begin{point}[proto-gns-ceil]{Lemma}%
We have~$\cceil{\omega}=
\ceil{\varrho_\omega}
= \bigcup_{a\in \scrA}\ceil{a^*\ceil{\omega}a}$
...
Note that $\cceil{a^*\ceil{\omega} a}
= \cceil{a}\cceil{\omega}\leq\cceil{\omega}$ by~\TODO{...},
and so
$\cceil{\varrho_\omega}
= \cceil{\ceil{\varrho_\omega}}
= \bigcup_{a\in\scrA} \cceil{ a^* \ceil{\omega} a} 
\leq \cceil{\omega}$.
Since also~$\ceil{\omega}\leq \bigcup_{a\in \scrA}\ceil{a^*\ceil{\omega}a}
=\ceil{\varrho_\Omega}$,
and thus~$\cceil{\omega}\leq \cceil{\ceil{\varrho_\omega}}
=\ceil{\varrho_\omega}$,
we get~$\ceil{\varrho_\omega} = \cceil{\omega}$.\qed
\end{point}
\end{point}
\begin{point}{Corollary}%
The central support of projection~$e$ from a von Neumann algebra~$\scrA$
can be obtained as~$\cceil{e}=\bigcup_{a\in\scrA} \ceil{a^*ea}$.
\begin{point}{Proof}%
Writing~$\Omega$ for the set of np-functionals with
$\omega(e^\perp)=0$,
we have~$\bigcup_{\omega\in\Omega}\ceil{\omega}=e$ by~\TODO{...},
and so
\TODO{add reasons}
\begin{alignat*}{3}
\cceil{e}
...
\ &=\ 
\textstyle
\bigcup_{\omega\in\Omega}\bigcup_{a\in \scrA}
\ceil{a^* \ceil{\omega} a}
\\
\ &=\
\textstyle
\bigcup_{a\in \scrA} \bigcup_{\omega\in\Omega}
\ceil{a^*\ceil{\omega} a}
\\
\ &=\ 
\textstyle
...
for all~$\omega\in\Omega$
we get~$\ceil{\varrho_\Omega}=0$,
which implies that~$\varrho_\Omega$ is injective.\qed
\end{point}
\end{point}
\end{parsec}
