...
\begin{parsec}%
\begin{point}{Exercise}%
Show that for every element~$b$ of a von Neumann algebra~$\scrA$
there is a least projection~$\Define{\ceill{b}}$ 
of~$\scrA$
(called the \Define{support} of~$b$)
with $b \ceill{b}=b$,
and there is a least projection~$\Define{\ceilr{b}}$
of~$\scrA$ (called the \Define{range} of~$b$)
with $\ceilr{b}b=b$.
...
$\ceill{b^*}=\ceilr{b}$;
\item
$\ceill{b} = \ceil{b} = \ceilr{b}$ when~$b$ is an effect;
\item
$\ceill{b}=\ceill{b^*b}$
and $\ceill{b}=\ceill{\lambda b}$
for every $\lambda\in \C\backslash\{0\}$;
\item
if $a\in\scrA$ commutes with~$b$,
then~$a$ commutes with~$\ceill{b}$.
\end{enumerate}
\end{point}
