...
of a von Neumann algebra~$\scrB$.
Let~$e$ be a projection in~$\scrB$.
Then $\Define{\ceil{e}_{\scrA^\square}}= 
\bigcup_{a\in\scrA} \ceil{a^* e a}$
is the least projection in~$\scrA^\square$
above~$e$.
\begin{point}{Proof}%
Let us first show that~$p:= \ceil{e}_{\scrA^\square}$
is in~$\scrA^\square$.
Let $b\in\scrA$ be given.
Since~$b^*(\,\cdot\,)b\colon \scrB\to\scrB$
is normal and $p=\bigcup_{a\in\scrA} \ceil{a^*ea}$,
we have $b^*pb\leq \ceil{b^*pb} = 
\bigcup_{a\in\scrA} \ceil{ b^* \ceil{a^* e a} b }
= \bigcup_{a\in\scrA} \ceil{(ab)^* \,e\, ab} \leq p$
by \TODO{} and \TODO{}.
...
we get  $bp=pbp=pb$
(upon applying~$(\,\cdot\,)^*$) and so~$p\in\scrA^\square$.

It remains to be shown that~$p$ is the least projection in~$\scrA^\square$
above~$e$. Let~$q$ be a projection in~$\scrA^\square$ above~$e$.
Since for~$a\in(\scrA)_1$,
we have~$aq^\perp a^*=  q^\perp aa^* q^\perp 
\leq q^\perp \leq e^\perp$,
and so $a^*ea\leq q$ by~\TODO{},
we get~$\ceil{a^*ea}\leq q$
for all~$a\in\scrA$,
and thus~$p=\bigcup_{a\in\scrA}\ceil{a^*ea} \leq q$.\qed
\end{point}
\end{point}
\begin{point}[carrier-vector-state]{Exercise}%
Show that given a vector~$x$ of Hilbert space~$\scrH$
and a von Neumann subalgebra~$\scrA$
of~$\scrB(\scrH)$,
the following coincide.
\begin{enumerate}
\item
$\ceil{\,\ketbra{x}{x}\,}_{\scrA^\square}$,
the least projection in~$\scrA^\square$
above $\ceil{\,\ketbra{x}{x}\,}$;
\item
$\ceil{\,\left<x,(\,\cdot\,)x\right>|\scrA^\square\,}$,
the carrier of the vector functional of~$\scrA^\square$
given by~$x$;
\item
$\bigcup_{a\in \scrA} \ceil{\,\ketbra{ax}{ax}\,}$,
the projection on
~$\overline{\scrA x}$.
\end{enumerate}
Conclude that $\overline{\scrA^{\square\square}x}
=\overline{\scrA x}$.
(Hint: $\scrA^{\square\square\square}=\scrA^\square$.)
...
\begin{enumerate}
\item
Show that  $\omega(a)=\left<x',\varrho'(a)x'\right>$,
where $\varrho'\colon \scrA\to\scrB(\scrH')$
is the nmiu-map given by 
$\varrho'(a)y=(ay_n)_n$
for all~$a\in\scrA$ and~$y\in\scrH'$.

\item
Let~$t\in\scrA^{\square\square}$ be given.
Show that $\varrho'(t)\in \varrho'(\scrA)^{\square\square}$.
\TODO{Check the difficulty of this}

Prove that  $\varrho'(t)x\,\in\,\overline{\varrho'(\scrA)^{\square\square}x}
\equiv \overline{\varrho'(\scrA)x}$.

Deduce
that for every~$\varepsilon>0$
one can find~$a\in\scrA$ with $\|t-a\|_\omega \leq \varepsilon$.

\item
Deduce that $\scrA^{\square\square}$
is contained in the ultrastrong closure of~$\scrA$.
\end{enumerate}
Conclude that~$\scrA^{\square\square}=\scrA$.
(Hint: $\scrA$ is ultrastrongly closed, by~\sref{}.)
\end{point}
\end{point}
\begin{point}[double-commutant]{Double Commutant Theorem}%
For a collection~$\scrA$ of bounded operators
...
\begin{enumerate}
\item
$\scrA^{\square\square}$, the ``double commutant'' of~$\scrA$
in~$\scrB(\scrH)$;
\item
the ultrastrong closure of~$\scrA$ in~$\scrB(\scrH)$;
\item
the ultraweak closure of~$\scrA$ in~$\scrB(\scrH)$;
\item
...
\begin{parsec}%
\begin{point}%
We now turn to a subtle, but suprisingly
difficult question:
\TODO{ultraweak permanence}.
...
are orthogonal too.
To this end,
note that the central elements of~$\varrho(\scrA)$
coincide with the central elements of~$\varrho(\scrA)^\square$
 --- indeed, $Z(\varrho(\scrA))
= \varrho(\scrA)\cap \varrho(\scrA)^\square
= \varrho(\scrA)^{\square\square} \cap \varrho(\scrA)^\square
= Z(\varrho(\scrA)^\square)$ 
by the bicommutant theorem \TODO{}  ---
so that $\sigma_\omega$ and~$\sigma_\omega'$
are identical on central projections,
and so their central
carriers must coincide,~$\cceil{\sigma_\omega}=\cceil{\sigma_\omega'}$.
Since the projections~$\cceil{\omega}$
...
\equiv \ceil{\tau_\omega'}$ are orthogonal,
