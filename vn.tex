...
(iff it is normal).
What about arbitrary functionals?
\end{point}
...
for some~$b\in\scrH_\omega$ with~$\|b\|_\omega\leq 1$ (see \TODO{}).
\end{enumerate}
Moreover, such~$f$ is ultraweakly continuous.
\begin{point}{Proof}%
The implication 
\ref{normal-functionals-lemma-2}$\Longrightarrow$%
\ref{normal-functionals-lemma-1}
follows immediately from Cauchy--Schwarz (\sref{cs}),
while the implication
\ref{normal-functionals-lemma-1}$\Longrightarrow$%
\ref{normal-functionals-lemma-2}
follows from Riesz' representation theorem (\TODO{}),
because  to a unique linear map 
$\scrH_\omega\to\C$
 \TODO{ref?}.
