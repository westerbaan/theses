...
We refer the reader to volumes~1 and~2 of~\cite{fremlin}
for the basics of measure theory.
\end{point}
\begin{point}%
...
Indeed, take for example~$X=[0,1]$,
and let~$f\colon X\to[0,\infty)$
be any non-measurable function (see~\TODO{...}).
Then there is no least measurable function $g\in \Real{\mathcal{L}(X)}$
above~$f$.

Indeed, if for example all the points of~$X$
are negligithere is a non-empty negligible subset~$A$
in~$X$
and a non-measurable function~$f\colon X\to\R_+$,
then there is no least measurable function $g \in \mathcal{L}_R(X)$

Indeed, given ~$A\subseteq X$
there is no least measurable subset~$B$
above~$A$,
and so the bounded directed subset~$D:=\{\,f\in \mathcal{L}^\infty(X)\colon\,
f\leq \mathbf{1}_A\,\}$
has no supremum on~$\Real{\mathcal{L}^\infty(X)}$,
where~$\Define{\mathbf{1}_A}$
denotes the \Define{indicator function} of~$A$
(which is~$1$ on~$A$ and~$0$ elsewhere).
...
Let~$X$ be a complete finite measure space.
A function $f\colon X\to\C$ is \Define{measurable}
when the inverse image~$f^{-1}(U)$
of every open subset~$U$ of~$\C$
is measurable
(cf.~121B of~\cite{fremlin}).
An important example of such a measurable
function is
the indicator function~$\Define{\mathbf{1}_A}\colon X\to\C$
of a measurable subset~$A\subseteq X$,
which equals~$1$ on~$A$ and is~$0$ elsewhere.
The bounded measurable functions $f\colon X\to\C$
endowed with the supremum norm and
coordinatewise operations
form a $C^*$-algebra
we'll denote by~$\Define{\mathcal{L}^\infty(X)}$
when endowed with coordinatewise operations and order,
which is complete in the following sense.

When~$f_1,f_2,\dotsc$ is a uniformly bounded
sequence in~$\mathcal{L}^\infty(X)$
such that~$f_n(x)$ converges for all~$x\in X$,
then~$x\mapsto \lim_n f_n(x)\colon X\to\C$
is again in~$\mathcal{L}^\infty(X)$
(c.~121F of~\sref{fremlin}).
In particular,
if~$f_1\leq f_2\leq \dotsb$
is a bounded ascending sequence in~$\mathcal{L}^\infty(X)$,
then the pointwise supremum $\bigvee_n f_n\colon
x\mapsto \bigvee_n f_n(x)\colon X\to \C$
is in~$\mathcal{L}^\infty(X)$,
and is, in fact, the
supremum of the~$f_n$ in~$\mathcal{L}^\infty(X)$.

A bounded directed subset of~$\mathcal{L}^\infty(X)$
...
We'll denote the set of
bounded measurable
functions $f\colon X\to\C$
by~$\Define{\mathcal{L}^\infty(X)}$.
...
concerning a finite measure space~$X$.
\begin{enumerate}
\item
Every function in~$\mathcal{L}^\infty(X)$
is integrable (in the sense of 122M of~\cite{fremlin})
and
	
\end{enumerate}
\end{point}
\begin{point}[Linfty-basic]{Exercise}%
Let~$X$ be a finite measure space.
\begin{enumerate}
\item
Show that~$\approx$ is a congruence for coordinatewise
addition on~$\mathcal{L}^\infty(X)$,
that is, given $f\approx f'$ and~$g\approx g'$
in~$\mathcal{L}^\infty(X)$ we have~$f+g\approx f'+g'$.

Show that~$\approx$ is a congruence for coordinatewise
(scalar) multiplication and involution.
Show that in this way~$L^\infty(X)$
becomes vector space.


\item
Given~$f\in \mathcal{L}^\infty(X)$
...
\end{equation*}
exists, and $f\mapsto \|f\|$
gives a seminorm on~$\mathcal{L}^\infty(X)$
with $\|f\|=0$ iff~$f\approx 0$.

Deduce that $\|f\|=\|f'\|$ for $f,f'\in \mathcal{L}^\infty(X)$
with $f\approx f'$,
and conclude that $f\mapsto \|f\|$ gives
a norm on~$L^\infty(X)$
(called the \Define{essential supremum norm}).

\item
Given a bounded
sequence~$f_1,f_2,\dotsc$ 
in~$\mathcal{L}^\infty(X)$
such that~$f_n(x)$ converges for almost all~$x\in X$,
show that there is~$f\in \mathcal{L}^\infty(X)$
such that~$f_n(x)$ converges to~$f(x)$
for almost all~$x\in X$.

Show that if in addition~$f_1^\circ,f_2^\circ,\dotsc$
is norm Cauchy,
then~$f_1^\circ,f_2^\circ ,\dotsc$ 
norm converges to~$f^\circ$.
Deduce that~$L^\infty(X)$ is norm complete.

Show that~$L^\infty(X)$ is a $C^*$-algebra
when endowed 
with
essential supremum norm,
and coordinatewise operations.
\end{enumerate}
\end{point}
\begin{point}[Linfty-vn]{Proposition}%
Given a finite measure space~$X$ 
the $C^*$-algebra $L^\infty(X)$
is a commutative von Neumann algebra.
\begin{point}{Proof}%
Our first objective is to show that a bounded
directed subset~$D$ of $\Real{L^\infty(X)}$
has a supremum.
The trick here is to reduce the problem
to a sequence in~$\mathcal{L}^\infty(X)$.
To begin,
note
that since~$D$ is bounded,
the supremum~$\bigvee_{d\in D}\|d\|$
exists (in~$\R$),
and so we can
find~$d_1\leq d_2 \leq \dotsb$ in~$D$
with $\bigvee_n \|d_n\| = \bigvee_{d\in D} \|d\|$. 
Pick $f_1,f_2,\dotsc$ in~$\mathcal{L}^\infty(X)$
with~$f_n^\circ=d_n$
for all~$n$.
Since $f_1^\circ \leq f_2^\circ\leq \dots$
we have
$f_1(x)\leq f_2(x)\leq \dotsb$
for \emph{almost} all~$x\in X$,
and since further~$f_1^\circ \leq f_2^\circ\leq \dots$
is bounded,
we see that
$f_1(x)\leq f_2(x)\leq \dotsb$
is bounded---and hence converges---for almost all~$x\in X$.
Thus by~\sref{Linfty-basic} there is~$f\in \mathcal{L}^\infty(X)$
such that~$f_1(x),\,f_2(x),\,\dotsc$
converges to~$f(x)$ for almost all~$x\in X$.
We'll show that~$f^\circ=\bigvee D$.
