...
to hone your understanding of the ultrastrong and ultraweak topologies,
and we hope you'll thereby appreciate 
...
\TODO{Is a von Neumann algebra ultraweakly complete (so not restricted
to a bounded set) ?}
\end{enumerate}
\end{point}
\end{parsec}
...
Show that a map~$f$ which satisfies one of the equivalent conditions above
need not be ultrastrongly continuous. (Hint: transpose.)
\end{point}
...
\TODO{Perhaps generalize from $b\mapsto a^*ba$
to a cp-map?}
\begin{point}{Proof}%
...
converges ultraweakly to~$0$.\qed
\end{point}
\end{point}
...
\begin{parsec}%
\begin{point}%
We have seen that a \emph{positive} functional on a von Neumann algebra
is ultraweakly continuous iff it is ultrastrongly continuous
(iff it is normal).
In this section,
we'll show that the same result holds for arbitrary functionals.
\end{point}
\begin{point}{Definition}%
Given an np-map $\omega\colon \scrA\to\C$
on a von Neumann algebra~$\scrA$,$b\in \scrA$,
let $\Define{a*\omega}\colon \scrA\to \C$ be
the np-map given by
...
Show that~$b_1*\omega,b_2*\omega,\dotsc$ Cauchy (in the operator norm
on bounded linear functionals $\scrA\to\C$),
and converges to an np-map.
\end{enumerate}
\end{point}
...
for all~$a\in \scrA$.
Note that $[b_n,a]_\omega = \sum_{k=0}^3 i^kf_{k,n}(a)$,
where $f_{k,n}\colon \scrA\to \C$
is the np-map given by $f_{k,n}(a)=\omega(\,(i^kb_n+1)^*\,a\,(i^kb_n+1)\,)$.
We claim that~$(f_{k,n})_n$
is Cauchy in the operator norm
so that by~\TODO{} it converges to some np-map $f_k\colon \scrA\to \C$.
Indeed, given $k,m,n$, and~$a\in\scrA$, we have
\begin{alignat*}{3}
	\left|f_{k,n}(a)-f_{k,m}(a)\right|
	\ &\leq\ 
\left|\omega(\, (b_n-b_m)^*\,a\,(i^kb_n+1)\,)\right|
\,+\,
\left|\omega(\,(i^kb_m+1)^*\,a\,(b_n-b_m)\,)\right| \\
&\leq\ 
\|b_n-b_m\|_\omega \|a\| \|i^kb_n+1\|_\omega
\,+\,
\|i^kb_m+1\|_\omega \|a\| \|b_n-b_m\|_\omega.
\end{alignat*}
...
\begin{enumerate}
\item
\label{normal-functional-1}
$f$ is ultraweakly continuous;
\item
\label{normal-funtional-2}
$\left|f(a)\right|\leq 
\left|\omega_1(a)\right|\vee\dotsb\vee \left|\omega_N(a)\right|$
for all~$a\in \scrA$, for some np-maps $\omega_n\colon 
\scrA\to \C$;
\item
\label{normal-functional-3}
$f=\omega_1-\omega_2+i(\omega_3-\omega_4)$
for some np-maps $\omega_1,\dotsc,\omega_4\colon \scrA\to\C$;
\item
\label{normal-functional-4}
$f$ is ultrastrongly continuous;
\item
\label{normal-functional-5}
There is~$\delta >0$ and an np-map $\omega\colon \scrA\to\C$
such that 
\begin{equation*}
\sup\,\{\ \left|f(a)\right|\colon\, 
a\in \scrA\colon\, \|a\|_\omega\leq \delta\ \}
\ < \ \infty,
\end{equation*}
(that is, $f$ is bounded on some $\|\,\cdot\,\|_\omega$-ball);
\item
\label{normal-functional-6}
$\left|f(a)\right| \leq \|a\|_\omega$ for all~$a\in \scrA$,
for some np-map $\omega\colon \scrA\to\C$;

\TODO{verify this carefully! (or find ref.)}
\end{enumerate}
\begin{point}{Proof}%
The implications
\ref{normal-functional-3}$\Longrightarrow$\ref{normal-functional-4},
\ref{normal-functional-4}$\Longrightarrow$\ref{normal-functional-5},
and
\ref{normal-functional-5}$\Longrightarrow$\ref{normal-functional-1}
are relatively obvious, and are left to the reader,
 which leaves us with
only two implications to prove.
\begin{point}{\ref{normal-functional-3}$\Longrightarrow$\ref{normal-functional-4}}%
Let~$B>0$ be such that~$\left|f(a)\right|\leq B$ for all~$a\in \scrA$
with $\|a\|_\omega\leq \delta$.
Let~$a\in\scrA$, and~$\varepsilon>0$ be given.
Then for~$\tilde{a}:=\delta(\varepsilon+\|a\|_\omega)^{-1}\,a$,
we have $\|\tilde{a}\|_\omega\leq \delta$,
and so~$\left|f(\tilde{a})\right|
...
B\delta^{-1}(\varepsilon+\|a\|_\omega)$.
Since~$\varepsilon>0$ was arbitrary, we get~$\left|f(a)\right|\leq
B\delta^{-1}\|a\|_\omega = \|a\|_{B^2\delta^{-2}\omega}$.
\end{point}
\begin{point}{\ref{normal-functional-4}$\Longrightarrow$\ref{normal-functional-5}}%
asdasd
\end{point}
\end{point}
\end{point}
...
(Hint: browse~\sref{cstar-involution-basic}.)
\item
