...
\begin{point}{Proof}%
Since~$\pi$ is the composition
of the $u^*(\,\cdot\,)u\colon \scrA\to\scrA$
and $u^*u(\,\cdot\,)u^*u\colon \scrA
\to u^*u\scrA u^*u$,
which are both known to be ncp-maps
(by~\sref{}),
we see that~$\pi$ is an ncp-map itself.
