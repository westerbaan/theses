...
\emph{Paschke dilation}\cite{wwpaschke}\REMARK{a reference to
Bas' thesis will be added here}
$\xymatrix{\scrA
	\ar[r]|-\varrho
&
	\scrP\ar[r]|-c
...
\end{point}
\begin{point}[corner-vna-basic]{Exercise}%
...
\end{parsec}
\begin{parsec}%
\begin{point}[dfn-standard-corner-and-filter]{Definition}%
...
\index{*bracketsf@$[f]$, for an ncp-map}
such that 
\begin{equation*}
\xymatrix@C=4em{
\scrA
...
which is a nmiu-isomorphism by~\sref{iso}.)
\end{point}
...
\end{equation*}
for all~$s\in \Proj(\scrA)$ and~$t\in \Proj(\scrB)$.
\begin{point}[diamond-sum-proof]{Proof}%
...
\end{point}
\begin{point}[rigid-ncp-extreme]{Proposition}%
...
but also to contrast it with
	...
	that appears in the following thesis.\REMARK{add ref.}
\end{point}
...
from~\cite{model}
build of von Neumann algebras,
but more of that later.
\end{point}
\end{parsec}
\subsection{Definition}
\begin{parsec}%
	\begin{point}[bilinear-basic]{Definition}%
A bilinear map
...
\end{enumerate}
\end{point}
\begin{point}[hilb-tensor-basic]{Proposition}%
...
\end{point}
\end{point}
\begin{point}[hilb-tensor-universal-property]{Theorem}%
...
\end{point}
\end{point}
\begin{point}[mult-completely-monotone]{Exercise}%
...
\end{point}
\begin{point}[hilb-tensor-functor]{Proposition}%
...
\end{point}
\end{point}
\begin{point}[tensor-product-norm]{Exercise}%
...
\end{point}
\begin{point}[tensor-functorial]{Proposition}%
...
\end{point}
\end{point}
\begin{point}[tensor-functor]{Exercise}%
...
proves useful.
\end{point}
\begin{point}[tensor-characterization]{Theorem}%
...
\end{point}
\begin{point}[sum-generation]{Exercise}%
Let~$(\scrA_i)_{i\in I}$
...
\end{enumerate}
\end{point}
\begin{point}[tensor-distributes-over-sums]{Proposition}%
...
\begin{itemize}
\item
The observation (by to Kornell, \cite{kornell2012})
that
the category 
$\op{(\W{miu})}$
is monoidal closed,
...
had no time to incorporate its proof in this text.
\end{point}
...
\begin{enumerate}
\item
Given a nmiu-functional $\varphi\colon \scrA\otimes \scrB\to\C$
...
and equalisers (\sref{vn-equalizers}),
...
\begin{point}[tensor-closed]{Theorem (Kornell)}%
...
and~$\scrB$.
\begin{point}{Proof}%
See Corollary~1 of~\cite{russodye}.\qed
\end{point}
\end{point}
...
\end{point}
\end{point}
\begin{point}[lem:measure-space-partition]{Exercise}%
