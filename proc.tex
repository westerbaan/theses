...
but to this end must verify first that $\pi_i(f(\scrA))= M_{N_i}$
for all~$i\in I$.
To see this, note that $f(\scrA)\subseteq \bigoplus_{i \in I} 
...
Indeed, since  $f(\scrA)\subseteq \varrho_1(\mathcal{F}_\mathrm{ha}(\scrA))$,
we have  $\scrB=W^*(f(\scrA))\subseteq 
\varrho_1(\mathcal{F}_\mathrm{ha}(\scrA))$---using here that
the range of an nmiu-map is a von Neumann subalgebra, see MARK.
...
\colon \haW{miu}\longrightarrow\haW{cpsu}.
\end{equation*}
\spacingfix{}
\begin{point}{20}{Proof}%
Clearly, the subcategory $\haW{miu}$ of~$\W{miu}$
is closed under products,
and these products
    are preserved by the inclusion 
    $U_{\mathrm{ha}}\colon \haW{miu}\longrightarrow \haW{cpsu}$.
By~\sref{ha-equalisers},
$\haW{miu}$ is closed under equalisers as well,
and these equalisers are preserved by~$U_\mathrm{ha}$.
As in the proof of~\sref{second-adjunction},
the result is obtained
by an application of Freyd's adjoint functor theorem,
but this time
    we use as solution set for a \emph{hereditarily atomic}
von Neumann algebra~$\scrA$
the set of ncpsu-maps $\gamma\colon \scrA\to \scrC$
where~$\scrC$
is a \emph{hereditarily atomic} von Neumann algebra on a subset of
$\kappa:= \smash{2^{2^{\#\C+\#\scrA}}}$.
Aside from this detail
the reasoning is the same as in~\sref{second-adjunction-proof}.\qed
\end{point}
\end{point}
\end{parsec}
