...
Whether this hints at an error in our proof,
or at the power of the adjoint functor theorem,
remains to be seen.
\end{point}
\end{parsec}%
\begin{parsec}{1254}%
...
\end{point}
\begin{point}{70}{Theorem}
Let~$\scrA$ and~$\scrB$ be hereditarily atomic 
von Neumann algebras.
Writing 
\begin{equation*}
    \textstyle
\Define{\scrA^{*_\mathrm{ha}\scrB}}
   \  =\  \bigoplus_{s\in S_{\scrA,\scrB}} M_{N_s},
\end{equation*}
\index{*AstarBha@$\scrA^{*_\mathrm{ha}\scrB}$}%
    the unique nmiu-map $\eta_{\scrA,\scrB}$ in
    \begin{equation*}
\xymatrix@C=8em{
\scrA
        \ar[r]^-{\Define{\eta_{\scrA,\scrB}}}
        \ar[rd]_{\left<s\right>_{s\in S_{\scrA,\scrB}}}
        &
...
\\
    ...
    \end{equation*}
is a universal arrow from~$\scrA$
    to $(\,\cdot\,)\otimes \scrB\colon \haW{miu}\to\haW{miu}$.
\begin{point}{80}{Proof}%
Let~$f\colon \scrA\to \scrC\otimes \scrB$
be a nmiu-map,
where $\scrC$ is some hereditarily atomic von Neumann algebra.
We must show that
there is a unique nmiu-map $\varrho\colon 
    \scrA^{*_\mathrm{ha}\scrB}\to\scrC$
    with $f=(\varrho\otimes\scrB)\circ \eta_{\scrA,\scrB}$.
