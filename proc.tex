...
the ncp-map $a\mapsto b^* a b,\ \ceill{b}\!\scrA\!\ceill{b}\to\scrA$
is rigid.
\begin{point}{Proof}%
Let~$g\colon \ceill{b}\!\scrA\!\ceill{b}\to\scrA$
be an ncp-map with~$g(1)=b^*b$
and $\ceil{b^*pb}=\ceil{g(p)}$ for every projection~$p$ 
of $\ceill{b}\!\scrA\!\ceill{b}$.
To prove that~$b^*(\,\cdot\,) b$ is rigid,
we must show that~$g=b^*(\,\cdot\,)b$.
Since~$b^*(\,\cdot\,)b$ is a filter
(by~\sref{canonical-filter})
and~$g(1)=b^*b$
there is a unique ncp-map~$h\colon \ceill{b}\!\scrA\!\ceill{b}
\to\ceill{b}\!\scrA\!\ceill{b}$
with~$g(a)=b^* h(a)b$ for all~$a\in\ceill{b}\!\scrA\!\ceill{b}$.
Our task is to show that~$h(a)=a$ for all
$a\in\ceill{b}\!\scrA\!\ceill{b}$,
and for this it suffices to show that,
for all~$a\in\ceill{b}\!\scrA\!\ceill{b}$,
\begin{equation}
...
we have $s_n b = bs_n = e_n$,
and so
$
\ceil{e_n g(p) e_n}
=
\ceil{s_n^* b^* g(b)b s_n }
