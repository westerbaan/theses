...
admit a significantly more concrete (yet still impredicative)
...
\begin{point}{10}
We're going to show that 
the inclusion $\haW{cpsu}\longrightarrow \haW{miu}$
has a left adjoint,
but for this we need some notation first.
\end{point}
\begin{point}{20}
...
Now, pick a set of representatives
$\Define{R_\scrA}$\index{RA@$R_\scrA$}
for this miu-equivalence.
\end{point}
\begin{point}{30}{Theorem}%
Given a hereditarily atomic von Neumann algebra~$\scrA$,
the ncpsu-map
\begin{equation*}
    \textstyle
    \left<r\right>_{r\in R_\scrA}\colon \scrA
    \longrightarrow 
     \bigoplus_{r\in R_\scrA} M_{N_r} =:
    \Define{\mathcal{F}_\mathrm{ha}(\scrA)}
\end{equation*}%
is a universal arrow from~$\scrA$ to the inclusion
functor $\haW{miu}\longrightarrow \haW{cpsu}$.
\begin{point}{40}{Proof}%
Let~$f\colon \scrA\to \scrB$
be an ncpsu-map to some hereditarily atomic von Neumann algebra~$\scrB$.
To prove this theorem we must show that there is a unique nmiu-map
$\varrho\colon \mathcal{F}_\mathrm{ha}(\scrA)\longrightarrow \scrB$
with $f = \varrho\circ \left<r\right>_{r\in R_\scrA}$.
We may assume that $W^*(f(\scrA))=\scrB$,
since the problem is easily reduced to this case.
\begin{point}{50}{Uniqueness}%
Let~$\varrho_1,\varrho_2\colon \mathcal{F}_\mathrm{ha}(\scrA)
\longrightarrow \scrB$
be nmiu-maps with 
\begin{equation*}
    \varrho_1\circ \left<r\right>_{r\in R_\scrA}
= \varrho_2 \circ \left<r\right>_{r\in R_\scrA}.
\end{equation*}
We must show that~$\varrho_1=\varrho_2$.
Writing $\scrB\equiv \bigoplus_{i \in I} M_{N_i}$
it suffices to show that~$\pi_i\circ \varrho_1=\pi_i\circ \varrho_2$
for all~$i\in I$, 
where $\pi_i$
is the $i$-th projection.

We'd like to reduce to the case that $\scrB= M_N$
for some~$N$ by considering $\pi_i\circ f$ instead of~$f$,
but to this end we must verify first that $\pi_i(f(\scrA))= M_{N_i}$
for all~$i\in I$.
To see this, note that clearly  $f(\scrA)\subseteq \bigoplus_{i \in I} 
W^*(\pi_i (f(\scrA)))\subseteq \scrB$,
and so~$\scrB = W^*(f(\scrA)) 
\subseteq  W^*(\pi_i (f(\scrA)))\subseteq \scrB$,
which implies that
$\bigoplus_{i\in I} W^*(\pi_i (f(\scrA)))=\scrB$,
and so, indeed, $\pi_i(f(\scrA))=M_{N_i}$ for all~$i\in I$.

By considering $\pi_i\circ f$ instead of~$f$,
we see that 
for the purpose of showing that~$\varrho_1=\varrho_2$
we may assume without loss of generality
that~$\scrB=M_N$ for some natural number~$N$.
We claim that~$\varrho_1$ is surjective.
Indeed, since $f=\varrho_1\circ \left<r\right>_{r\in R_\scrA}$,
and so $f(\scrA)\subseteq \varrho_1(\mathcal{F}_\mathrm{ha}(\scrA))$,
we have  $\scrB=W^*(f(\scrA))\subseteq 
\varrho_1(\mathcal{F}_\mathrm{ha}(\scrA))$---using here that
the range of an nmiu-map is a von Neumann subalgebra, see~\sref{nmiu-image}.

We claim that~$\varrho_1$ must be of the form~$\varrho_1'\circ \pi_{r_1}$
for some nmiu-isomorphism $\varrho_1'\colon M_{N_{r_1}}\to M_N$,
and $r_1\in R_\scrA$.
To see this,
first note that 
since~$\varrho_1$ is surjective, it sends
central projections of~$\mathcal{F}_\mathrm{ha}(\scrA)$
to central projections of~$M_N$.
For each~$r\in R_\scrA$,
let $c_r$ denote the central projection in~$\mathcal{F}_\mathrm{ha}(\scrA)$
given by $c_r(r)=1$ and $c_r(r')=0$ for all~$r'\in R_\scrA\backslash\{r\}$.
Then~$(c_r)_{r\in R_\scrA}$
is a family of orthogonal central projections with 
    $\sum_{r\in R_\scrA} c_r=1$.
So~$(\varrho_1(c_r))_{r\in R_\scrA}$
is an orthogonal family of central projections as well,
    with $\sum_{r\in R_\scrA} \varrho_1(c_r)=1$.
Since the only non-zero central projection 
    in~$M_N$ (being a factor see~\sref{central-examples})
is~$1$, it follows that
there is exactly one $r_1\in R_\scrA$
with $\varrho_1(c_{r_1})=1$,
and $\varrho_1(c_r)=0$ for all $r\in R_\scrA\backslash\{r_1\}$.
From this one easily deduces (c.f.~\sref{nmiu-factors}),
that~$\varrho_1$ must be of the form
$\varrho_1 = \varrho_1'\circ \pi_{r_1}$
for some nmiu-map $\varrho_1'\colon M_{N_{r_1}}\to M_N$.
Since~$\varrho_1$ is surjective,
$\varrho_1'$ must be surjective too.
Moreover,
$\varrho_1'$ is injective,
because~$\ceil{\varrho_1'}$ is a central projectio  (see~\sref{carrier-miu}).
in~$M_{N_{r_1}}$ that can't be~$0$.
Hence~$\varrho_1'$ is a nmiu-isomorphism.

By a similar reasoning~$\varrho_2\equiv \varrho_2'\circ \pi_{r_2}$
for some $r_2\in R_\scrA$ and some nmiu-isomorphism 
$\varrho_2\colon M_{N_{r_2}}\to M_N$.
Since then  
\begin{equation*}
    \varrho_1'\circ r_1
= \varrho_1'\circ \pi_{r_1} \circ \left<r\right>_{r\in R_\scrA}
= \varrho_1\circ \left<r\right>_{r\in R_\scrA}
= \varrho_2 \circ \left<r\right>_{r\in R_\scrA}
= \varrho_2'\circ r_2,
\end{equation*}
we see that~$r_1$ and~$r_2$ are miu-equivalent,
and thus equal.
Since then
$\varrho_1'\circ r_1 = \varrho_2'\circ r_2 \equiv 
\varrho_2'\circ r_1$,
the assumption
$W^*(r_1(\scrA))=M_{N_{r_1}}$
entails~$\varrho_1' = \varrho_2'$.
Hence $\varrho_1\equiv \varrho_1'\circ \pi_{r_1}
= \varrho_2'\circ \pi_{r_2}\equiv \varrho_2$.
\end{point}
\begin{point}{60}{Existence}
Recall that we're given a ncpsu-map $f \colon \scrA\to\scrB$
    with $W^*(f(\scrA))=\scrB$,
and that we must find a nmiu-map $\varrho\colon 
\mathcal{F}_\mathrm{ha}(\scrA)\to \scrB$
with $f= \varrho\circ \left<r\right>_{r\in R_\scrA}$.
Writing $\scrB\equiv \bigoplus_{i \in I}M_{N_i}$,
we again have $W^*(\pi_i(f(\scrA)))=M_{N_i}$
for all~$i\in I$.
By definition of~$R_\scrA$
there is, given $i\in I$,
a unique $r_i\in R_\scrA$
miu-equivalent to~$\pi_i\circ f$.
So there's a nmiu-isomorphism $\varphi_i\colon M_{N_{r_i}}\to M_{N_i}$
with $\varphi_i \circ r_i = \pi_i\circ f$.
Now, define~$\varrho := \left<\varphi_i\circ \pi_{r_i}\right>_{i\in I}$.
Then~$\varrho$ is a nmiu-map,
and~$f = \varrho \circ \left<r\right>_{r\in R_\scrA}$,
because
$\pi_i\circ f 
\equiv  \varphi_i \circ r_i
    \equiv \varphi_i\circ \pi_{r_i} \circ \left<r\right>_{r\in R_\scrA}
=  \pi_i \circ  \varrho \circ \left<r\right>_{r\in R_\scrA}$
for all~$i\in I$.\qed
\end{point}
\end{point}
\end{point}
\begin{point}{70}{Corollary}%
The inclusion $\haW{miu}\longrightarrow\haW{cpsu}$
has a left adjoint 
\begin{equation*}\Define{\mathcal{F}_\mathrm{ha}}
\colon \haW{cpsu}\longrightarrow\haW{miu}.
\end{equation*}
\index{F_ha@$\mathcal{F}_\mathrm{ha}\colon \haW{cpsu}\to\haW{miu}$}%
\spacingfix{}
\end{point}
\end{parsec}
