...
\end{point}
\end{parsec}
\section{Model of the Quantum Lambda Calculus}
\begin{parsec}%
\begin{point}%
...
especially designed
to include both 
function types and quantum data types in a single system.
This unification
...
is seems,
is to find a concrete model for the quantum lambda calculus:
the first model (other than the term model)
 appeared only five years later.

We end this thesis by showing that
von Neumann algebras
form a model 
of the quantum lambda calculus.
For this we do not actually
need to deal with the quantum lambda calculus itself
or even to know what it is, because
Selinger and Valiron have already identified the type
of category that may serve as a model of the quantum lambda calculus,
and so we will only 
show that $\op{(\W{miu})}$ is such a category,\footnote{%
If you're interested the resulting
model is conceretely  described in~\TODO{Kenta et Bram}.}
and for we essentially need to prove that:
\begin{enumerate}
\item
The category 
$\op{(\W{miu})}$
...
\end{equation*}
\item
All functors in the diagram 
above except~$\mathcal{F}$
are strong monoidal,
that is, preserve the tensor product
up to isomorphism
(with as tensor product on~$\Cat{Set}$
simply the cartesian product, $\times$).
\item
$\op{(\W{cpsu})}$ is isomormorphic
to the Kleisli category
of the adjunction on the right
in the diagram above (via the comparision functor \TODO{...}).
\end{enumerate}
\end{point}
\end{parsec}
\begin{parsec}%
...
is injective (by \TODO{tensor of injections is injective})
