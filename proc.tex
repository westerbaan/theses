...
Our description
of the norm and ultraweak topology on the 
algebraic tensor product below
...
That this determines both norm and 
...
\begin{parsec}%
\begin{point}{Proposition}%
When the algebraic tensor product
$\scrA\odot \scrB$
of von Neumann algebras~$\scrA$ and~$\scrB$ of bounded operators
on Hilbert spaces $\scrH$ and~$\scrK$, respectively,
is mapped into~$\scrB(\scrH\otimes \scrK)$
via the extension of the bilinear map~$\beta(A,B)=A\otimes B$,
this extension~$\beta_\odot\colon \scrA\odot\scrB\to \scrB(\scrH\otimes \scrK)$
is an isometry (with respect to the tensor product norm)
and a homeomorphism from the ultraweak tensor product topology
to the ultraweak topology on~$\scrB(\scrH\otimes \scrK)$.
\begin{point}{Proof}%
