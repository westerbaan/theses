...
in concrete terms,
and will call it the \emph{tensor product topology}.
We'll also need to introduce
a \emph{tensor product norm} on~$\scrA\odot\scrB$
that will turn out to be the norm on~$\scrA\odot \scrB$
induced by~$\scrA\otimes \scrB$.
\end{point}
\begin{point}{Definition}%
Let~$\scrA$ and~$\scrB$ be von Neumann algebras.
\begin{enumerate}
\item
We'll call a map $\omega \colon \scrA\odot\scrB\to\C$
a \Define{product functional}
when $\omega\equiv \sigma\odot\tau$
for some np-maps $\sigma\colon\scrA\to\C$
and~$\tau\colon \scrB\to\C$,
and a \Define{simple functional}
when
$\omega\equiv (\sigma\odot \tau)(t^*(\,\cdot\,)t)$
for some~$t\in \scrA\odot\scrB$
and~$\sigma$ and~$\tau$ as before.

\Define{Product states} and 
\Define{simple states}
are unital versions of such maps.
\item
Each simple functional $\omega \colon \scrA\odot\scrB\to\C$
gives us an operation~$\Define{[\,\cdot\,,\,\cdot\,]_\omega}$,
that will turn out to be an inner product in~\TODO{},
given by
$\Define{[s,t]_\omega}:=\omega(s^*t)$
(cf.~\sref{state-inner-product}),
and an associated semi-norm
denoted by~$\Define{\|t\|_\omega}:=[t,t]_\omega = \omega(t^*t)$.
...
\end{equation*}
where~$\omega$ ranges over all simple \emph{states}
on~$\scrA\odot\scrB$.
\item
...
We'll see that all simple functionals are bounded in~\TODO{...}.
