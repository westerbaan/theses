...
(see~\sref{approximate-pseudoinverse-def},
which exists by~\sref{approximate-pseudo-inverse}),
...
for~$b\in\scrB$ --- 
which are clearly ncp by~\sref{ad-ncp},
converge ultraweakly to~$g$,
and in fact uniformly so on~$[0,1]_\scrB$ by~\TODO{...},
so that~$g$ is ncp by~\sref{ncp-uwlim}.\qed
\end{point}
\end{point}
\end{parsec}
\begin{parsec}
