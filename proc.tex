...
\begin{enumerate}
\item
Assuming that~$p$ and~$q$ are centrally similar,
show that if~$a\in\scrA$ commutes with~$p$,
then~$a$ commutes with~$q$,
and conclude that~$p$ and~$q$ commute.
\item
Show that $\ceil{p}=\ceil{q}$
when~$p$ and~$q$ are centrally similar.

\item
Assuming~$p$ and~$q$ are invertible,
show that~$p$ and~$q$ are centrally similar
iff $qp^{-1}$ is central iff $pq^{-1}$ is central
iff both
$\frac{p\wedge q}{p}$ and~$\frac{p\wedge q}{q}$
are central.
\item
Assume that~$p$ and~$q$ commute and~$\ceil{p}=\ceil{q}$,
and let~$e_1\leq e_2\leq \dotsb$
be the sequence of projections from~\TODO{...}
commuting with~$p$ and~$q$
such that~$\bigcup_n e_n = \ceil{p}=\ceil{q}$
and $e_n p$ and~$e_n q$ are invertible for all~$n$.

Show that $\frac{e_np\,\wedge\, e_nq}{e_n p}$
converges ultraweakly to~$\frac{p\wedge q}{p}$.

Conclude that~$p$ and~$q$ are centrally similar
iff~$e_np$ and~$e_nq$ are centrally similar for all~$n$.
\end{enumerate}
\end{point}
