...
\begin{parsec}%
\begin{point}%
Let us now turn to the matter
of a universal property for the tensor product
of von Neumann algebras.

To this end recall that the algebraic tensor product
...
map $\beta_\otimes \colon  \scrT\to\C$,
...
\item
\TODO{definitions of (completely) positive bilinear maps}
\end{enumerate}
\end{enumerate}
\end{point}
...
be a tensor product of von Neumann algebras.

The extension  $\gamma_\odot\colon \scrA\odot\scrB\to\scrT$
is an isometry
and a topological embedding
(from the tensor product topology on~$\scrA\odot\scrB$
to the ultraweak topology on~$\scrT$).

Moreover,
the assignment $f\mapsto f\circ\gamma_\odot$
gives an isometry $\scrA_*\to(\scrA\odot\scrB)^*$
whose range
consists
of all norm limits of simple functionals on~$\scrA\odot\scrB$.
\begin{point}{Proof}%
Recall that by definition of the tensor product (see~\sref{tensor})
the product functionals on~$\scrT$
are center separating,
and the image $\gamma_\odot(\scrA\odot\scrB)$ 
of the algebraic tensor product
is ultraweakly dense in~$\scrT$.
This entails by~\sref{vn-center-separating-funcamental}
...
Since $\omega/\omega(1)$ is a in~$\Omega$ for every~$\omega\in\Omega$
with~$\omega\neq 0$,
the subset~$\Omega_1$ of~$\Omega$
of unital maps is order separating as well,
and so determines the norm on~$\scrT$
via~$\|a\|^2=\|a^*a\| = \sup_{\omega\in\Omega_1} \omega(a^*a)$
for all~$a\in\scrT$
by~\sref{order-separating-norm}.
Taking~$a=\gamma_\odot(s)$
where~$s\in\scrA\odot\scrB$
yields $\|\gamma_\odot(s)\|^2 = \sup_{\omega\in\Omega_1}
\omega(\gamma_\odot( s^*s ))=
\sup_{\omega\in\Omega_1} \|s\|_{\omega\circ \tau_\odot}^2$,
where we've used
the obvious fact that~$\omega\circ \gamma_\odot$
is a basic functional (\sref{tensor-extra})
for every~$\omega\in\Omega_1$.
Since every unital basic functional is of this form
(since product functionals, and thus basic functionals too,
can be extended uniquely from~$\scrA\odot\scrB$ to~$\scrT$),
we see that~$\|\gamma_\odot(s)\|=\sup_{\omega\in\Omega_1}
\|s\|_{\omega\circ \tau_\odot}=\|s\|$
for all~$s\in\scrA\odot\scrB$
by definition of the tensor product norm
on~$\scrA\odot\scrB$ from~\sref{tensor-extra}.
Hence~$\gamma_\odot$ is an isometry.

Let~$f\in\scrT_*$
be a normal functional on~$\scrT$.
Since~$\|f(\gamma_\odot(s))\|\leq \|f\| \|s\|$
for all~$s\in\scrA\odot\scrB$,
we see that $f\circ \gamma_\odot$
is bounded;
and clearly so too is
the linear map $(\,\cdot\,)\circ \gamma_\odot\colon 
\scrA_*\to(\scrA\odot\scrB)^*$
thereby obtained,
but it requires some heavier machinery to 
show that $(\,\cdot\,)\circ \gamma_\odot$ is an isometry.
Indeed, as a consequence
of the polar decomposition of normal functionals
(\sref{polar-decomposition-of-functional})
there is a partial isometry~$u$ in~$\scrT$
with~$f(u)=\|f\|$ (see~\TODO{...}). 
Now, let~$\scrA$ be the norm closure
of~$\gamma_\odot(\scrA\odot\scrB)$,
which is a $C^*$-subalgebra of~$\scrT$,
which is ultraweakly 
and thus ultrastrongly dense in~$\scrT$ by~\sref{ultraclosed}.
Thus by Kaplansky's density theorem (\sref{kaplansky})~$u$
is the ultrastrong limit of a net of elements from $(\scrA)_1$.
Now, since each element~$a$ of the unit ball~$(\scrA)_1$
of~$\scrA$
can be obtained as a limit of a sequence $t_1,t_2,\dotsc$ of elements
from~$\gamma_\odot(\scrA\odot\scrB)$,
and the norms~$\|t_n\|$ will tend to~$\|a\|\leq 1$,
one can find,
given $\varepsilon>0$,
a net~$(s_\alpha)_\alpha$
in $\scrA\odot\scrB$
such that~$\gamma_\odot(s_\alpha)$
converges ultrastrongly to~$u$
as~$\alpha\to\infty$
and  $\|s_\alpha\|\leq 1+\varepsilon$ for all~$\alpha$.
Since then
$\left|f(\gamma_\odot(s_\alpha))|\right|\leq \|f\circ \gamma_\odot\|
\|s_\alpha\|\leq \|f\circ\gamma_\odot\|(1+\varepsilon)$
for all~$\alpha$,
we have $\|f\|=f(u)=\left|f(u)\right|
=\lim_\alpha \left|f(s_\alpha)\right|
\leq \|f\circ \gamma_\odot\|(1+\varepsilon)$.
As~$\varepsilon>0$ was arbitrary,
we get~$\|f\|\leq \|f\circ \gamma_\odot\|$,
and conclude that  $\|f\|=\|f\circ \gamma_\odot\|$.
Hence~$f\mapsto f\circ \gamma_\odot$
is an isometry.

Let us show that~$\gamma_\odot\colon \scrA\odot\scrB\to\scrT$
is a ultraweak embedding,
that is,
an ultraweak homeomorphism onto its range.
To begin, $\gamma_\odot$ is ultraweakly continuous,
because $\omega\circ\gamma_\odot$
is ultraweakly continuous for every np-functional $\omega$ on~$\scrT$;
indeed, such~$\omega$
is the norm limit of finite sums of functionals from~$\Omega$,
and thus (as and~$f\mapsto f\circ \gamma_\odot$ is bounded),
$\omega\circ\gamma_\odot$
is the norm limit of simple functionals on~$\scrA\odot\scrB$,
which is ultraweakly continuous
by definition~\sref{tensor-extra}.
It remains to be shown that the
inverse of~$\gamma_\odot$
as map $\scrA\odot\scrB\to\gamma_\odot(\scrA\odot\scrB)$
is ultraweakly continuous,
and for this
it suffices to show that any norm limit of simple functionals
on~$\scrA\odot\scrB$
can be extended to~$\scrT$ along~$\gamma_\odot$.
This is, of course,
pretty obvious
since we've already seen that basic functionals can be extended,
and so simple functionals can be extended,
and thus limits of simple functionals can be extended
(using here the facts that~$f\mapsto f\circ \gamma_\odot$
is an isometry, and the norm limit
of an np-functional is again an np-functional, see~\TODO{...}).\qed
\end{point}
\end{point}
\begin{point}{Theorem}%
...
\end{point}
\begin{point}{Exercise}%
Show that the extension $\beta_\otimes\colon \scrA\otimes \scrB\to\scrC$
of a normal bounded bilinear map $\scrA\times \scrB\to\scrC$
between von Neumann algebras
along a tensor product  $\otimes \colon \scrA\times \scrB\to\scrA\otimes \scrB$
of~$\scrA$ and~$\scrB$
is unital iff~$\beta$ is unital;
$\beta_\otimes$ is involution preserving iff~$\beta$ is involution preserving;
and
$\beta_\otimes$ is multiplicative iff~$\beta$ is multiplicative.
\end{point}
\begin{point}{Exercise}%
Show that the tensor product of von Neumann algebras~$\scrA$
and~$\scrB$ is unique in
the sense
that when~$\otimes\colon \scrA\times \scrB\to\scrA\otimes \scrB$
and~$\otimes'\colon \scrA\times \scrB\to\scrA\otimes'\scrB$
are tensor products of~$\scrA$ and~$\scrB$,
then there is a unique
nmiu-isomorphism $\varphi\colon \scrA\otimes \scrB\to\scrA\otimes'\scrB$
with $\varphi(a\otimes b)=a\otimes'b$
for all~$a\in\scrA$ and~$b\in\scrB$.
\end{point}
\end{parsec}
\begin{parsec}%
\begin{point}[tensor-fundamental-lemma]{Lemma}%
A nmiu-bilinear map $\gamma\colon \scrA\times \scrB\to\scrT$
is a tensor product if
\begin{enumerate}
\item
\label{tensor-fundamental-lemma-1}
$\gamma_\odot\colon \scrA\odot\scrB\to\scrT$
has ultraweakly dense range, 
\item
\label{tensor-fundamental-lemma-2}
$\gamma$ \emph{has product functionals},
that is,
for all np-maps
 $\sigma\colon \scrA\to\C$,
$\tau\colon \scrB\to\C$
there is a np-functional 
$\Define{\gamma(\sigma, \tau)}\colon \scrT\to\C$
with $\gamma(\sigma,\tau)(\gamma(a, b))= \sigma(a)\tau(b)$
for all~$a\in\scrA$ and~$b\in\scrB$,
and
\item
\label{tensor-fundamental-lemma-3}
the collection
of all such product functionals  $\gamma(\sigma, \tau)$
is faithful.

(We'll see in~\TODO{...} that this third condition can be dropped.)
\end{enumerate}
Moreover, in that case~$\gamma_\odot$
is an isometry
and a topological embedding
from the ultraweak tensor product topology on~$\scrA\odot \scrB$
to the ultraweak topology on~$\scrT$.
\begin{point}{Proof}%
Let us first show that~$\gamma_\odot$
is an isometry,
that is, that~$\|t\|=  \|\gamma_\odot(t)\|$
for given~$t\in\scrA\odot\scrB$.
To see that~$\|t\| \leq \|\gamma_\odot(t)\|$
is suffices to show that $\|t\|_\omega \leq \|\gamma_\odot(t)\|$
for any basic~$\omega\colon \scrA\odot\scrB\to\C$
with~$\omega(1)\leq 1$ by definition of the tensor product norm.
Note that $\omega$ is of the form
$\omega=(\sigma\odot\tau)(s^*(\,\cdot\,)s)$
for some np-functionals $\sigma\colon\scrA\to\C$
and~$\tau\colon \scrB\to\C$
and~$s\in\scrA\odot\scrB$.
By condition~\ref{tensor-fundamental-lemma-2}
there is an np-map $\gamma(\sigma,\tau)\colon \scrT\to\C$
with $\gamma(\sigma,\tau)(\gamma(a,b))=\sigma(a)\tau(b)$
for all~$a \in \scrA$ and~$b\in \scrB$,
or, in other words, $\gamma(\sigma,\tau)\circ \gamma_\odot = \sigma\odot\tau$.
Then the np-functional $\omega'$ on~$\scrT$
given by~$\omega'=
\gamma(\sigma,\tau)(\gamma_\odot(s)^*(\,\cdot\,)\gamma_\odot(s))$
clearly extends~$\omega$
in the sense that $\omega'\circ \gamma_\odot = \omega$,
so that
$\omega'(1)=\omega(1)\leq 1$
and
$\|t\|^2_\omega = \omega(t^*t)
=\omega'(\gamma_\odot(t^*t))
\leq \omega'(1) \,\|\gamma_\odot(t^*t)\|
\leq \|\gamma_\odot(t)\|^2$,
and thus~$\|t\|\leq \|\gamma_\odot(t)\|$.

\end{point}
\end{point}
\begin{point}{Theorem}%
A nmiu-bilinear map~$\gamma\colon\scrA\times \scrB\to\scrT$
is a tensor product iff
\begin{enumerate}
\item
$\gamma_\odot\colon \scrA\odot\scrB\to\scrT$
has ultraweakly dense range,
...
$\gamma$ \emph{has product functionals},
that is,
for all np-maps $\sigma\colon \scrA\to\C$, $\tau\colon \scrB\to\C$
there is an np-functional $\omega\colon \scrT\to\C$ 
with $\omega(\gamma(a,b))=\sigma(a)\tau(b)$.
\end{enumerate}
\begin{point}{Proof}%
	\TODO{...}
\end{point}
...
\begin{point}{Proposition}%
Let~$\scrA$ and~$\scrB$
be von Neumann algebras of bounded operators
on Hilbert spaces~$\scrH$ and~$\scrK$, respectively.
Let~$\beta\colon \scrA\times \scrB\to \scrB(\scrH\otimes \scrK)$
be the bilinear map that sends~$(A,B)$ to~$A\otimes B$.
Then~$\beta_\otimes\colon \scrA\otimes \scrB\to 
\scrB(\scrH\otimes \scrK)$
is an isometry with respect
to the tensor product norm on~$\scrA\otimes \scrB$
and the operator norm on~$\scrB(\scrH\otimes\scrK)$.
\end{point}
\end{parsec}
\begin{parsec}%
\begin{point}{Theorem}%
