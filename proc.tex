...
This description
is based on the simple premise that the product functionals
$\sigma\otimes \tau\colon \scrA\otimes\scrB\to\C$
should turn out to be center separating;
...
and say, for instance, that $\beta$ is \Define{nmiu-bilinear}
when it is normal, unital, multiplicative and involution preserving.
\item
A \Define{tensor product} of~$\scrA$ and~$\scrB$
is a bounded nmiu-bilinear map 
$\Define{\otimes} \colon \scrA\times \scrB\to
\Define{\scrA\otimes \scrB}$
which is universal
in the sense
that for every
normal bounded bilinear map $\beta\colon \scrA\times \scrB\to\scrC$
to a von Neumann algebra~$\scrC$
...
\begin{point}{Proof}%
We need to check that the three conditions of~\sref{tensor-fundamental-lemma} hold.
\begin{point}{Condition~\ref{tensor-fundamental-lemma-1}}%
Since~$\scrT$ is easily seen
to be the ultraweak closure
of the range of
$\otimes_\odot\colon \scrA\odot\scrB\to\scrB(\scrH\otimes\scrK)$,
the range of~$\gamma_\odot$ (being the same
as the range of~$\otimes_\odot$)
is ultraweakly dense in~$\scrT$.
\end{point}
\begin{point}{Condition~\ref{tensor-fundamental-lemma-2}}%
...
for all~$A\in\scrA$ and~$B\in\scrB$.
\end{point}
\begin{point}{Condition~\ref{tensor-fundamental-lemma-3}}%
It remains to be shown that the
product functionals
on~$\scrT$ form a faithful collection.
These are---as we've just seen---all
of the form
