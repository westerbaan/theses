...
d^*(\scrA)_1 d\to\scrA$
is ultraweakly continuous by~\sref{div-usc},
as is~$f$ by~\sref{p-uwcont}
(also) as map from~$(\scrB)_1$ to~$d^*(\scrA)_1 d$,
so that~$g$ is ultraweakly continuous on~$(\scrB)_1$,
...
and so~$\ceil{f(\ceill{a})}\ceil{f(\ceilr{b})}=0$ (by~\sref{gardner-3}).
Now,
since $\ceill{f(a)}\leq \ceil{f(\ceill{a})}$
	and $\ceilr{f(b)}\leq \ceil{f(\ceilr{b})}$
	by~\sref{ncp-ceill},
...
of a von Neumann algebra~$\scrA$.
Recall from~\sref{commutative-cstar-basic}(5)
that~$p\wedge q$ is defined when~$p$ and~$q$ commute.
\begin{enumerate}
\item
...
$\ceil{p}=\ceil{q}$.
\item[2a.]
Assuming that $\ceil{p}=\ceil{q}=1$
and that $p^2\leq Bq^2$ for some $B\in [0,\infty)$,
...
\item
Show that when~$\ceil{p}=\ceil{q}=1$,
$p$ and~$q$ commute,
...
\item
Show that when~$\ceil{p}=\ceil{q}=1$
and~$p$ and~$q$ are commuting and pseudoinvertible,
then:
