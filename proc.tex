...
the closely related and essential facts that
...
(Hint: apply Kaplanksy's density theorem
to the norm closure~$\scrS$ of~$\gamma_\odot(\scrA\odot\scrB)$
being  a $C^*$-subalgebra of~$\scrT$
that is ultraweakly and thus ultrastrongly dense in~$\scrT$
by~\sref{ultraclosed}.)
...
(We'll  in~\sref{} that any 
bounded ultraweakly continuous functional
on~$\scrA\odot\scrB$ can be extended uniquely
to a normal functional on~$\scrT$.)
\end{enumerate}
\end{point}
\begin{point}[tensor-bounded-dense]{Exercise}%
To proceed we need the following strengthening of one of the results
just established in~\sref{tensor-basic}:
show that given a tensor product
$\gamma\colon \scrA\times\scrB\to\scrT$
of von Neumann algebras,
and
$\varepsilon>0$,
each element~$t$ in~$\scrT$
can be obtained
as the ultrastrong limit
of a net~$(s_\alpha)_\alpha$
from $\gamma_\odot(\scrA\odot\scrB)$
with~$\sup_\alpha\|s_\alpha\|\leq \|t\|(1+\varepsilon)$.
\end{point}
...
\begin{point}{Proof}%
Since the uniqueness of~$\beta_\gamma$ is rather obvious,
we only concern ourselves with its existence.
Let~$t\in\scrT$ be given
in order to define~$\beta_\gamma(t)$.
Note that by~\sref{tensor-bounded-dense} 
there is a net~$(s_\alpha)_\alpha$
in~$\scrA\odot\scrB$
such that~$\gamma_\odot(s_\alpha)$
converges ultrastrongly (and so ultraweakly too)
to~$t$,
and~$\|\gamma_\odot(s_\alpha)\|\leq2\|t\|$
for all~$\alpha$.
Now,
(using here that
$\gamma_\odot$ is an isometry and a ultraweak
topological embedding, \sref{tensor-basic})
the net~$(s_\alpha)_\alpha$
is bounded an ultraweakly Cauchy,
and so
the net
$(\beta_\odot(s_\alpha))_\alpha$
in~$\scrC$
is bounded and ultraweakly Cauchy,
and thus converges (by~\sref{vn-complete})
to some element
$\uwlim_\alpha \beta_\odot(s_\alpha)$
of~$\scrC$.

\begin{point}%
Of course we'd like to define $\beta_\gamma(t):=\uwlim_\alpha
\beta_\odot(s_\alpha)$,
but must first check
that $\uwlim_\alpha \beta_\odot(s_\alpha')
=\uwlim_\alpha \beta_\odot(s_\alpha)$
...
although matters can always be arranged this way
without loss of generality.
Then as the difference $\gamma_\odot(s_\alpha)-\gamma_\odot(s_\alpha')$
converges ultraweakly to~$0$ in~$\scrT$ as~$\alpha\to\infty$,
so will $s_\alpha-s_\alpha'$ converge to~$0$ in~$\scrA\odot\scrB$
as~$\alpha\to\infty$
(because~$\gamma_\odot$ is an ultraweak topological embedding),
and thus $\beta_\odot(s_\alpha)-\beta_\odot(s_\alpha')$
converges to~$0$ as~$\alpha\to\infty$
in~$\scrC$,
implying that $\uwlim_\alpha\beta_\odot(s_\alpha)
= \uwlim_\alpha \beta_\odot(s_\alpha')$.
\end{point}
\begin{point}%
In this way
we obtain a map $\beta_\gamma\colon \scrT\to\scrC$
--- which is clearly linear.
We also easily get~$\beta_\gamma\circ \gamma=\beta$,
and as a result~$\beta_\gamma\circ \gamma_\odot = \beta_\odot$.
The map~$\beta_\gamma$ is also bounded,
because since~$\|s_\alpha\|\equiv \|\gamma_\odot(s_\alpha)\|\leq 2\|t\|$
for all~$\alpha$,
where $(s_\alpha)_\alpha$ and~$t$ are as before,
we have $\|\beta_\odot (s_\alpha)\|\leq 2\|\beta_\odot\|\|t\|$
for all~$\alpha$,
and so~$\|\beta_\gamma(t)\|=\|\uwlim_\alpha \beta_\odot(s_\alpha)\|
\leq 2\|\beta_\odot\|\|t\|$.
That, finally, $\beta_\gamma$ is ultraweakly continuous
follows by a standard but abstract argument from the fact
that $\beta_\odot$ is \emph{uniformly} ultraweakly continuous.
We'll give a concrete version of this argument here.
To begin it suffices to show that~$\omega\circ \beta_\gamma$
is ultraweakly continuous
at~$0$
where~$\omega\colon \scrC\to\C$
is an np-functional.
Let~$\varepsilon>0$ be given.
Since~$\beta_\odot$ 
is ultraweakly continuous,
and 
thus $\omega\circ \beta_\odot$ too,
there is~$\delta>0$ and a functional~$\nu\colon \scrA\odot\scrB\to\C$
which is the operator-norm limit
of simple functionals
such that $\left|\nu(s)\right|\leq \delta \implies 
\left|\omega(\beta_\odot(s))\right|\leq \varepsilon$
for all~$s\in\scrA\odot\scrB$.
We claim that $\left|\tilde \nu(t)\right|\leq \delta/2 \implies
\left|\omega(\beta_\gamma(t))\right|\leq 2\varepsilon$
for all~$t\in\scrT$,
where~$\tilde\nu$ is the unique extension of~$\nu$
to a functional on~$\scrT$ along~$\gamma_\odot$ (see~\sref{tensor-basic}) ---
which implies, of course,
that $\omega\circ \beta_\gamma$ is ultraweakly continuous on~$0$.
So let~$t\in\scrT$ with $\left|\tilde\nu(t)\right|\leq \delta/2$
be given.
Pick (as before) a bounded net~$(s_\alpha)_\alpha$
in~$\scrA\odot\scrB$
such that $\gamma_\odot(s_\alpha)$
converges to~$t$ as~$\alpha\to\infty$, and observe that,
for all~$\alpha$,
\begin{equation*}
\left|\omega(\beta_\gamma(t))\right|
\ \leq\ 
\left|\omega(\beta_\gamma(t)-\beta_\odot(s_\alpha))\right|
\,+\,\left|\omega(\beta_\odot(s_\alpha))\right|.
\end{equation*}
The first term on the right-hand side above will 
vanish as~$\alpha\to\infty$ (since 
$\beta_\gamma (t)=\uwlim_\alpha \beta_\odot(s_\alpha)$),
and will thus be smaller than~$\varepsilon$ 
for sufficiently large~$\alpha$.
Since~$\lim_\alpha \left|\nu(s_\alpha)\right|=\left|\tilde\nu(t)\right|
\leq \delta/2<\delta$
we see 
that for sufficiently large~$\alpha$
we'll have~$\left|\nu(s_\alpha)\right|\leq \delta$ 
and with it~$\left|\omega(\beta_\odot(s))\right|\leq \varepsilon$.
Combined,
we get $\left|\omega(\beta_\gamma(t))\right|\leq 2\varepsilon$,
and so~$\beta_\gamma$ is ultraweakly continuous.\qed
\end{point}
\end{point}
\end{point}
\end{parsec}%
...
\begin{enumerate}
\item
$\|\beta_\gamma\|=\|\beta_\odot\|$
--- perhaps by  adapting the proof in~\sref{tensor-universal-property-1}
that~$\beta_\gamma$ is bounded
by choosing $(s_\alpha)_\alpha$
with  $\|\gamma_\odot(s_\alpha)\|\leq (1+\varepsilon)\|t\|$
instead of $\|\gamma_\odot(s_\alpha)\|\leq 2\|t\|$;
\item
$\beta_\gamma$
...
$\beta$ is completely positive.
\end{enumerate}
\TODO{ Add lemma after Kaplansky's density theorem
on a dense $*$-subalgebra of a von Neumann algebra. }
\end{point}
\begin{point}{Proposition}%
...
is bounded, normal and completely positive;
...
To begin,
note that~$\beta$ is completely positive,
by~\TODO{...},
because for each~$N$
the bilinear map $M_N\beta = (M_N\otimes)\circ (M_Nf\,\times\,M_Ng)$
sends a pair of positive matrices
to a positive matrix
(using here that~$\otimes$
is miu-bilinear and thus compeletely positive, by~\TODO{...}).
...
of the tensor product norm, see~\sref{tensor-extra}) 
to show that
...
= \sum_{ij} \sigma(c_i^* f(\,\cdot\,)c_j)\,\odot\,
\tau(d_i^* g(\,\cdot\,)d_j),
\end{equation*}
...
thus obtained is nowhere near to~$\|f\|\|g\|$,
it does allow
...
see~\TODO{...})
its norm is by~\TODO{...} given by~$\|\omega'\|=\omega'(1)\equiv
