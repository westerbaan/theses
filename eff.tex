...
The right pullback square of \eqref{pullbacks}
    ...
    from a partial map~$f$ in~$\Par C$ that is total (i.e.~$1 \after f = 1$).
\item
The joint-monicity
...
\end{point}
\begin{point}{Exercise}%
Show that if we have any pullback square in some category --- say
\begin{equation*}
...
\begin{equation*}
    ...
    \xymatrix{
        X+A \pullback \ar@^{>}[r]^{\id+\hat{f}} \ar@^{>}[d]_{\pproj_2}
        & X+B \ar@^{>}[d]^{\pproj_2}
        \\ A \ar@^{>}[r]_{\hat{f}}
        & B
    }
