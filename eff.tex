...
\end{point}
\begin{point}{Definition}%
An \Define{effect monoid}~$M$
is a categorical monoid in~$\mathsf{EA}$ \cite{probdistrconv}
    --- concretely:
    $M$ is an effect algebra
...
\end{enumerate}
An effect monoid~$M$ is \Define{commutative}
    provided~$a\odot b = b\odot a$ for all~$a,b \in M$.
\end{point}
\begin{point}{Examples}%
...
\begin{enumerate}
\item
    $C$ is a finPAC \cite{kentapartial} (cf.~\cite{arbib}) --- that is: if
    \begin{enumerate}
        \item 
    ...
    \begin{enumerate}
        \item for each object~$X$ the PCM~$C(X,I)$
                is an effect algebra \cite{ea} --- that is:
        \begin{enumerate}
            \item
...
\TODO{}
\begin{point}%
We will first prove that~$\Par C$ has finite coproducts.
It is sufficient to show~$\Par C$ has an initial object~$0$
    and binary coproducts.
It is easy to the initial object of~$C$ is
    also the initial object of~$\Par C$.
Let~$\kappa_1 \colon X \to X+Y \leftarrow Y \colon \kappa_2$
be a coproduct in~$C$.
We will show~$X + Y$ is a
    a coproduct in~$\Par C$ with coprojections~$\kappa_1 \colon X \to X+Y+1$
    and~$\kappa_2 \colon Y \to X+Y+1$.
Let~$f\colon X \to Z+1$ and~$g\colon Y \to Z+1$ be given.
Then 
\end{point}
\end{point}
\end{point}
