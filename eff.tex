...
\end{point}
\begin{point}%
We will prove the necessity in \sref{dagger-thm-necessity}
    and sufficiency in \TODO{}.
...
Predicates in a~$\dagger$-effectus have a square root:
    for every predicate~$p$,
    there is a predicate~$q$
    with~$\andthen{q}{q}=p$.
\begin{point}{Proof}%
...
\end{equation*}
which shows~$p$ has as square root~$p$.  \qed
\end{point}
\end{point}
...
\begin{parsec}%
\begin{point}{Proposition}%
In a~$\dagger'$-effectus, we have~$(f \after g)^\dagger = g^\dagger \after f^\dagger$
    for pure~$f,g$.
\begin{point}{Proof}%
For brevity, write~$p = 1 \after f$, $q = 1 \after g$,
    $s = \IM f$ and $t = \IM g$.
Let~$\varphi$, $\psi$, $\chi$, $\alpha$, $\beta$ and $\gamma$
    be the unique isomorphisms such that respectively
\begin{align*}
    ...
    \zeta_{\ceil{p}} \after \asrt_p \after \pi_t
        & \ = \ \pi_{\lceil t \after \asrt_p \after \pi_{\ceil{p}}\rceil} \after \chi \\
    \pi_s \after \varphi \after \pi_{\lceil t \after \asrt_p \after \pi_{\ceil{p}} \rceil}
    &\ = \ \pi_{\lceil t\after \asrt_p\after\pi_{\ceil{p}}\rceil \after \varphi^{-1}\after \zeta_s }
        \after \alpha \\
        \zeta_{\ceil{p \after \pi_t}} \after \psi \after \zeta_{\ceil{q}}
           & \  = \ \beta \after \zeta_{\lceil p \after \pi_t \rceil \after \psi \after \zeta_{\ceil{q}}} \\
    \asrt_{\ceil{p \after \pi_t} \after \psi \after \zeta_{\ceil{q}}} \after \asrt_q
    & \ = \ \gamma \after \asrt_{p \after g}
\end{align*}
\begin{alignat*}{2}
g^\dagger \after f^\dagger
& = 
    \asrt_q \after \pi_{\ceil{q}} \after \psi^{-1} \after \zeta_{t} \after 
    \asrt_p \after \pi_{\ceil{p}} \after \varphi^{-1} \after \zeta_{s} \\
\end{alignat*}
\end{point}
\end{point}
\end{parsec}
