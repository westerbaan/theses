...
Some other examples appear later on in
    \sref{emod-effectus}, \sref{exc-dm-effectus} and
    \sref{aconvm-is-effectus}.
...
\end{point}
\end{point}
\begin{point}{80}{Exercise}%
    Show that the category~$\op{\mathsf{Rng}}$ of unit rings
...
\begin{enumerate}
    \item 
        ...
        Conclude~$\op{\mathsf{Rng}}$ does not have separating predicates.
    \item
        Show that there is no unit-preserving
            ring-homomorphism~$\Z_2 \to \Z$.
            Conclude~$\op{\mathsf{Rng}}$ does not have separating states.
\end{enumerate}
\end{point}
\end{parsec}
\begin{parsec}{1920}%
...
form except perphaps for the left pullback square of~\eqref{pullbacks}.
\begin{point}{20}{Proof}%
...
\end{point}
\end{point}
\begin{point}{70}[diamond-equiv-equiv]{Exercise}%
...
Let~$f\colon X \to Y$ be any map in an effectus.
    ...
    such that
\begin{quote}
    for every (other) triple~$(P', \varrho', h')$
