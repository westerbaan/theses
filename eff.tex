...
A subset~$D \subseteq E$ is a \Define{sub-effect algebra} of~$E$
...
\end{enumerate}    
\end{multicols}
An ortholattice is \Define{orthomodular}
 provided
\begin{equation*}
...
The \texttt{assert} statement checks whether
    ...
    is true --- if it is, it will continue the program
...
In an~$\&$-effectus, a map~$f$ is \Define{pristine}
...
\end{quote}
We say an effectus \Define{has dilations} if
...
The key point is that it is easier to compute and understand
    these algebraic invariants, then it is to work with the spaces
    themselves.
One normally thinks of a category of spaces as being quite different
    ...
    that~$\op\vN$, which is a category of (non-commutative) spaces,
