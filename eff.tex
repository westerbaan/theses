...
Suppose~$E$ is an effect algebra.
If the supremum~$a \vee b$
    exists for some~$a,b \in E$ with~$a \perp b$,
    then their infimum~$a \wedge b$ exists as well and
\begin{equation*}
...
The result follows from this lemma:
    if~$(x \ominus c) \vee (x \ominus d)$ exists,
    then~$c \wedge d$ exists as well
    and~$(x \ominus c) \vee (x \ominus d)
        = x \ominus (c \wedge d)$.
Indeed:
\begin{equation*}
    a \vee b \ = \ 
        ((a \ovee b) \ominus a) \vee ((a \ovee b) \ominus b) \ = \ 
        (a \ovee b) \ominus (a \wedge b)
\end{equation*}
    and so~$(a \vee b) \ovee (a \wedge b) = a \ovee b$, as desired.
\begin{point}{30}%
Now, we prove the lemma.
Assume~$(x \ominus c) \vee (x \ominus d)$ exists.
Note~$x \ominus c = (x^\perp \ovee c)^\perp$.
As~$z \mapsto z^\perp$ is an order anti-isomorphism,
    we see~$(x^\perp \ovee c) \wedge (x^\perp \ovee d)$
        exists and~$(x^\perp \ovee c) \wedge (x^\perp \ovee d)
                                    = ((x \ominus c)\vee (x \ominus d))^\perp$.
Clearly~$ (x^\perp \ovee c) \wedge
        (x^\perp \ovee d) \geq x^\perp$.
    Write~$r := ((x^\perp \ovee c) \wedge
        (x^\perp \ovee d)) \ominus x^\perp$.
We will show~$r = c \wedge d$.
It is easy to see~$r \leq c$ and~$r \leq d$.
Assume~$s$ is any element with~$s \leq c$ and~$s \leq d$.
Note~$x^\perp \ovee s \leq x^\perp \ovee c$
and~$x^\perp \ovee s \leq x^\perp \ovee d$,
    hence~$x^\perp \ovee s \leq (x^\perp \ovee c) \wedge (x^\perp \ovee d)$
    and so~$ s \leq  r$.
We have shown~$r = c \wedge d$.
Consequently~$ (x \ominus c)\vee (x \ominus d)
        = ((x^\perp \ovee c) \wedge (x^\perp \ovee d))^\perp
        = (r \ovee x^\perp)^\perp = 
            x \ominus (c \wedge d) $, as promised. \qed
\end{point}    
\end{point}
\end{point}
...
As~$C(I,I)$ is an effect algebra~$1 = \id \ovee \id^\perp$
    for some~$\id^\perp$.
    So by $\mathsf{PCM}$-enrichment~$1 \after 1 = (1 \after \id) \ovee (1 \after\id^\perp) = 
...
\end{point}
\end{point}
\begin{point}{50}[eff-aconv-exa]{Examples}%
...
What about initial object?
Let's first consider the corner-case~$M=1$.
Each non-empty set has a unique formal $1$-convex combination,
    and thus each abstract~$1$-convex set~$X$ must be a singleton
        as~$x = h(\ket{x}) = h(\ket{y}) = y$ for any~$x,y\in X$.
    This makes any of them initial.
For the other case, assume~$M \neq 1$.
Then~$\mathcal{D}_M \emptyset = \emptyset$.
...
\end{point}
\begin{point}{70}[eff-divisoid-add]{Proposition}%
If $a \perp b$ and~$a \ovee b \leq c$ in an effect divisoid,
then~$\rfrac{a \ovee b}{c } = \rfrac{a}{c} \ovee \rfrac{b}{c}$.
\begin{point}{80}{Proof}%
It is sufficient to prove for~$c=a\ovee b$, as
\begin{equation*}
\rfrac{a \ovee b}{c}
    = \rfrac{a \ovee b}{c} \odot \rfrac{a \ovee b}{a \ovee b}
    = \rfrac{a \ovee b}{c} \odot
        \left( \rfrac{a}{a \ovee b} \ovee \rfrac{b}{a \ovee b}  \right)
        = \rfrac{a}{c} \ovee \rfrac{b}{c}.
\end{equation*}
By definition~$\rfrac{a}{a \ovee b} \leq \rfrac{a\ovee b}{a \ovee b}$.
Define~$w \equiv \rfrac{a \ovee b}{a \ovee b} \ominus \rfrac{a}{a \ovee b}$.
Note~$(a\ovee b) \odot w = (a \ovee b) \ominus a = b$.
Thus~$ w = \rfrac{b}{a \ovee b}$
    and hence~$
    \rfrac{a \ovee b}{a\ovee b} 
        = \rfrac{a}{a\ovee b} \ovee w
        = \rfrac{a}{a\ovee b} \ovee \rfrac{b}{a \ovee b}$ as desired.
\qed
\end{point}
\end{point}
\end{parsec}
...
\end{point}
\spacingfix{}
\begin{point}{30}[eff-dagger-conc-ex]{Example}%
...
\item
    A diagram~$A \xrightarrow{f} B \xrightarrow{g} C$
        is exact at $B$
        if and only if~$\IMperp f = \ceil{1 \after g}$.
\item
