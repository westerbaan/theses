...
We say~$(X,d)$ is an \Define{abstract $M$-convex set}
    provided~$h(\ket{x})=x$
    and
\begin{equation*}
    h \Bigl( \bigovee_{i,j} \sigma_i \odot \lambda_{i,j} \ket{x_{ij}} \Bigr)
    \ = \ 
...
We say that a map~$f\colon X \to Y$
    between abstract~$M$-convex sets~$(X,h_X)$ and~$(Y,h_Y)$
...
(Equivalently: $\AConvM$
    ...
    of the~$\mathcal{D}_M$.)
We call a abstract $M$-convex set~$(X,h)$ \Define{cancellative}
provided~$
    h(\lambda \ket{x_1} \ovee \lambda^\perp \ket{y}) =
    h(\lambda \ket{x_2} \ovee \lambda^\perp \ket{y})$
    implies~$x_1=x_2$
    for any~$x_1,x_2,y \in X$ and~$\lambda \in M, \lambda \neq 0$.
\end{point}
\begin{point}{Examples}%
We give examples of abstract convex sets.
\begin{enumerate}
\item
...
This equivalence relation~$\sim$
    is a congruence for the convex structure on~$T$
    (inherited by~$\R^2$)
...
Furthermore, every semilattice~$(L, \vee)$
    ...
    with
    \begin{equation*}
    h(\lambda_1 \ket{x_1} \ovee \cdots \ovee \lambda_n \ket{x_n})
        \ =\  \bigvee_{i; \lambda_i \neq 0} x_i.
    \end{equation*}
