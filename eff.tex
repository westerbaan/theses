...
\end{equation*}
\begin{point}[modularity-lemma-proof]{Proof}%
...
\end{point}
\end{point}
\begin{point}[dfn-orthomodular-lattice]{Definition}%
...
\end{enumerate}    
\end{multicols}
    An ortholattice is \Define{orthomodular}\index{orthomodular!lattice}
 provided
\begin{equation*}
...
\end{point}
\begin{point}[orth-ea-is-orthomodular]{Proposition}%
An effect algebra~$E$
...
\begin{parsec}%
\begin{point}[dfn-effectus]{Definition}%
...
\item\label{eff-joint-monicity} and the following two arrows are jointly monic.
    ...
    \end{equation*}
\end{enumerate}
An arrow~$X \to Y+1$ is called a \Define{partial map} and written
    \index{partial map (in an effectus)}
    $\Define{X \pto Y}$.
    \index{*pto@$X\pto Y$}
\begin{point}%
...
\begin{point}{$\mathsf{PCM}$-enrichment, I}%
...
\end{point}
\begin{point}{$\mathsf{PCM}$-enrichment, II}%
...
\begin{point}{Finite products and initial object}%
For every~$M$-effect module~$E$,
    there is a unique $M$-effect modulemap~$M \to E$,
...
A \Define{formal $M$-convex combination} over~$X$
    \index{convex@(formal) $M$-convex combination}
...
\end{point}
\end{point}
\begin{point}[quot-fact-system]{Exercise}
...
\end{point}
\begin{point}[dfn-eff-grothendieck]{Definition}%
For an effectus~$C$,
...
\section{Effectus with comprehension and images}
\begin{parsec}%
\begin{point}%
We continue with our second axiom.
\end{point}
\begin{point}[dfn-comprehension]{Definition}%
Let~$C$ be an effectus.
...
\end{point}
\begin{point}[compr-grothendieck]{Exercise*}%
...
\item
    We say a predicate~$p$ is (image) \Define{sharp} \index{sharp}
        ...
        \index{SPred@$\SPred$}
\item
    We define~$\Define{\floor{p}} = \IM \pi_p$, \index{*floor@$\floor{\ }$}
...
\end{point}
\begin{point}[img-tupling]{Exercise}%
...
\section{\texorpdfstring{$\diamond$-effectus}{%
                    diamond-effectus}}
\begin{parsec}%
\begin{point}%
...
\end{point}
\end{point}
\begin{point}[diamond-oml]{Proposition (Cho)}%
In a~$\diamond$-effectus,
...
\end{point}
\end{point}
\begin{point}[spred-infimum]{Lemma}%
...
we have
    ...
    for sharp~$s,t$ and  quotient~$\xi$ of~$s^\perp$.
\end{point}
\end{parsec}
...
Show that in a~$\&$-effectus,
    ...
    (Hint: use \sref{sum-sharp-sharp} and \sref{img-tupling}.)
\end{point}
\end{parsec}

\section{\texorpdfstring{$\dagger$-effectus}{%
                    dagger-effectus}}
\begin{parsec}%
\begin{point}{Definition}%
A \Define{$\dagger$-category} (``dagger category'')
    \index{*dagg@$\dagger$!-category}
...
\end{point}
\begin{point}{Example}%
By \sref{vn-is-dagger-category},
the category~$\op\vN$ is a~$\dagger$-effectus.
\end{point}
...
\end{point}
\begin{point}[vn-is-dagger-category]{Corollary}%
...
In a~$\dagger'$-effectus, for a pure map~$f$, define
    \index{*dagg@$\dagger$!in a $\dagger'$-effectus}
\begin{equation*}
    \Define{f^\dagger}
        \ =\  \asrt_{1 \after f} \after
    \pi_{\ceil{1\after f}} \after
...
\begin{parsec}%
\begin{point}%
As they are the the main topic of the first part of this thesis,
...
The effectus $\op\vN$ has dilations,
    ...
    does not.
\end{point}
\begin{point}[dils-abstract-basics]{Proposition}%
...
\begin{point}%
    To finish the discussion of dilations,
        we mention how to state \sref{paschke-correspondence}
    in an~$\&$-effectus.
\end{point}
\begin{point}{Definition}%
In an~$\&$-effectus,
