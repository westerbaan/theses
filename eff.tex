...
if there is a tuple~$\varphi_1, \ldots, \varphi_n \in \mathcal{D}_M X$
...
Trivially, the one-element set~$1$ is an abstract~$M$-convex set
    and the final object in~$\AConvM$.
As~$\mathcal{D}_M \emptyset = \emptyset$,
    the empty set is also an abstract~$M$-convex set
...
we have
\begin{equation*}
    \alpha(z) = \lambda_0 \ket{\kappa_1(\cdot)}
        \ovee \bigovee_i \lambda_i \ket{\kappa_2(y_i)}
        \qquad
    \beta(z) = \sigma_0 \ket{\kappa_2(\cdot)}
        \ovee \bigovee_i \sigma_i \ket{\kappa_1(x_i)}
\end{equation*}
