...
\end{point}
\end{point}
\begin{point}[factor-compr]{Proposition}%
...
In any homological category,
    ...
    \cite[\S3.4]{grandis}.
The following is a translation of Grandis' Snake Lemma
    into~$\dagger$-effectus jargon.
\end{point}
\end{point}
\begin{point}{Theorem}%
Assume we are given the following commuting
    diagram in a~$\dagger$-effectus.
\begin{equation*}
    \xymatrix{
        &A \ar[d]^{a} \ar[r]^f
    &B \ar[d]^{b} \ar[r]^g
...
\end{equation*}
Furthermore, assume
    $b$ is left-modular over~$\IM f$;
    $b$ is right-modular over~$\IM h$
    and the diagram has exact rows
    --- that is:
    \begin{multicols}{2}
    \begin{enumerate}
        \item
...
\end{align*}
Then: the map~$d \equiv a_\zeta \after h^\dagger \after b
                    \after g^\dagger \after c_\pi$
        turns
\begin{equation*}
    \xymatrix{
