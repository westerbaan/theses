...
\begin{point}[emod-effectus]{Theorem}%
    The category~$\EMod^{\mathsf{op}}_M$ is an effectus for any effect monoid~$M$.
In fact: every effectus~$C$ (in total form)
    with separating predicates and scalars~$M$
...
The study of
    ...
    as algebraic structures goes back a long time,
        see e.g.~\cite{stone1949postulates,neumann1970quasivariety,flood1981semiconvex,tz2009convex}.
...
\begin{parsec}%
\begin{point}%
Our next project is to investigate whether~$\AConvM$ is an effectus.
We can show~$\AConvM$ is an effectus, if~$M$ has a certain partial division.
Unfortunately it is unknown whether~$\AConvM$ is an effectus in general.
The case~$M=[0,1]$ is much easier and has already been shown in
...
Assume~$\kappa_1(x) = \kappa_1(x')$
    for some~$x,x' \in X$.
    Let~$\Phi_1, \ldots, \Phi_l \in \mathcal{D}_M^2{(X+Y)}$
    be a derivation
    of~$\kappa_1(x) = \kappa_1(x')$
    as in \sref{elements-coprod-conv}.
By induction, we see that for each~$1 \leq i < l$,
    we have~$\mu(\Phi_i) \in (\mathcal{D}_M \kappa_1)(X)$
    and~$h_X (\mu(\Phi_i)) = x$.
\end{point}
\begin{point}%
We continue with the hardest part:
    the proof that the square on the left of \eqref{pullbacks}
    is a pullback in~$\AConvM$.
\begin{equation*}
\xymatrix{
    Z \ar@/^1pc/[rrd]^\beta
...
\begin{equation*}
    ...
    \bigovee^n_{i=1} \lambda_i \ket{\kappa_2(y_i)}
        \ovee \bigovee^m_{i=1} \sigma_i \ket{\kappa_1(x_i)}\Bigr),
\end{equation*}
where~$h_2$ is the convex structure on~$X+Y$.
...
\end{align*}
and for each~$1 \leq i < l$
either
\begin{enumerate}
\item
$\mu(\Phi_i) = \mu(\Phi_{i+1})$ or
\item
for each~$1 \leq j < l$
we have
$\zeta_{ij} = \zeta_{(i+1)j}$
and either
\begin{enumerate}
\item
    $\varphi_{ij} = \varphi_{(i+1)j}$ or
\item
    there are~$\chi_1,\chi_2 \in \mathcal{D}_M Y$
with~$h_{Y} (\chi_1) = h_{Y} (\chi_2)$,
$\varphi_{ij} = (\mathcal{D}_M \kappa_2) (\chi_1)$ and
$\varphi_{(i+1)j} = (\mathcal{D}_M \kappa_2) (\chi_2)$;
\end{enumerate}
\end{enumerate}
\emph{and} the analogous condition for the~$\Psi_i$.
...
\end{align*}
where~$\omega_{ij} \in \mathcal{D}_M(X+Y)$ are defined as follows.
\begin{align*}
    \omega_{ij} (\kappa_2 (y)) &\ =\ \varphi_{ij}(\kappa_2(y))  &
    \omega_{ij} (\kappa_1(x)) &\ =\ \psi_{11}(\kappa_1(x)) 
&& \text{for $1 \leq i \leq l$}
    \\
    \omega_{(i+l)j} (\kappa_2 (y)) &\ =\ \varphi_{l1}(\kappa_2(y))  &
    \omega_{(i+l)j} (\kappa_1(x)) &\ =\ \psi_{ij}(\kappa_1(x))
&& \text{for $1 \leq i \leq k$}
\end{align*}
We want to show~$\Omega_i$
    is a derivation in the sense of \sref{least-conv-cong}.
It is easy to see that if~$\mu(\Phi_i) = \mu(\Phi_{i+1})$,
    then~$\mu(\Omega_i) = \mu(\Omega_{i+1})$
    (and the analogous statement for~$\Psi_i$).
For the other case,
    assume~$\zeta_{ij} = \zeta_{(i+1)j}$
    and~$\varphi_{ij} \mathrel{R} \varphi_{(i+1)j}$
    for all~$j$.
\end{point}
\end{point}
\end{parsec}
