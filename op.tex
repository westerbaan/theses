...
\begin{parsec}%
\begin{point}{Lemma}%
Let~$D$ be a bounded directed subset of self-adjoint
elements of a von Neumann algebra~$\scrA$,
and let~$a\in \scrA$.
Then~$a^*d$ converges ultraweakly to $a^*\bigvee D$.
\end{point}
\begin{point}{Proof}%
Let~$\omega\colon \scrA\to \C$ be a normal positive linear map,
and let~$d \in D$.
Then by~\TODO{} we have,
writing $s:=\bigvee D$,
\begin{alignat*}{3}
|\,\omega(\, a^*s\,-\,a^*d\,)\, |^2
\ &=\  |\,\omega(\ (\,\sqrt{s-d}\,a\,)^*\ \sqrt{s-d}\ )\,|^2\\
\ &\leq\ 
\omega(\,a^*\,(s-d)\,a\,)\ \omega(s-d) \\
\ &\leq\ 
2(\sup_{d'\in D}\|d'
\|)\ 
\omega(\,a^*a\,)\ 
\omega(s-d).
\end{alignat*}
Thus~$\omega(a^*s-a^*d)$ converges to~$0$.
Hence~$a^*d$ converges ultraweakly to~$a^*s$.
\end{point}
\end{parsec}
%
%  ad is normal
...
to~$a^* \,(\bigvee D)\,a$,
because $(\lambda+a)^*\,d\,(\lambda+a)$
converges ultraweakly to $(\lambda+a)^*\,(\bigvee D)\,(\lambda+a)$
by~\sref{ad-normal-1} and~\TODO{},
and $a^*d+da$ converges ultraweakly to $a^*(\bigvee D)+(\bigvee D)a$
by~\sref{}.
Hence~$\bigvee_{d\in D} a^* \,d\, a = a^*\,(\bigvee D)\,a$
by~\sref{}.
\end{point}
\end{point}
\end{parsec}
