...
\begin{parsec}%
\begin{point}%
\TODO{Remark on Gelfand--Mazur not useful}
...
For each~$a\in\scrA$ we have $\spec(a)=\{f(a)\colon f\in\spec(\scrA)\}$.
\begin{point}{Proof}%
Let~$f\in \spec(\scrA)$,
and~$a\in\scrA$.
...
\TODO{fix error: $a$ must be self-adjoint}
...
for each~$a\in\scrA$
using~\sref{spectrum-miu} and~\sref{norm-spectrum}.
...
\begin{point}{Proof}%
We are going to use the fact that each
net in~$X$ has a convergent subnet
to obtain the point~$x$.
Let~$D$ be the set of $f\in C(X)$
with~$\tau(f)=1$ and $0\leq f\leq 1$
Then~$D$ is downwards directed,
because if~$f,g\in D$,
then $\tau(f g)=\tau(f)\,\tau(g)=1$,
and $0\leq fg\leq f\wedge g$.
Since clearly $0\notin D$,
we can choose for each~$f\in D$
a point $x_f\in X$ with~$f(x_f)>0$.
Now, since~$X$ is compact,
the net~$(x_f)_{f\in D}$
has a subnet which converges to some~$x\in X$.
