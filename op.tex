...
we turn to the multiplicative states
on a commutative $C^*$-algebra~$\scrA$.
To obtain such multiplicative states
we consider a special type
of order ideal.

\TODO{Show that a $C^*$-algebra need not have any
multiplicative states. 
Make remark on extreme states.}
\end{point}
\begin{point}{Definition}%
...
Let~$I$ be a Riesz ideal of a commutative $C^*$-algebra~$\scrA$.
...
by~\TODO{multiplication by a commuting positive operator is monotone},
and so~$ax\in I$,
...
\begin{enumerate}
\item
Let~$a$ be a self-adjoint element
of~$\scrA$.
Show that
\begin{equation*}
...
\,\leq\, n\left|a\right| \ ]\,\}
\end{equation*}
is the least Riesz ideal that contains~$a$.

Show that $(a)_m=\scrA$ iff $a$ is invertible.

Show that~$(a)=(a)_m$ when~$a\geq 0$.

Give an example of~$\scrA$ and self-adjoint~$a$ with~$(a)\neq (a)_m$.
\item
Show that~$I+J$ is a Riesz ideal of~$\scrA$
when $I$ and~$J$ are Riesz ideals. (Hint: use~\sref{riesz-decomposition-lemma}.)

Give an example of~$\scrA$ and order ideals~$I$ and~$J$ of~$\scrA$
such that $I+J$ is not an order ideal.

\item
Show that each proper Riesz ideal is contained
in a maximal Riesz ideal.
\end{enumerate}
\end{point}
\begin{point}[maximal-riesz-ideal-maximal-order-ideal]{Lemma}%
A maximal Riesz ideal~$I$ of a commutative $C^*$-algebra $\scrA$
...
\end{point}
\begin{point}{Proposition}
Let~$I$ be a maximal Riesz ideal of a commutative $C^*$-algebra.
...
\end{point}
\begin{point}{Definition}%
The \Define{spectrum} of a commutative $C^*$-algebra~$\scrA$,
denoted by~\Define{$\spec(a)$},
is the set of miu-maps $f\colon \scrA\to\C$.

We endow~$\spec(\scrA)$
with the topology of pointwise convergence
(that is, the topology $\spec(\scrA)$ inherits
being a subset of the product space $ \prod_{a\in\scrA} \C$).
\end{point}
\begin{point}{Exercise}%
For an element~$a$ of a commutative $C^*$-algebra
we have
$\spec(a) = \{f(a)\colon f\in \spec(\scrA)\}$.
\end{point}
\end{parsec}
