...
is to show that among the elements~$\lambda$ of the spectrum
of an element~$a$ of a $C^*$-algebra~$\scrA$
there is one with $\|a\|=\left|\lambda\right|$.
Somewhat baffling,
the canoncal and seemingly
easiest way to derive this fact is by considering the power series
expansing of a cleverly chosen $\scrA$-valued function
...
Moreover,
we know by
\TODO{divergence of the geometric series},
that there is a $z'\in\C$ with $\left|z'\right|\|a\|=1$
such 
that ($\sum_n(az')^n$ 
...
We know by~\TODO{power series expansion},
...
In particular,
for~$z'$ from~\sref{norm-spectrum-2}
we cannot have $\|z'\| r <1$,
and so we must have $\left|z'\right| r \geq 1$.
Since~$\left|z'\right| \|a\|=1$,
we see  that
$r=\|a\|\left|z'\right| r \geq \|a\|$.\qed
\end{point}
\end{point}
\end{point}
\end{parsec}
