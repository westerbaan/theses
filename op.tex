...
\Define{positive} if $\|a-\|a\|\|\leq \|a\|$,
and the set of positive elements of~$\scrA$
is denoted by~$\pos{\scrA}$.
\end{point}
\TODO{move the definition of positive,
and some elementary properties here}
\end{parsec}
...
we turn Gelfand's representation theorem~\sref{gelfand}
to an equivalence between the categories $\op{(\cCstar{miu})}$
and~$\CH$.
...
\end{point}
\begin{point}{Theorem}%
The Gelfand representation\TODO{...}
\end{point}
\end{parsec}
