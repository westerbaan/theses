...
\end{point}
\begin{point}{Definition}%
In this section a \Define{triangle}~$T$
consists of a tuple of three complex numbers~$w_0,w_1,w_2$
called the
\Define{vertices} of~$T$---the order
of the vertices will be important
in a moment.
The \Define{boundary} of
such a triangle~$T$,
written~$\Define{\partial T}$,
is the union of the three line segment
between~$w_0$, $w_1$ and~$w_2$,
and the \Define{closure} of~$T$,
written $\Define{\mathrm{cl}(T)}$,
is the convex hull of~$w_0,w_1,w_2$.
The \Define{interior} of~$T$
is then  simply given by
$\Define{\mathrm{in}(T)}=\mathrm{cl}(T)\backslash \partial T$,
and the \Define{length}
of~$T$ is given by
\begin{equation*}
...
\end{equation*}
Given an $\scrA$-valued
holomorphic function~$f$
with $\mathrm{cl}(T)\subseteq \dom(f)$
the integral of~$f$ along~$T$
is defined by
\begin{equation*}
...
Let~$\scrH$ be a Hilbert space,
and let~$(x_\alpha)_\alpha$ be a net in~$\scrH$.
\begin{enumerate}
\item
