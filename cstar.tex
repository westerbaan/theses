...
as we'll see in~\sref{cp-russo-dye}.
\end{point}
\end{point}
...
(see~\sref{vector-states-order-separating});
\item
on a $C^*$-algebra $\scrA$:
...
\end{point}
\end{point}
\begin{point}[vector-states-order-separating]{Proposition}%
The vector states
of~$\scrB^a(X)$
are order separating (see~\sref{separating})
for every Hilbert $\scrA$-module~$X$.
\begin{point}{Proof}%
Let~$T\in (\scrB^a(X))_+$ be given.
By~\sref{order-separating-norm}
it suffices to show
that~$\|T\|=\sup_{x\in (X)_1} 
...
we have $
\|T\| = \|T^{\nicefrac{1}{2}}\|^2
=(\,\sup_{x\in (X)_1}\left\|T^{\nicefrac{1}{2}}x\right\|\,)^2
=\sup_{x\in (X)_1} \left\|\left<x,Tx\right>\right\|$.\qed
\end{point}
\end{point}
\begin{point}[positive-operators]{Corollary}%
For an adjointable operator~$T$
on a Hilbert $\scrA$-module~$X$, we have
\begin{enumerate}
\item
...
because the vector states on~$\scrB^a(X)$ 
are order separating 
by~\sref{vector-states-order-separating}.\qed
\end{point}
\end{point}
...
\begin{enumerate}
\item
the map $b\mapsto a^*ba\colon \scrA\to\scrA$
is completely positive 
for every~$a\in\scrA$, and
\item
the map $T\mapsto S^* T S\colon \scrB^a(X)
\to\scrB^a(Y)$
is completely positive
...
= \|f(1)\|^2 \|a\|^2$
for all $a\in \scrA$
by~\sref{cp-cs} and~\sref{weak-russo-dye}.\qed
\end{point}
\end{point}
