...
\begin{point}{Remark}%
We will realize later on that the
requirement that a state is involutive
is redundant,
(because
every positive map between $C^*$-algebras
is involutive, see~\sref{p-implies-i}.)
\end{point}
\end{point}
\begin{point}%
...
of which we'll now show two (\sref{p-implies-i} and \sref{astara-positive}).
\end{point}
\begin{point}[p-implies-i]{Exercise}%
	Let~$f\colon \scrA\to\scrB$
	be a p-map between $C^*$-algebras $\scrA$ and~$\scrB$.
	We will see that~$f$ is involutive,
	that is, $f(a^*)=f(a)^*$ for all~$a\in \scrA$.
\begin{enumerate}
	\item
		Show that~$f(a)$ is self-adjoint
		for every self-adjoint $a\in \scrA$
		(by writing $a\equiv a_+ - a_-$ and using that~$f$
		is positive).
	\item
		Deduce 
		that $f(\Real{a})=\Real{f(a)}$
		and $f(\Imag{a}) = \Imag{f(a)}$
		for all~$a\in \scrA$ (see~\sref{cstar-involution-basic}).
	\item
		Conclude that $f(a^*)=f(a)^*$ for all~$a\in\scrA$.
\end{enumerate}
\end{point}
...
for $a,b\in\scrA$,
because $\omega$ is involutive (by~\sref{p-implies-i}).
...
given by the inner product $[\,\cdot\,,\,\cdot\,]_\omega$
(viz.~$\|a\|_\omega = \omega(a^*a)^{\nicefrac{1}{2}}$)
will play a prominent role in this section,
and all throughout the next chapter, 
and so deserves some scrutiny.
\begin{enumerate}
\item
Use Cauchy--Schwarz
...
However,
show that we do not have $\|ab\|_\omega \leq \|a\|_\omega \|b\|_\omega$
