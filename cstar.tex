...
\end{equation*}
In that case, we call~$T$ \Define{adjointable}.
It is not difficult to see that such adjointable~$T$
is adjoint to exactly one~$S$, which we denote by~$\Define{T^*}$.
The set of such adjointable bounded module maps~$T$ is denoted
...
Show that $\|T^*T\|=\|T\|^2$ and $\|T^*\|=\|T\|$
for every adjointable module map~$T$ on a pre-Hilbert $\scrA$-module.
