...
\begin{point}%
	...
	is this:
\end{point}
\begin{point}[hilb-weak-bounded-completeness]{Proposition}%
...
\end{point}
\begin{point}{Remark}%
The condition in~\sref{hilb-weak-bounded-completeness} 
...
As it turns out,
not every p-map
enjoys has this property,
and in fact,
we'll show that if a p-map $\omega\colon \scrB(\scrH)\to\C$ preserves
suprema of self-adjoint operators,
then $\omega\equiv \sum_{n=1}^\infty \left<x_n,(\,\cdot\,)x_n\right>$
for some $x_1,x_2,\dotsc$ in~$\scrH$
with $\sum_n \|x_n\|^2 <\infty$.

\TODO{Proof that these maps are indeed.}
