...
We end
with allusions to von Neumann algebras---which are the
...
For an example
of a infinite-dimensional Hilbert
space,
...
for a Hilbert space~$\Define{\ell_\infty}$
...
converges for~$x,y\in\bigoplus_i \scrH_i$ to begin with.
Given~$\varepsilon>0$
we must
...
\end{equation*}
we 
see that any~$G\subseteq I$
with $\sum_{i\in I\backslash G} \|x_i\|^2 \leq \varepsilon$
and~$\sum_{i\in I\backslash G} \|y_i\|^2 \leq \varepsilon$
will do.

It is easily seen that
$\left<x,y\right>:=\sum_i \left<x_i,y_i\right>$
gives an inner product on~$\bigoplus_i \scrH_i$; the
...
for all~$n,m\geq N$.
Note that since for such~$n$ the sum
\begin{equation*}
\sum_i \|(x_\infty)_i - (x_n)_i \|^2
\ \equiv\ 
...
we can find
a finite subset~$F$ such that second term in the right-hand side above
...
after having establised that cp-maps are closed under composition.
\end{point}
\end{point}
...
\REMARK{Picture will be improved}
\begin{equation*}
\begin{tikzpicture}
\coordinate (A) at (0,0){};
\coordinate (B) at (4,0){};
...
around~$z_0$.
There is nothing special about triangle:
a similar result---not needed here---holds
for a broad class of curves
(c.f.~Thm~2.9 of~\cite{conway2000}).
...
Let~$f$ be a holomorphic $\scrA$-valued function
defined on the boundary an interior
of a regular $K$-gon
with vertices $w_0,\dotsc,w_{K-1},w_K=w_0$
as in~\sref{cauchy-formula}.
Then for every element~$z$ of an open disk in interior of the $K$-gon
with center~$w$,
\begin{equation*}
f(z)\ = \ 
...
\begin{enumerate}
\item $\spec(a)\neq \varnothing$;
\item $\spec(a) =\{\lambda\}$ iff $a=\lambda$ for every $\lambda \in\R$.
\end{enumerate}
\end{point}
...
\item
\Define{center separating} if $a\in\scrA_+$
is zero iff $\omega(b^*ab)=0$ for all~$\omega\in\Omega$.
\end{enumerate}
\REMARK{Examples that show these notions
are all different will be included.}
\end{point}
\begin{point}{Remark}%
...
(see~\sref{gelfand-representation-isometry}); and
\item
...
\end{equation*}
\REMARK{Be warned:
this proof being much simpler than one given by Kadison 
might contain an error---he shows
shows that $\scrA/I$ is total and Archimedean first.}
We claim that $1\notin I$.
...
for every continous map $f\colon \spec(a)\to\C$
...
\REMARK{Will refer here
to Paschke's dilation 
(in Bas' thesis)
as generalization of the GNS-construction.}
\end{point}
...
but more on that in~\REMARK{Reference
to Bas' thesis will be added here}
\end{point}
\end{point}
...
\end{point}
\end{point}
\begin{point}[cp-comprehension]{Corollary}%
Let~$f\colon \scrA\to\scrB$ be a cp-map between
$C^*$-algebras,
...
by pointing two special properties
of~$\scrB(\scrH)$
...
is called \Define{\textbf{n}ormal} when
...
$\omega$ is of the form described in~\sref{bh-functional-lemma},
