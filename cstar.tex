...
form a category which we denote by~$\Define{\Cstar{miu}}$.
The full subcategory of commutative $C^*$-algebras
is denoted by~$\Define{\cCstar{miu}}$.
\end{point}
...
Since~$1=\sum_{e\in\scrE} \ketbra{e}{e}$
and~$\omega$ is normal
we have $\omega(1)=\sum_{e\in\scrE} \omega(\ketbra{e}{e})
...
Note that for each~$x\in\scrH$
we have $\omega(\ketbra{x}{x})
...
=\omega'(\ketbra{x}{x})$.
Thus~$\omega(P)=\omega'(P)$
for every projection~$P$ in~$\scrB(\scrH)$
because $P=\sum_{f\in\scrF}\ketbra{f}{f}$
for any orthonormal basis~$\scrF$ 
of~$\Ran(P)$.
Since the linear span
of projections is norm dense in~$\scrB(\scrH)$
by~\TODO{...}
we get~$\omega=\omega'$.\qed
\end{point}
\end{point}

\TODO{Definition of $\ketbra{x}{x}$.}

\TODO{Projections dense in~$\scrB(\scrH)$}
\end{parsec}
\end{document}
