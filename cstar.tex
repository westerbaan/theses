...
In particular,
given $T\in\scrB^a(X)$ and
writing~$S=\{x\in X\colon \|x\|=1\}$,
\begin{enumerate}
\item
$T$ is self-adjoint iff $\left<x,Tx\right>$
is self-adjoint for all~$x\in S$;
\item
$0\leq T$ iff $0\leq\left<x,Tx\right>$
for all~$x\in S$;
\item
$\|T\|=\sup_{x\in S}\|\left<x,Tx\right>\|$
...
=\sup_{x\in S} \left\|\left<x,Tx\right>\right\|$.\qed
\end{point}
\end{point}
