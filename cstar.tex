...
We'll also prove two handy lemmas for later use.
\end{point}
...
\end{point}
\begin{point}{Lemma}%
	$f(a^*b) f(b^*a)\leq \|f(b^*b)\|\,f(a^*a)$
for every linear map $f\colon \scrA\to\scrB$
between $C^*$-algebras
and $a,b\in\scrA$,
...
and so $(x,y)\mapsto \left<x,Ty\right>$
gives an $\scrB$-valued inner product
on~$\scrB^2$.
Then  
{
\newcommand\twovect[2]{%
...
\begin{alignat*}{3}
	f(a^*b)\,f(b^*a)
	\ &=\  \left<\,\onezero,\,T\zeroone\,\right> \ 
\left<\,\zeroone,\,T\onezero\,\right> \\
\ &\leq\  
\left\|\left<\,\zeroone,\,T\zeroone\right>\right\| \ 
\left<\,\onezero,\,T\onezero\,\right>
\ =\  \|f(b^*b)\|\  f(a^*a)
\end{alignat*}
}
by Cauchy--Schwarz for $\left<\,\cdot\,,T\,\cdot\,\right>$, 
see \sref{chilb-cs}. \qed
\end{point}
\end{point}
\end{parsec}
\begin{parsec}%
