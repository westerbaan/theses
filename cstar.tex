...
Let~$a$ be a self-adjoint element of a $C^*$-algebra $\scrA$ with $\|a\|>1$.
\begin{enumerate}
\item
Show that $1,\,a,\,a^2,\,a^4,\,a^8,\,\dotsc$
diverges
(using $\|a^2\|=\|a\|^2$).
\item
Show that $\sum_n a^n$ diverges.
\item
Show, however, that~$\sum_n a^n$
might converge when the assumption that~$a$ is self-adjoint is dropped.
(Hint: $\bigl(\begin{smallmatrix}
	0&2\\0&0
\end{smallmatrix}\bigr)$.)
\end{enumerate}
\end{point}
...
\end{point}
\begin{point}{Exercise}%
Let~$f$ be an $\scrA$-valued
function, and let~$w\in \dom(f)$.

Stretching accepted terminology, we'll say that
a sequence $a_0,a_1,a_2,\dotsc\in\scrA$
is a \Define{power series expansion} of~$f$ around~$w$
when~$f(z)=\sum_n a_n (z-w)^n$ for all~$z\in \dom(f)$.
\begin{enumerate}
\item
Show that if~$f$ has a power series expansion $a_0,a_1,a_2,\dotsc$
around~$w$,
then~$f$ is holomorphic,
and~$f'$ has power series expansion~$a_1,\,2a_2,\,3a_3,\,\dotsc$
around~$w$.
\item
Show that if $a_0,a_1,\dotsc$ is a power series expansion
of~$f$ around~$w$,
and $f(z)=0$ for all~$z\in\dom(f)$,
then~$0=a_0=a_1=\dotsb$.

\item
\Define{(Unicity of the power series expansion)}\ 
Suppose that $a_0,a_1,\dotsc$
and $b_1,b_2,\dotsc$ are power series expansions of~$f$
around~$w$.
Show that~$b_n=a_n$ for all~$n$.

\item
\Define{(Rigidity of power series)}\ 
Suppose that~$f$ has a power series expansion
around~$w$
and let~$U$ be a non-empty open subset of~$\dom(f)$
that contains~$w$.

Show that a power series expansion~$b_1,b_2,\dotsc$
of~$f|U$ around~$w$
is also a power series expansion of~$f$ around~$w$.
\end{enumerate}
\end{point}
