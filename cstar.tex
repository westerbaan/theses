...
\begin{parsec}%
\begin{point}%
\TODO{perhaps move this down, below
complete positivity}
...
This has the obvious --- but important --- consequence
that any unital map between $C^*$-algebras
preserves the order iff it preserves the norm
of self-adjoint elements.
\end{point}
\begin{point}{Exercise}%
Show that for a unital linear map $f\colon \scrA\to\scrB$
between $C^*$-algebras,
the following are equivalent.
\begin{enumerate}
\item
	$f$ is an \Define{isometry on~$\sa{\scrA}$}, 
...
\end{enumerate}
\begin{point}{Warning}%
While a bipositive unital map between $C^*$-algebras
is an isometry on self-adjoint elements,
it may not preserve the norm of arbitrary elements:
...
\begin{parsec}%
\begin{point}\label{completion-inner-product-space}%
...
\end{point}
\begin{point}{Proposition}%
Let~$f\colon \scrA\to\scrB$ be a positive map between
$C^*$-algebras.
If either~$\scrA$ or~$\scrB$ is commutative,
then~$f$ is completely positive.
\begin{point}{Proof}%
Let~$a_1,\dotsc,a_N\in \scrA$
and $b_1,\dotsc,b_N\in\scrB$ be given.

Suppose that~$\scrB$ is commutative.
Since for every miu-map $\omega\colon\scrB\to\C$,
we have
$\omega(\,\sum_{i,j} b_i^*f(a_i^*a_j)b_j\,)
= \omega(f(\,\sum_{i,j}(a_i\omega(b_i))^*\,a_j \omega(b_j)\,))\,\geq \,0$,
we see that $\sum_{i,j} b_i^*f(a_i^*a_j)b_j$ is positive
(by \sref{gelfand}),
and so~$f$ is completely positive.
