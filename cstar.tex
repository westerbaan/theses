...
\index{equalizer!in~$\Cstar{pu}$ and $\cCstar{pu}$}
...
$a-i$ is invertible.
\begin{point}{Proof}%
(Based on Proposition 4.1.1(ii) of~\sref{kr}.)
...
will be explained in~\sref{center-separating}.)
\end{enumerate}
\end{point}
...
These functionals $\left<x,(\,\cdot\,)x\right>$ 
		are called \Define{vector functionals}%
\index{vector functional}%
\index{functional!vector}.
\end{enumerate}
\begin{point}
We'll also see
that
none of the four notions
of separation coincides.
\begin{enumerate}
\item
A single non-zero vector from a Hilbert space~$\scrH$
...
(\alpha-q(a))\ ]\,\wedge\,
		\\
		&\exists\lambda,\mu\in \R\,[\ 
\lambda (\alpha-q(a))\,\le\, \Imag{b}\,\leq\, \mu (\alpha-q(a)) \ ] \,\}.
\end{alignat*}
We claim that $1\notin J$.
...
is to be isomorphic to a $C^*$-algebra
...
An \Define{inner product} on a right $\scrA$-module~$X$
