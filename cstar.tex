...
such that,
for all~$x,y\in V$,
$\left<x,\,\cdot\,\right>\colon V\to \C$ is linear;
...
For a bounded adjointable operator~$T$ on a pre-Hilbert space~$\scrH$
\begin{equation*}
...
and conclude that~$\left<y,x-y\right>=0$.
Show that~$y+c$ is a projection of~$x+c$ on~$C$
for every~$c\in C$.
Conclude that~$y$ is the unique projection of~$x$ on~$C$,
and that~$\left<y',x-y\right>\equiv\left<y',(x+y'-y)-y'\right>=0$
for every~$y'\in C$.
\end{point}
...
we have $\|x_\infty -x_n\|\leq \varepsilon$.
...
\begin{parsec}{90}%
\begin{point}{10}%
Let us now generalise the notion of a positive function
in~$C(X)$
...
(see~\sref{cx-positive}) on which we can base such a  generalisation,
...
iff $0\leq f\leq 2t$, for $t\geq 0$.)
\begin{point}{30}{Exercise}%
...
\index{$(\,\cdot\,)^\perp$!$a^\perp$, orthosupplement of an effect}
is an effect too.
\end{point}
...
we refer to~\sref{bh-normal} and~\sref{p-uwcont}.)
\end{enumerate}%
\spacingfix{}%
\begin{point}{30}%
...
when~$b\in\scrA$ is invertible and $\|a\| < \|b^{-1}\|^{-1}$.
\item
...
and let~$b\in\scrA$ with~$\|a-b\|\leq\frac{1}{2}\|a^{-1}\|^{-1}$.
We claim that~$b$ is invertible,
and~$\|a^{-1}-b^{-1}\|\leq 2\|a-b\|\,\|a^{-1}\|^2$.
Since $\|a-b\|\leq \frac{1}{2}\|a^{-1}\|^{-1}$
we have
...
Indeed, for~$n$ such that~$\|a\|^2<2n+1$,
...
\begin{enumerate}
\item
The spectrum of a continuous function~$f\colon X\to \C$
on a compact Hausdorff space~$X$
...
and the function $x\mapsto \Define{f'}(x)$
with $\dom(f')=\dom(f)$
...
and $(f+g)'=f'+g'$ and $(f\cdot g)' = f'g+fg'$.
...
for all~$z\in \C$ is holomorphic, and $f'(z)=0$ for all~$z\in \C$.
...
\begin{alignat*}{3}
	...
	z^{-1}\,dz
	\ &=\ 
	i\int_{0}^b \frac{a-it}{a^2+t^2}dt
	\\
	\ &=\ 
...
Conclude that
\begin{equation*}
	\frac{1}{2\pi i}\sum_{n=0}^{N-1}\int_{w_n}^{w_{n+1}}(z-z_0)^{-1}\,dz
	\ = \ 1
\end{equation*}
for a regular $N$-gon whose vertices $w_0,\dotsc,w_{N-1}$
are numbered counterclockwise,
where~$w_N:=w_0$,
and~$z_0$ in its interior.

(Hint: partition the region between the $N$-gon
and a triangle~$T$ with~$z_0\in\mathrm{in}(T)$
into triangles, and use~\sref{goursat}.)
\end{enumerate}%
\spacingfix{}%
\end{point}%
\begin{point}{90}%
Thus integration of 
$z\mapsto (z-z_0)^{-1}$
along a triangle~$T$ (or regular polygon) detects
the number of times it winds
around~$z_0$.
...
with centre~$c\in\C$,
circumradius~$r$
and vertices $w_n := c+r\cos(2\pi n/N)+ir\sin(2\pi n/N)$.
...
and~$\lambda\in\R$
show that $\spec(a) \subseteq\{\lambda\}$ iff $a=\lambda$.
\end{point}
...
Conclude that $\|a\| = \inf\{ \lambda \geq 0\colon
-\lambda \leq a\leq \lambda\}$.
...
we see that $\lambda^{-1}(1+b(\lambda-ab)^{-1}a)$
...
The set of all pu-maps $\omega\colon \scrA\to\C$
    (called \Define{states}\index{state of a $C^*$-algebra})
on a $C^*$-algebra
...
The unital vector functionals (called \Define{vector states})%
\index{vector state = unital vector functional}
are order separating too.
\end{enumerate}%
\spacingfix%
\begin{point}{40}
...
\begin{alignat*}{3}
	...
	\lambda (\alpha-q(a))\,\leq\, q(\Real{b})\,\leq\, \mu
(\alpha-q(a))\ ]\,\wedge\,
		\\
		&\exists\lambda,\mu\in \R\,[\
\lambda (\alpha-q(a))\,\leq\, q(\Imag{b})\,\leq\, \mu (\alpha-q(a)) \ ] \,\}.
\end{alignat*}
We claim that $1\notin J$.
...
by~\sref{square-commuting-monotone}, and so
...
\item
If~$a,b\in\Real{\scrA}$ commute, and~$0\leq a\leq b$, then~$a^2\leq b^2$.
...
\item
Show that $\left|a+b\right|\leq \left|a\right|+\left|b\right|$.
In fact,
$\left|a+b\right|\vee\left|a-b\right|
\,=\,\left|a\right|+\left|b\right|$.
\item
Show that the $C^*$-subalgebra generated by two commuting
self-adjoint elements $a$ and~$b$ of an arbitrary $C^*$-algebra~$\scrB$ is commutative.
This means~$a\wedge b$ and~$a\vee b$ make sense
for commuting self-adjoint elements~$a$ and~$b$
of any $C^*$-algebra.
\end{enumerate}%
\spacingfix%
\end{point}%
...
\end{point}
\begin{point}{20}[gelfand-setting]{Setting}%
...
Recall that~$\scrA$ is commutative
in this setting (\sref{gelfand-setting}),
which we'll use several times below.

We'll show first that for~$a,b\in\pos{\scrA}$
with~$ab=0$ either $a\in I$ or~$b\in I$.
To this end, consider
the Riesz ideal~$I+(a)_m$,
where~$(a)_m$ is the Riesz ideal generated by~$a$
from~\sref{riesz-ideal-basic}(1).
...
\leq \left|x\right| + na$,
using that~$\sa{\scrA}$ is a Riesz space,
see~\sref{commutative-cstar-basic}.
...
\ \in\ I\quad \text{by \sref{riesz-ideal-ring-ideal}.}
\end{alignat*}
...
Thus, by the previous discussion, either~$a_+\in I\subseteq J$
...
and so~$I=J$.
Thus~$I$ is a maximal order ideal.\qed
\end{point}
\end{point}
...
$\Define{C^*(a)}$%
\index{Cstara@$C^*(a)$, $C^*$-subalgebra generated by~$a$}
of~$\scrA$
that contains~$a$.

Given~$b\in C^*(a)$
show that~$bc=cb$ for all~$c\in\scrA$
with~$ac=ca$ and $a^*c=ca^*$.
\item
...
as~$n\to\infty$ (see~\sref{fc-uniformly-continuous} below).
In other words,
...
+ \frac{1}{2}\beta \in E$.
\begin{point}{41}[fc-uniformly-continuous]{$(a+\frac{1}{n})^\alpha$
	converges to~$a^\alpha$}
...
&&\text{by~\sref{gelfand-representation-isometry}}
\\
...
&&\text{by~\sref{functional-calculus}(4)}
\\
\ &\leq\ \varepsilon,
\end{alignat*}
...
and the functional
	...
	C(\spec(b))\to \scrA$, which being an miu-map is norm continuous
	as well.
\end{point}
...
and that it can be generalised to apply
...
\end{equation*}
\item
Show that $\|ab\|_\omega \leq \|a\|\,\|b\|_\omega$
for all $a,b\in\scrA$
(using $a^*a\leq \|a\|^2$).
...
\index{inner product!*C-valued@$\C$-valued!completion}
...
(because $\|ab\|_\omega\leq \|a\|\|b\|_\omega$
...
given by~\sref{gns} is an miu-map.
\begin{point}{80}{Proof}%
...
= \bigoplus_{\omega\in\Omega} \varrho_\omega(a)x(\omega)$,
...
\index{$\left<\,\cdot\,,\,\cdot\,\right>$, inner product!*avalued@$\scrA$-valued}
...
We say that such an inner product is \Define{definite}%
	\index{$\left<\,\cdot\,,\,\cdot\,\right>$, inner product!*avalued@$\scrA$-valued!definite}
...
Show that
the subunital vector functionals%
\index{vector functional!for a Hilbert $\scrA$-module}
of~$\scrB^a(X)$,
that is, the functionals $\left<x,(\,\cdot\,)x\right>$
with~$x\in (X)_{\leq 1}:=\{\,x\in X\colon\,\|x\|\leq 1\,\}$,
are order separating (see~\sref{separating}).
Conclude that
...
\item
$T$ is self-adjoint iff $\left<x,Tx\right>$
is self-adjoint for all~$x\in (X)_{\leq 1}$;
\item
$0\leq T$ iff $0\leq\left<x,Tx\right>$
...
\setcounter{enumi}{2}
\item
	$\|T\|=\sup_{x\in (X)_{\leq 1}}\|\left<x,Tx\right>\|$
when~$T$ is self-adjoint.
\end{enumerate}
\end{point}
...
\begin{enumerate}
\item
	Show that every $M\times N$-matrix~$A$ (over~$\scrA$)
...
(Yes, all module maps from~$\scrA^N$ to~$\scrA^M$
are bounded and adjointable.)
...
(Hint: use~\sref{chilb-vector-states-order-separating}.)
\item
...
and that~$M_nf$ is bounded by~$n^2\|f\|$ when~$f$ is bounded.
\end{enumerate}%
\spacingfix%
\end{point}%
\end{parsec}%
...
This result will be useful at the end of this
...
on~$\scrB(\scrH)$ is the topology induced by the seminorms
$T\mapsto \|Tx\|\equiv \smash{\left<x,T^*Tx\right>^{%
\nicefrac{1}{2}}}$
where~$x\in\scrH$.
...
(c.f.~\sref{inner-product-basic}\eqref{polarization-identity})
\begin{equation*}
	\textstyle
...
Let~$\scrH$ be a Hilbert space,
and~$\scrD$ a non-empty upwards directed subset of~$\Real{\scrB(\scrH)}$
...
we may assume that~$\scrD$ has a least element~$T_0$;
for if it does not,
pick some~$T_0$ from~$\scrD$,
and replace~$\scrD$ by~$\{\,T\in\scrD\colon T_0\leq T\,\}$.
(If the latter WOT-converges,
then so does the former,
...
In particular,
$(\left<x,Tx\right>)_{T\in\scrD}$ is Cauchy;
and it is bounded as well,
being squeezed between~$\left<x,T_0x\right>$
and~$\sup_{T\in\scrD}\left<x,Tx\right>$.
Thus, by~\sref{bh-wot-bounded-complete},
$(T)_{T\in\scrD}$
WOT-converges to some~$T'$ from~$\scrB(\scrH)$.
Note that~$T'$ is self-adjoint
by~\sref{hilb-positive-operators}(1),
as $\left<x,T'x\right>=\lim_{T\in\scrD}\left<x,Tx\right>$
is real for every~$x\in\scrH$.
...
\index{*infsup@$\bigvee D$, supremum of~$D$!in $\scrB(\scrH)$}
\end{point}
\end{parsec}
...
We use the letter ``n'' to abbreviate ``normal'' in line with~\sref{maps}.
...
show that if~$x_\alpha$ converges to~$x$,
then $\left<x_\alpha,(\,\cdot\,)x_\alpha\right>$
...
\begin{equation*}
	\omega(A)\ = \
	\lim_{\substack{\scrF\subseteq\scrE\\\text{finite}}}\ \
	\sum_{e,e'\in\scrF} \left<e,Ae'\right>\ \omega(\,\ketbra{e}{e'}\,),
\end{equation*}
