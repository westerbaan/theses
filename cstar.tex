...
For a collection $\Omega$ 
of pu-maps
on a $C^*$-algebra~$\scrA$
--- so each $\omega\in\Omega$ is of the form
$\omega\colon \scrA\to\scrB_\omega$
for some~$C^*$-algebra $\scrB_\omega$ ---
the following are equivalent.
\begin{enumerate}
\item 
	$0\leq a$ iff $\forall \omega\in\Omega\ [\ 0\leq \omega(a)\ ]$.
\item
