...
### A7. 104III — what does `p ∧ q` mean?  (the carrier half was ruled on 2026-08-19)
`proc.tex:1465`.  **The carrier question is settled and incorporated**: points
2a, 3 and 4 now assume `⌈p⌉ = ⌈q⌉ = 1`, and point 5 assumes `⌈p⌉ = ⌈q⌉`
(erratum `parsec-1040.30`).  The 2026-08-16 proposal `⌈⌈p⌉⌉ = ⌈⌈q⌉⌉` was
dropped: it cannot repair .4, whose refuting witness has `p = q` and therefore
satisfies every hypothesis reflexive in the pair.  See HANDOFF, "Resolved by
the author (2026-08-19)", for the authorised Lean-side change.

**Still open** is the second question, which the ruling did not touch.  Points
3, 4 (third `iff`) and 5 mention `p ∧ q`, and 104VII's own construction of the
`eₙ` (proc.tex:1591) takes an approximate pseudoinverse of it.

* Read as the thesis's `⋀` (vn.tex:371, the infimum in the order of `𝒜`),
  Kadison's anti-lattice theorem makes `p ∧ q` fail to exist outside the
  commutative case, so those hypotheses are rarely satisfiable and 104VII's
  example construction presupposes something it need not have.
* Read as the meet in the commutative von Neumann algebra generated by the
  commuting pair, everything exists and the hint at proc.tex:1516 reads
  naturally.

The reading does **not** decide truth under the new hypotheses — the argument
below works for any lower bound `m` of `p` and `q` with `m ≥ pq/(‖p‖+‖q‖)`,
which covers both.  It decides what the statements *say*, and whether the text
needs a sentence, since `⋀` is defined elsewhere to mean the first thing.

**The argument for .3 under faithfulness**, offered for the write-up and *not*
formalized: `pq/(‖p‖+‖q‖)` is a lower bound of both `p` and `q`, and
`⌈pq⌉ = 1` (if `pqr = 0` for a projection `r` then `qr = 0` because `⌈p⌉ = 1`,
hence `r = 0` because `⌈q⌉ = 1`), so `⌈m⌉ = 1` for every lower bound `m` above
it.  Then `m = cp` with `c` central gives `⌈c⌉ ≥ ⌈cp⌉ = ⌈m⌉ = 1`, which is
exactly the missing carrier condition, and likewise for `d`.  (Caveat: this
reads `m/p` as the genuine quotient, i.e. it assumes `m ∈ 𝒜p` and `m ∈ 𝒜q`,
which our `div` does not force — off its domain `div` has the junk value `0`,
which is central.)

Nothing is waiting on this: 104VII is proved without .3/.4/.5 (session 91), so
all four parts are leaves.
