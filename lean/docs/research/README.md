...
* `rec-citations.md` — REC's external citations checked against the sources: A–S *Geometry* Lemma 4.4, 9.43, 9.48, *State spaces* (1.82), 1.106, 1.108, 1.114, H-O–S 7.2.7, van de Wetering Prop 46 verified; REC 55 is Shultz 1979 Thm 3.9; the vdW paper is *Sequential product spaces are Jordan algebras* (arXiv:1803.11139v3). Lean docstring fixes applied.
* `as943-abstract.md` — the abstract A–S hypothesis stays open: compatibility of nested compressions (G2) follows from its kernel condition (so `ChainDense`'s commutation clause is redundant), but chain density does not (a spanning family in M₂(ℝ)_sa has non-dense chains); no counterexample on symmetric or Vinberg cones. Reviewed: §1, §2 stand; the §4 equivalence needs T extended first.
* `as943-fable.md` — the abstract A–S hypothesis is TRUE for finite-dimensional W: symmetry + spanning give a commutative product with L(W) ⊆ g(K); the characteristic function makes g(K) = L(W) ⊕ stab(1) a Cartan decomposition, so K is homogeneous and self-dual and the product is the Koecher–Vinberg Jordan product. Open in infinite dimension (needs a bound on L(a) and a Koecher–Vinberg substitute). Reviewed: stands; polar-decomposition citation should be Knapp Thm 6.31 (identity component), §4's ℓ^∞-sum remark covers only coordinatewise families.
* `as943-infinite.md` — the abstract A–S hypothesis in infinite dimension reduced to: bounded L(a) + Jordan identity + the order axiom (a Prop ⟺ statement); neutrality of states under U_i and homogeneity given boundedness proved. Reviewed: those stand; the "Lie-triple condition ⇒ Jordan" heuristic is BROKEN (4-dim nil counterexample), leaving formal reality + Hentzel–Peresi (unverified) as a lead. Still open.
* `as943-almostjordan.md` — the abstract A–S hypothesis ⟺ (i) boundedness of L + (K₃) (commutators [[T_i,T_j],T_k] = L̄([T_i,T_j]e_k)): (K) is exactly almost-Jordan; order derivations make a∘a = 0 ⇒ a = 0; Hentzel–Peresi (Zbl 0716.17033: the Jordan-associator ideal of an almost Jordan ring squares to zero) then gives the Jordan identity; the order axiom follows. Reviewed: stands (Hentzel–Peresi via its zbMATH review; the paper itself not read). (i) and (K₃) open.
* `rec136-composites.md` — no literature "no composites" theorem fits REC 136 in infinite dimension: Hanche-Olsen 1985 needs local tomography and a qubit factor; Barnum–Graydon–Wilce is finite-dimensional and its Thm 4.12 is REC 135's own proof. Reviewed: stands.
* `rec136-native.md` — REC 136 without the exchangeable family: three pairwise op-commuting copies of V_W in V_{(W⊗W)⊗W}, mapped by χ into the Albert algebra, contradict a finite-dimensional Albert lemma (Peirce + cubic trace + spectral idempotents, no classification). REC 136 would need only JBWExceptionalSummand + a Jordan hom χ : V → Alb with χ1 ≠ 0. Reviewed: stands after two small fixes (G1 idempotent choice in case (1,2); G2 run E3 on S ⊕ ℝ(1−e)).
