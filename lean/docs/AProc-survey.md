...
> axiom-clean.  **+270 lines**, all in `QuantumLambda.lean`; every new name
> `private`, so no public name was added and nothing downstream changed.
>
> * **proc.tex:5265's "exactly as in `second-adjunction-proof`" is accurate**,
>   and 124III's apparatus carried almost the whole proof: `exists_ncpsu`
>   `Corestrict`, `vnsub_wstar_eq_top`, `ncpsuCompNmiu`, **124I**
>   `vn_generation_bound`, **47IV**.3 and **47V** are all reused verbatim.
> * **Three things got *cheaper* than 124III, not dearer.**  (i) The solution
>   set is indexed by *presentations* `⊕_{j∈J} M_{Nⱼ+1}` with `J ⊆ K`, because
>   `HereditarilyAtomic` (**84bII**) *is* the direct-sum decomposition; so only
>   the summand index set is relabelled (`exists_lp_reindex`, written for
>   125cIII and moved up), and **125II** `vn_gns_bound` /
>   `exists_smallRealization` are **not used** — `#K = 2^(2^(𝔠+#𝒜))` suffices,
>   one exponential less than 124III.  (ii) The product is taken over the
>   `Σ`-type of (entry, summand) pairs, so it is *literally* a direct sum of
>   matrix algebras and the recorded new obligation **"a direct sum of
>   hereditarily atomic algebras is hereditarily atomic" was never needed** —
>   it is `StarAlgEquiv.refl`, and what would have been the flattening
>   isomorphism is one block projection out of **47IV**.  (iii) The factors are
>   `M_{N+1}`, always nontrivial, so 124III's `Nontrivial` field and its
>   trivial-target case both vanish.
> * **`ha_equalisers` (84bV) is not used either** — nor is any equaliser
>   statement about *hereditarily atomic* algebras: Freyd's equaliser step
>   collapses, exactly as in 124III, to **47V** `vn_equalisers` plus minimality
>   of `W*(G)`, and hereditary atomicity of the carrier is **84bIII**.
> * **`hA` is unused**, and provably so: `F_ha` exists for *every* von Neumann
>   algebra, hereditarily atomic or not.  Not an erratum (the thesis states
>   the adjunction on `haW*_cpsu`), but worth recording.
> * **Next root in `QuantumLambda.lean`: 121II `intersection_tensor`**
>   (`:306`, proc.tex:4450) — after 125bII it is the *only* remaining sorry in
>   the file with no open in-file dependency, and it is class (c′): proc.tex
>   :4473 gives no argument, only "See Corollary IV.5.10 of Takesaki I".  The
>   whole rest of the file hangs off it *as printed*: 125IV `equaliser_lemma`
>   (`:1783`, proc.tex:4846) uses it for the single step
>   `Ã⊗𝒞 = (Ã⊗B(ℋ)) ∩ (𝒜⊗𝒞)`; 125VI `tensor_equalisers` uses 125IV; 125VIIb
>   `tensor_preimage` and 125eIII `tensorBsurjectivity` use 125VI; 125VIII
>   `tensor_closed` → 125dII `ha_tensor_closed` (proc.tex:5538 defers to
>   `tensor-closed-proof`); 125eIIa `tensor_map_factorisation` is "by
...
| 125bII | `ha_second_adjunction` | :2205 | (a) | **CLOSED (session 82)** — ~250 lines.  proc.tex:5265's "exactly as in 124III" is accurate, and 124III's apparatus did most of the work.  Three of the recorded ingredients turned out to be unnecessary: "a direct sum of hereditarily atomic algebras is hereditarily atomic" (the product is formed flat over a `Σ`-type, so it *is* a direct sum of matrix algebras), **84bV** `ha_equalisers` (Freyd's equaliser step is **47V** as in 124III), and **125II** `vn_gns_bound` (no Hilbert-space realisation is needed — `HereditarilyAtomic` already is the direct-sum decomposition, so only the summand index set is relabelled, by `exists_lp_reindex`).  `hA` is unused |
