...
3. If ours diverges and the printed route can be followed in **≤ 300 new lines** with tools already in the tree or in Mathlib: rewrite the proof onto the printed route, keeping the statement byte-identical. Compile until exit 0 with no new warnings. Set `proof` to `faithful` (or `mild` if a local shortcut remains, saying which), status `repaired <date>: <one sentence on what the printed route needed>`.
