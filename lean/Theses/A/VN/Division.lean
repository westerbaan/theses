...
`fdcstar` packages the induction of the section above.

Until 2026-09-05 it was proved **not** by the thesis's argument but by taking
the *algebra* decomposition from Mathlib's Wedderburn-Artin theorem
...
upgrade needed Skolem-Noether for matrix algebras, which Mathlib does not have
and which was proved here.  That route transcribed none of vn.tex:5798-6027,
and in particular left 840.40 `suprema-in-fdvna` -- that a finite-dimensional
C*-algebra is a von Neumann algebra -- unformalized anywhere in the tree.  Its
five auxiliaries (`cstar_jacobson_eq_bot`, `matrix_single_mul_single`,
`matrix_exists_intertwiner`, `matrix_exists_algEquiv_conj`,
`central_idempotent_isSelfAdjoint`) and the abbreviation `MatProd` were left
in place when the printed proof landed and were deleted, unconsumed, on
2026-09-05; `docs/DEAD-LIMBS.md` §16 records them. -/
