...
/-- **142VIII** (dils.tex:1495, Example): for a pre-Hilbert 𝒷-module `X` and
**every** 𝒷-linear `T : X → X` the map `⟨·, T·⟩` is a 𝒷-sesquilinear form.

"𝒷-linear" is rendered as a ℂ-linear map with `T (b • x) = b • T x`, the
shape used for module maps throughout (`SelfDual`, `hilbmod_adjoint_exists`);
over a *unital* 𝒷 the ℂ-linearity is not an extra assumption, being
`T (c • x) = T ((c • 1) • x) = (c • 1) • T x = c • T x`.

The proof is one line per clause of **142VII**: the identity needs only
linearity in each slot.  `ba_isBSesquilinear` (`SelfDualCompletion.lean`) is
the case `T ∈ 𝒷ᵃ(X)` of this, stated over Mathlib's `CStarModule` (whose
bundled inner product is `cstarBInner`), which is the case **152III** and
**152XII** use. -/
theorem blinear_isBSesquilinear (B : BInner 𝒷 X) (T : X →ₗ[ℂ] X)
    (hT : ∀ (b : 𝒷) (x : X), T (b • x) = b • T x) :
    IsBSesquilinear (fun x y : X => B.inner x (T y)) where
  add_left x y z := B.inner_add_left x y (T z)
  add_right x y z := by
    simp only [map_add]
    exact B.inner_add_right x (T y) (T z)
  smul_op β b x y := by
    simp only [hT]
    rw [B.inner_op_smul_right, B.inner_op_smul_left, mul_assoc]
  smul_left_complex c x y := B.inner_smul_left_complex c x (T y)
  smul_right_complex c x y := by
    simp only [map_smul]
    exact B.inner_smul_right_complex c x (T y)
...
**146V** (`dils-uniformity-examples`, dils.tex:1843, Examples) is rendered
clause by clause: (1) every metric space is a uniformity, subbase
`ε̂ = {(x,y) ; d(x,y) ≤ ε}` — Mathlib's `Metric.uniformity_basis_dist_le`
over `PseudoMetricSpace.toUniformSpace`, the instance every
`NormedAddCommGroup` here already carries; (2) a family of pseudometrics —
`uniformity_of_pseudometrics` below, through this exercise; (3) a vector
space with a family of seminorms — the ultranorm uniformity's
`hasBasis_uniformity` (`SelfDualCompletion.lean`), through Mathlib's
`withSeminorms`; (4) the ultrastrong and ultraweak uniformities of a von
Neumann algebra — `ultrastrongUniformity` and `ultraweakUniformity` below,
with their subbases and with their topologies identified with thesis A's
`ultrastrong` / `ultraweak` (**42III**).
...
/-- **146V**.2 (`dils-uniformity-examples`, dils.tex:1843, Examples): a
(possibly infinite) family `(d_α)` of **pseudometrics** on a set `X` fixes a
uniformity, through the subbase `E_{α,ε} = {(x,y) ; d_α(x,y) ≤ ε}` of
**146IIIa**.

A pseudometric is taken as the point takes it — symmetric, `d α x x = 0`, and
obeying the triangle inequality, with no separation axiom — so the family is
a bare family of functions rather than a family of `PseudoMetricSpace`
instances (which could not coexist on one `X` anyway).  The three subbase
axioms are one line each: reflexivity is `d α x x = 0 ≤ ε`, the
half-entourage is `E_{α,ε/2} ∘ E_{α,ε/2} ⊆ E_{α,ε}` by the triangle
inequality, and `E_{α,ε}` is its own symmetric; `exc_subbase` (**146IV**)
does the rest.  Clause 3 of the point, a family of seminorms, is the special
case `d_α(x,y) = ‖x-y‖_α`. -/
theorem uniformity_of_pseudometrics {X : Type v} {ι : Type w} (d : ι → X → X → ℝ)
    (hself : ∀ (α : ι) (x : X), d α x x = 0)
    (hsymm : ∀ (α : ι) (x y : X), d α x y = d α y x)
    (htri : ∀ (α : ι) (x y z : X), d α x z ≤ d α x y + d α y z) :
    ∃ U : UniformSpace X, @uniformity X U =
      Filter.generate {E : Set (X × X) | ∃ (α : ι) (ε : ℝ), 0 < ε ∧
        E = {p : X × X | d α p.1 p.2 ≤ ε}} := by
  refine exc_subbase _ ?_ ?_ ?_
  · rintro V ⟨α, ε, hε, rfl⟩ x
    show d α x x ≤ ε
    rw [hself]
    exact hε.le
  · rintro V ⟨α, ε, hε, rfl⟩
    refine ⟨{p : X × X | d α p.1 p.2 ≤ ε / 2}, ⟨α, ε / 2, by positivity, rfl⟩, ?_⟩
    rintro ⟨x, z⟩ ⟨y, hxy, hyz⟩
    show d α x z ≤ ε
    have h1 : d α x y ≤ ε / 2 := hxy
    have h2 : d α y z ≤ ε / 2 := hyz
    have := htri α x y z
    linarith
  · rintro V ⟨α, ε, hε, rfl⟩
    refine ⟨{p : X × X | d α p.1 p.2 ≤ ε}, ⟨α, ε, hε, rfl⟩, ?_⟩
    rintro ⟨x, y⟩ hp
    have : d α y x ≤ ε := hp
    show d α x y ≤ ε
    rw [hsymm]
...
-- a `UniformSpace`/`PseudoMetricSpace` *value*, deliberately not an instance
set_option warn.classDefReducibility false in
/-- **146V**.4 (dils.tex:1843, Examples): the pseudometric
`d_ω(a,b) = ‖a - b‖_ω` attached to an np-functional `ω`, i.e. the case of
**146V**.3 (a vector space with a family of seminorms) at the ultrastrong
seminorms `‖b‖_ω = ω(b*b)^½` of **42III**. -/
noncomputable def omegaPseudoMetric (ω : NPFunctional 𝒷) : PseudoMetricSpace 𝒷 where
  dist a b := omegaNorm 𝒷 ω (a - b)
  dist_self a := by simp
  dist_comm a b := by
    show omegaNorm 𝒷 ω (a - b) = omegaNorm 𝒷 ω (b - a)
    rw [← omegaNorm_neg ω (a - b), neg_sub]
  dist_triangle a b c := omegaNorm_sub_le ω a b c

-- a `UniformSpace`/`PseudoMetricSpace` *value*, deliberately not an instance
set_option warn.classDefReducibility false in
/-- **146V**.4 (dils.tex:1843, Examples): the **ultrastrong uniformity** of a
von Neumann algebra, *as a uniform space* — the one fixed by the seminorms
`‖b‖_ω = ω(b*b)^½`, through the pseudometrics `d_ω` of **146V**.2.  (Thesis A
attaches the same seminorms to a *topology*, `Theses.A.VN.ultrastrong`,
**42III**; that topology is this uniformity's, `toTopologicalSpace_ultrastrongUniformity`
below, which is why the two names may be used of the same thing.) -/
noncomputable def ultrastrongUniformity : UniformSpace 𝒷 :=
  ⨅ ω : NPFunctional 𝒷, (omegaPseudoMetric 𝒷 ω).toUniformSpace

-- a `UniformSpace`/`PseudoMetricSpace` *value*, deliberately not an instance
set_option warn.classDefReducibility false in
/-- **146V**.4 (dils.tex:1843, Examples): the **ultraweak uniformity** of a
von Neumann algebra, *as a uniform space* — the one fixed by the seminorms
`|ω(b)|`, here as the coarsest uniformity making every np-functional
uniformly continuous, which is the same thing (`uniformity_ultraweakUniformity`
gives its subbase `{(a,b) ; |ω(a-b)| ≤ ε}` in the form **146V** asks for). -/
noncomputable def ultraweakUniformity : UniformSpace 𝒷 :=
  ⨅ ω : NPFunctional 𝒷, UniformSpace.comap (fun a => ω a) inferInstance

omit [CStarAlgebra 𝒷] [PartialOrder 𝒷] [StarOrderedRing 𝒷] in
private theorem iInf_uniformity_generate {ι : Type*} (u : ι → UniformSpace 𝒷)
    (S : ι → Set (Set (𝒷 × 𝒷)))
    (hS : ∀ i, @uniformity 𝒷 (u i) = Filter.generate (S i)) :
    @uniformity 𝒷 (⨅ i, u i) = Filter.generate (⋃ i, S i) := by
  rw [iInf_uniformity]
  refine le_antisymm ?_ ?_
  · refine Filter.le_generate_iff.mpr ?_
    rintro E hE
    rw [Set.mem_iUnion] at hE
    obtain ⟨i, hi⟩ := hE
    have h1 : E ∈ @uniformity 𝒷 (u i) := by
      rw [hS i]
      exact Filter.mem_generate_of_mem hi
    exact (iInf_le (fun i => @uniformity 𝒷 (u i)) i) h1
  · refine le_iInf fun i => ?_
    rw [hS i]
    refine Filter.le_generate_iff.mpr fun E hE => ?_
    exact Filter.mem_generate_of_mem (Set.mem_iUnion.mpr ⟨i, hE⟩)

/-- **146V**.4: the entourages of the ultrastrong uniformity are exactly the
ones generated by the subbase `E_{ω,ε} = {(a,b) ; ‖a-b‖_ω ≤ ε}` of
**146IIIa** — "the ultrastrong uniformity is fixed by the seminorms
`‖b‖_f = f(b*b)^½`". -/
theorem uniformity_ultrastrongUniformity :
    @uniformity 𝒷 (ultrastrongUniformity 𝒷) =
      Filter.generate {E : Set (𝒷 × 𝒷) | ∃ (ω : NPFunctional 𝒷) (ε : ℝ), 0 < ε ∧
        E = {p : 𝒷 × 𝒷 | omegaNorm 𝒷 ω (p.1 - p.2) ≤ ε}} := by
  rw [ultrastrongUniformity,
    iInf_uniformity_generate 𝒷 _
      (fun ω : NPFunctional 𝒷 => {E : Set (𝒷 × 𝒷) | ∃ ε : ℝ, 0 < ε ∧
        E = {p : 𝒷 × 𝒷 | omegaNorm 𝒷 ω (p.1 - p.2) ≤ ε}}) ?_]
  · congr 1
    ext E
    rw [Set.mem_iUnion]
    exact ⟨fun ⟨ω, ε, hε, hE⟩ => ⟨ω, ε, hε, hE⟩, fun ⟨ω, ε, hε, hE⟩ => ⟨ω, ε, hε, hE⟩⟩
  · intro ω
    have h := (@Metric.uniformity_basis_dist_le 𝒷 (omegaPseudoMetric 𝒷 ω)).eq_generate
    rw [h]
    congr 1
    ext E
    exact ⟨fun ⟨ε, hε, hE⟩ => ⟨ε, hε, hE.symm⟩, fun ⟨ε, hε, hE⟩ => ⟨ε, hε, hE.symm⟩⟩

/-- The topology of the ultrastrong *uniformity* is thesis A's ultrastrong
*topology* (**42III**, `Theses.A.VN.ultrastrong`): both are generated by the
balls of the seminorms `‖·‖_ω`.  This is what justifies the name in
**146V**.4. -/
theorem toTopologicalSpace_ultrastrongUniformity :
    (ultrastrongUniformity 𝒷).toTopologicalSpace = ultrastrong 𝒷 := by
  rw [ultrastrongUniformity, UniformSpace.toTopologicalSpace_iInf]
  refine le_antisymm ?_ ?_
  · refine le_generateFrom ?_
    rintro U ⟨ω, b, ε, hε, rfl⟩
    refine (iInf_le (fun ω : NPFunctional 𝒷 =>
      (omegaPseudoMetric 𝒷 ω).toUniformSpace.toTopologicalSpace) ω) _ ?_
    have : {a : 𝒷 | omegaNorm 𝒷 ω (a - b) < ε} = @Metric.ball 𝒷 (omegaPseudoMetric 𝒷 ω) b ε := rfl
    rw [this]
    exact @Metric.isOpen_ball 𝒷 (omegaPseudoMetric 𝒷 ω) b ε
  · refine le_iInf fun ω => ?_
    intro U hU
    rw [@Metric.isOpen_iff 𝒷 (omegaPseudoMetric 𝒷 ω)] at hU
    refine (@isOpen_iff_forall_mem_open 𝒷 (ultrastrong 𝒷) U).mpr fun x hx => ?_
    obtain ⟨ε, hε, hball⟩ := hU x hx
    refine ⟨{a : 𝒷 | omegaNorm 𝒷 ω (a - x) < ε}, hball, ?_, ?_⟩
    · exact TopologicalSpace.isOpen_generateFrom_of_mem ⟨ω, x, ε, hε, rfl⟩
    · show omegaNorm 𝒷 ω (x - x) < ε
      simpa using hε

omit [StarOrderedRing 𝒷] in
/-- The topology of the ultraweak *uniformity* is thesis A's ultraweak
*topology* (**42III**, `Theses.A.VN.ultraweak`): both are the initial
structure for the np-functionals. -/
theorem toTopologicalSpace_ultraweakUniformity :
    (ultraweakUniformity 𝒷).toTopologicalSpace = ultraweak 𝒷 := by
  rw [ultraweakUniformity, UniformSpace.toTopologicalSpace_iInf, ultraweak]
  rfl

omit [StarOrderedRing 𝒷] in
/-- **146V**.4: the entourages of the ultraweak uniformity are the ones
generated by the subbase `{(a,b) ; |ω(a-b)| ≤ ε}` — "the ultraweak uniformity
is given by the seminorms `|f(b)|`". -/
theorem uniformity_ultraweakUniformity :
    @uniformity 𝒷 (ultraweakUniformity 𝒷) =
      Filter.generate {E : Set (𝒷 × 𝒷) | ∃ (ω : NPFunctional 𝒷) (ε : ℝ), 0 < ε ∧
        E = {p : 𝒷 × 𝒷 | ‖ω (p.1 - p.2)‖ ≤ ε}} := by
  rw [ultraweakUniformity,
    iInf_uniformity_generate 𝒷 _
      (fun ω : NPFunctional 𝒷 => {E : Set (𝒷 × 𝒷) | ∃ ε : ℝ, 0 < ε ∧
        E = {p : 𝒷 × 𝒷 | ‖ω (p.1 - p.2)‖ ≤ ε}}) ?_]
  · congr 1
    ext E
    rw [Set.mem_iUnion]
    exact ⟨fun ⟨ω, ε, hε, hE⟩ => ⟨ω, ε, hε, hE⟩, fun ⟨ω, ε, hε, hE⟩ => ⟨ω, ε, hε, hE⟩⟩
  · intro ω
    have hpre : ∀ ε : ℝ,
        (Prod.map (fun a : 𝒷 => (ω a : ℂ)) fun a : 𝒷 => (ω a : ℂ)) ⁻¹'
            {p : ℂ × ℂ | dist p.1 p.2 ≤ ε}
          = {p : 𝒷 × 𝒷 | ‖ω (p.1 - p.2)‖ ≤ ε} := by
      intro ε
      ext p
      show dist (ω p.1) (ω p.2) ≤ ε ↔ ‖ω (p.1 - p.2)‖ ≤ ε
      rw [Complex.dist_eq, npFunctional_sub]
    rw [uniformity_comap,
      ((Metric.uniformity_basis_dist_le (α := ℂ)).comap
        (Prod.map (fun a : 𝒷 => (ω a : ℂ)) fun a : 𝒷 => (ω a : ℂ))).eq_generate]
    congr 1
    ext E
    exact ⟨fun ⟨ε, hε, hE⟩ => ⟨ε, hε, by rw [← hE, hpre]⟩,
      fun ⟨ε, hε, hE⟩ => ⟨ε, hε, by rw [hE, hpre]⟩⟩
...
section BewareExample

local notation "ℓ²" => lp (fun _ : ℕ => ℂ) 2

private theorem norm_single_one (n : ℕ) : ‖(lp.single 2 n (1 : ℂ) : ℓ²)‖ = 1 := by
  rw [lp.norm_single (by norm_num)]
  simp

private theorem ketbraNat_ne_zero (n : ℕ) : ketbraNat n n ≠ 0 := by
  intro h
  have h1 : ‖ketbraNat n n‖ = 1 := by
    rw [ketbraNat, ketbra_norm, norm_single_one, mul_one]
  rw [h, norm_zero] at h1
  exact zero_ne_one h1

/-- **149IIb** (dils.tex:2182, Beware), the `B(ℓ²)` counterexample of its
first clause: the projections `|n⟩⟨n|` of **43II**, in `B(ℓ²)` viewed as a
Hilbert module over itself, are an **orthonormal** family whose expansion of
`1` does **not** converge in norm — indeed
`‖1 - ∑_{n ∈ s} |n⟩⟨n|⟨|n⟩⟨n|, 1⟩‖ = 1` for every finite `s`.

This is what the Beware needs: some authors (`landi2012orthogonal`,
`manuilov2000hilbertc`) call an orthonormal `E ⊆ X` a basis when
...
multiplying by `|k⟩⟨k|` for `k ∉ s` would give `|k⟩⟨k| = 0`, against
`‖|k⟩⟨k|‖ = 1` — and a non-zero projection of a C*-algebra has norm one, by
`‖q‖² = ‖q*q‖ = ‖q‖`. -/
theorem onbasis_beware_ketbraNat :
    OrthonormalFam (ℓ² →L[ℂ] ℓ²) (fun n : ℕ => ketbraNat n n) ∧
      (∀ s : Finset ℕ,
        ‖(1 : ℓ² →L[ℂ] ℓ²) -
      ...
      ¬ Tendsto
        (fun s : Finset ℕ =>
          ∑ n ∈ s, (inner (ℓ² →L[ℂ] ℓ²) (ketbraNat n n) 1 : ℓ² →L[ℂ] ℓ²) • ketbraNat n n)
        atTop (𝓝 (1 : ℓ² →L[ℂ] ℓ²)) := by
  ...
  -- the partial sums are the projections `∑_{n ∈ s} |n⟩⟨n|`
  have hpart : ∀ s : Finset ℕ,
      ∑ n ∈ s, (inner (ℓ² →L[ℂ] ℓ²) (ketbraNat n n) 1 : ℓ² →L[ℂ] ℓ²) • ketbraNat n n
        = ∑ n ∈ s, ketbraNat n n := by
    intro s
    refine Finset.sum_congr rfl fun n _ => ?_
    have h1 : (inner (ℓ² →L[ℂ] ℓ²) (ketbraNat n n) 1 : ℓ² →L[ℂ] ℓ²) = ketbraNat n n := by
      rw [WithCStarModule.inner_def, one_mul, hstar]
    rw [h1, smul_eq_mul, hmul n]
  -- `1 - ∑_{n ∈ s} |n⟩⟨n|` is a non-zero projection, so has norm one
  have hkey : ∀ s : Finset ℕ, ‖(1 : ℓ² →L[ℂ] ℓ²) - ∑ n ∈ s, ketbraNat n n‖ = 1 := by
    intro s
    set q : ℓ² →L[ℂ] ℓ² := 1 - ∑ n ∈ s, ketbraNat n n with hq
    have hqstar : star q = q := by
      rw [hq, star_sub, star_one, star_sum]
      exact congrArg _ (Finset.sum_congr rfl fun n _ => hstar n)
    have hPidem : (∑ n ∈ s, ketbraNat n n) * (∑ n ∈ s, ketbraNat n n)
        = ∑ n ∈ s, ketbraNat n n := by
      rw [Finset.sum_mul]
    ...
    have hqidem : q * q = q := by
      rw [hq, sub_mul, mul_sub, mul_sub, one_mul, mul_one, hPidem]
      abel
    have hqne : q ≠ 0 := by
      intro h
      obtain ⟨k, hk⟩ := Infinite.exists_notMem_finset s
      have hsum : ∑ n ∈ s, ketbraNat n n = 1 := by
        have h2 : (1 : ℓ² →L[ℂ] ℓ²) - ∑ n ∈ s, ketbraNat n n = 0 := h
        have := sub_eq_zero.mp h2
        exact this.symm
      have h1 : ketbraNat k k * ∑ n ∈ s, ketbraNat n n = 0 := by
        rw [Finset.mul_sum]
        refine Finset.sum_eq_zero fun n hn => ?_
        exact hmul0 k n (fun hkn => hk (hkn ▸ hn))
      rw [hsum, mul_one] at h1
      exact ketbraNat_ne_zero k h1
    have hnormsq : ‖q‖ * ‖q‖ = ‖q‖ := by
      have h := CStarRing.norm_star_mul_self (x := q)
      rw [hqstar, hqidem] at h
      exact h.symm
    have hnorm0 : ‖q‖ ≠ 0 := fun h => hqne (norm_eq_zero.mp h)
    exact mul_right_cancel₀ hnorm0 (by rw [hnormsq, one_mul])
  refine ⟨⟨fun m n hmn => ?_, fun n => ⟨⟨?_, ?_⟩, ?_⟩⟩, fun s => ?_, ?_⟩
  · rw [WithCStarModule.inner_def, hstar, hmul0 n m (Ne.symm hmn)]
  · show IsIdempotentElem (inner (ℓ² →L[ℂ] ℓ²) (ketbraNat n n) (ketbraNat n n) : ℓ² →L[ℂ] ℓ²)
    rw [WithCStarModule.inner_def, hstar, hmul n]
    show ketbraNat n n * ketbraNat n n = ketbraNat n n
    rw [hmul n]
  · show IsSelfAdjoint (inner (ℓ² →L[ℂ] ℓ²) (ketbraNat n n) (ketbraNat n n) : ℓ² →L[ℂ] ℓ²)
    rw [WithCStarModule.inner_def, hstar, hmul n]
    exact hstar n
  · rw [WithCStarModule.inner_def, hstar, hmul n]
    exact ketbraNat_ne_zero n
  · rw [hpart s]
    exact hkey s
  · intro htend
    rw [Metric.tendsto_nhds] at htend
    obtain ⟨s, hs⟩ := (htend 1 zero_lt_one).exists
    rw [dist_eq_norm, hpart s, ← norm_neg, neg_sub, hkey s] at hs
